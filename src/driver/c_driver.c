...
static void decide_external_preprocessor(void) {
  if (driver_preprocessor != NULL) return;
...
static void init_external_preprocessor(void) {
  ...
  driver_add_flag(o, "-D__CPARSER_PATCHLEVEL__=\"%s\"", CPARSER_PATCHLEVEL);
...
static void determine_unit_standard(compilation_unit_t *unit) {
  if (unit->standard != STANDARD_DEFAULT) return;
...
static void init_c_dialect(bool const is_cpp,
                           compilation_unit_t const *const unit) {
  ...
  if (!is_cpp) {
    switch (standard) {
      ...
      case STANDARD_C99:
        ...
        break;
      case STANDARD_C11:
        features = _C89 | _C99 | _C11;
        break;
      case STANDARD_GNU89:
        features = _C89 | _GNUC;
        break;
      case STANDARD_GNU11:
        features = _C89 | _C99 | _C11 | _GNUC;
        break;
      ...
      case STANDARD_DEFAULT: {
        ...
        warningf(WARN_OTHER, &pos,
                 "command line option '%hs%hs' is not valid for C",
                 "-std=", str_lang_standard(standard));
      } /* FALLTHROUGH */
      case STANDARD_GNU99:
        features = _C89 | _C99 | _GNUC;
        break;
      default:
        panic("invalid standard");
    }
  ...
  dialect.c89 = dialect.c99 = dialect.c11 = dialect.gnu = dialect.ms =
      dialect.cpp = false;
  if (features & _C11) dialect.c89 = dialect.c99 = dialect.c11 = true;
  if (features & _C99) dialect.c89 = dialect.c99 = true;
  if (features & _C89) dialect.c89 = true;
  if (features & _GNUC) dialect.gnu = true;
  if (features & _MS) dialect.ms = true;
  if (features & _CXX) dialect.c89 = dialect.cpp = true;
...
static const char *get_dependency_filename(compilation_env_t *env,
  ...
  if (dependency_file != NULL) return dependency_file;
...
static bool run_external_preprocessor(compilation_env_t *env,
  ...
  if (lang) driver_add_flag(&cppflags_obst, "-x%s", lang);

  if (unit->type == COMPILATION_UNIT_C || unit->type == COMPILATION_UNIT_CXX) {
  ...
  if (is_warn_on(WARN_UNDEF)) driver_add_flag(&cppflags_obst, "-Wundef");
  ...
  if (is_warn_on(WARN_COMMENT)) driver_add_flag(&cppflags_obst, "-Wcomment");
...
static void decide_assembler(void) {
  if (driver_assembler != NULL) return;
...
static bool assemble(compilation_unit_t *unit, const char *o_name) {
  if (!asflags) asflags = obstack_nul_finish(&asflags_obst);

  decide_assembler();
  obstack_printf(&asflags_obst, "%s", driver_assembler);
  if (asflags[0] != '\0') obstack_printf(&asflags_obst, " %s", asflags);
...
static void append_standard_include_paths(void) {
  ...
  const char *triple = multilib_directory_target_triple != NULL
                           ? multilib_directory_target_triple
                           : target.triple;
  if (triple != NULL) {
...
static bool start_preprocessing(compilation_env_t *env,
                                compilation_unit_t *unit) {
  (void)env;
  if (!open_input(unit)) return false;

  init_parser_and_ast(unit);
  if (!driver_no_stdinc) append_standard_include_paths();
  append_environment_include_paths();
  setup_preprocessor();
  process_cmdline_defines();
  if (driver_verbose) print_include_paths();
...
static bool preprocess(compilation_env_t *env, compilation_unit_t *unit) {
  ...
  compilation_unit_handler preprocessor = driver_use_integrated_preprocessor
                                              ? start_preprocessing
                                              : run_external_preprocessor;
...
static bool finish_preprocessing(compilation_env_t *env,
  ...
  if (dump_defines) print_defines();

  close_pp_input();
  set_preprocessor_output(NULL);
  bool res = close_input(unit);
  if (!res || error_count > 0) return false;

  if (construct_dep_target) {
    const char *dep_target_name = get_dependency_filename(env, unit);
    FILE *dep_out = fopen(dep_target_name, "w");
    if (dep_out == NULL) {
      errorf(NULL, "Opening dependency file '%s' failed: %s", dep_target_name,
             strerror(errno));
      return false;
    }
...
bool print_preprocessing_tokens(compilation_env_t *env,
                                compilation_unit_t *unit) {
  if (!open_output(env)) return false;
...
bool generate_code_final(compilation_env_t *env, compilation_unit_t *unit) {
  if (!open_output_for_unit(env, unit, ".s")) return false;
  bool res = do_generate_code(env->out, unit);
  close_output(env);
  if (!res) unlink(env->outname);
  return res;
}
...
bool link_program(compilation_env_t *env, compilation_unit_t *units) {
  ...
  for (compilation_unit_t *unit = units; unit != NULL; unit = unit->next) {
    if (unit->type != COMPILATION_UNIT_OBJECT) continue;

    driver_add_flag(&file_obst, "%s", unit->name);
  }

  if (lsysroot) obstack_printf(&file_obst, " --sysroot=%s", lsysroot);
  ...
  char commandline_without_nopie[strlen(commandline) + strlen(redirect) +
                                 strlen(err_without_nopie) + 1];
  snprintf(commandline_without_nopie, sizeof(commandline_without_nopie),
           "%s%s%s", commandline, redirect, err_without_nopie);
  ...
  char commandline_nopie[strlen(commandline) + strlen(no_pie) +
                         strlen(redirect) + strlen(err_nopie) + 1];
  snprintf(commandline_nopie, sizeof(commandline_nopie), "%s%s%s%s",
           commandline, no_pie, redirect, err_nopie);

  /* Workaround for systems that expect PIE code when no flags are given:
   * Try linking with -no-pie flag first if we do not explicitly want PIC code.
   * Old linkers don't support the -no-pie flag, so we have to try linking
   * without the flag if the first try failed. */
  bool try_nopie_first = !target.pic;

  char *first_linker_pass =
      try_nopie_first ? commandline_nopie : commandline_without_nopie;
...
bool write_ir_file(compilation_env_t *env, compilation_unit_t *unit) {
  if (!open_output_for_unit(env, unit, ".ir")) return false;
...
bool dump_irg(compilation_env_t *env, compilation_unit_t *units) {
  ...
  if (env->outname == NULL) env->outname = "a.vcg";
  if (!open_output(env)) return false;
...
void set_default_handlers(void) {
  ...
  set_unit_handler(COMPILATION_UNIT_PREPROCESSED_CXX, start_preprocessing,
                   false);
