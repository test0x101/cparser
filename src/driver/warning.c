...
#define M(warning, state, option, explanation) \
  [warning] = {state, option, explanation},
    WARNINGS(M)
#undef M
#undef OFF
#undef ON
...
#define OPTX(x) else if (streq(s, x))
  ...
  OPTX("comments") { SET(WARN_COMMENT); }
  OPTX("extra") {
  extra:
    /* TODO */
  ...
  OPTX("parentheses") { warn_parentheses(on, off); }
  OPTX("unused") { warn_unused(on, off); }
#undef SET
#undef OPT_X
  else if (streq(opt /* sic */, "error-implicit-function-declaration")) {
    /* GCC legacy: This way it only can be activated. */
    warning[WARN_IMPLICIT_FUNCTION_DECLARATION].state =
        WARN_STATE_ON | WARN_STATE_ERROR;
  }
  else {
    warningf(WARN_UNKNOWN_WARNING_OPTION, NULL,
             "ignoring unknown option '%hs%hs'", "-W", opt);
  }
}

void disable_all_warnings(void) {
  for (warning_switch_t *i = warning; i != endof(warning); ++i) {
    if (i != &warning[WARN_ERROR] && i != &warning[WARN_FATAL_ERRORS]) {
