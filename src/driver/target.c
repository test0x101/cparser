...
void target_adjust_types_and_dialect(void) {
  ...
  assert(ir_platform_type_size(IR_TYPE_SHORT) == props[ATOMIC_TYPE_SHORT].size);
  ...
  assert(ir_platform_type_size(IR_TYPE_FLOAT) == props[ATOMIC_TYPE_FLOAT].size);
...
static bool is_ia32_cpu(const char *cpu) {
  ...
  return cpu[0] == 'i' && cpu[2] == '8' && cpu[3] == '6' && cpu[1] >= '3' &&
         cpu[1] <= '7';
}
...
void set_target_option(char const *const arg) {
  ...
  if (!res)
    panic(
        "setting firm backend option '%s' failed (maybe an outdated version of "
        "firm is used)",
        arg);
}
...
static bool pass_options_to_firm_be(void) {
  ...
  for (codegen_option_t *option = codegen_options; option != NULL;
    ...
    static char const *const pass_to_cpp_and_ld[] = {"soft-float"};
...
static void determine_target_machine(void) {
  if (target.machine == NULL) target.machine = ir_get_host_machine_triple();
...
static void set_options_from_be(void) {
  ...
  if (strstart(ir_triple_get_operating_system(target.machine), "mingw")) {
    ...
    if (ir_target_pointer_size() == 4) dialect.support_fastcall_stdcall = true;
  }
}
