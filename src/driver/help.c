...
static void print_help_basic(const char *argv0) {
  ...
  help_simple("--help-preprocessor",
              "Display information about preprocessor options");
  ...
  help_simple("--help-codegen",
              "Display information about code-generation options");
  help_simple("--help-optimization",
              "Display information about optimization options");
  help_simple("--help-linker", "Display information about linker options");
  help_simple("--help-language-tools",
              "Display information about language tools options");
  help_simple("--help-debug",
              "Display information about compiler debugging options");
...
static void print_help_preprocessor(void) {
  ...
  help_equals("--sysroot", "DIR",
              "set (default) root directory for (headers and) libraries");
  ...
  help_prefix("-MF", "file", "");
  help_prefix("-idirafter", "DIR",
              "Append to header searchpath (after -I dirs)");
  help_prefix("-iquote", "DIR",
              "Append to header searchpath for #include with quoted argument");
  help_prefix("-isystem", "DIR", "Append to system header searchpath");
  help_aprefix("-Wp,", "OPTION", "Pass option directly to preprocessor");
  help_spaced("-Xpreprocessor", "OPTION",
              "Pass option directly to preprocessor");
  help_equals("-finput-charset", "CHARSET", "Select encoding of input files");
  help_f_yesno("-fdollars-in-identifiers",
               "Accept dollar symbol in identifiers");
  /* Undocumented:
...
static void print_help_parser(void) {
  ...
  help_simple("-fshort-wchar",
              "Type \"wchar_t\" is unsigned short instead of int");
  ...
  help_f_yesno("-ffreestanding",
               "Compile in freestanding mode (see ISO C standard)");
  help_f_yesno("-fhosted", "Compile in hosted (not freestanding) mode");
  help_f_yesno("-fbuiltin",
               "Expect standard semantic from libc function names");
  /* Undocumented:
   * help_equals("--std", "", "");
   */
}
...
static void print_help_warnings(void) {
  help_f_yesno("-fdiagnostics-show-option",
               "Show the switch, which controls a warning, after each warning");
  help_simple("-w", "Disable all warnings");
  help_simple("-pedantic",
              "Enable pedantic warnings (equivalent to -Wpedantic)");
  help_simple("-pedantic-errors",
              "Error on pedantic warnings (equivalent to -Werror=pedantic)");
...
static void print_help_optimization(void) {
  ...
  put_choice("3",
             "Aggressively optimize at the expense of compilation speed and "
             "code size");
...
static void print_help_codegeneration(void) {
  help_spaced("-target", "TARGET",
              "Specify target architecture as CPU-manufacturer-OS triple");
  ...
  help_equals("-fvisibility", "VISIBILITY",
              "Set default visibility for external symbols");
  put_choice("default", "");
  put_choice("internal", "");
  put_choice("hidden", "");
  put_choice("protected", "");
  help_f_yesno("-fomit-frame-pointer",
               "Produce code without frame pointer where possible");
  help_f_yesno("-fprofile-generate",
               "Generate instrumented code to collect profile information");
  help_f_yesno("-fprofile-use",
               "Use profile information generated by instrumented binaries");
...
static void print_help_debug(void) {
  ...
  help_spaced("--dump-function", "FUNC",
              "Preprocess, parse and output vcg graph of func");
  help_simple(
      "--export-ir",
      "Preprocess, parse and output compiler intermediate representation");
...
static void print_help_language_tools(void) {
  help_simple(
      "--print-fluffy",
      "Preprocess, parse and generate declarations for the fluffy language");
  help_simple("--print-jna",
              "Preprocess, parse and generate declarations for JNA");
  help_spaced("--jna-limit", "FILENAME", "");
  help_spaced("--jna-libname", "NAME", "");
  help_simple("--print-compound-sizes",
              "Preprocess, parse and print sizes of compound types marked with "
              "dllexport");
}
