...
static const char *get_max_string(atomic_type_kind_t akind) {
  ...
  if (flags & ATOMIC_TYPE_FLAG_SIGNED) --bits;
  switch (bits) {
    case 7:
      return "127";
    case 8:
...
static void define_type_min(const char *name, atomic_type_kind_t akind) {
  ...
  if (flags & ATOMIC_TYPE_FLAG_SIGNED) {
    ...
    add_define_prop_fmt("__%s_MIN__", name, "0%s", get_literal_suffix(akind));
  }
}
...
static void define_float_properties(const char *prefix,
  ...
  if (arith == irma_ieee754 && exp_size == 8 && mant_size == 23) {
    ...
    p = &prop_f64;
  } else if (arith == irma_x86_extended_float && exp_size == 15 &&
             mant_size == 64) {
...
static bool is_ILP(unsigned const size_int, unsigned const size_long,
                   unsigned const size_pointer) {
  return get_atomic_type_size(ATOMIC_TYPE_INT) == size_int &&
...
void add_predefined_macros(void) {
  ...
  if (dialect.c99) add_define("__STDC_VERSION__", "199901L", true);
  if (dialect.cpp) add_define("__cplusplus", "1", true);
  ...
  if (dialect.cpp) add_define("__GNUG__", "4", false);

  if (!firm_is_inlining_enabled()) add_define("__NO_INLINE__", "1", false);
  if (dialect.c99) {
  ...
  for (atomic_type_kind_t i = ATOMIC_TYPE_FIRST; i <= ATOMIC_TYPE_LAST; ++i) {
    ...
    if (!(flags & ATOMIC_TYPE_FLAG_INTEGER)) continue;
  ...
  for (ir_platform_define_t const *define = ir_platform_define_first();
    ...
    if (name[0] != '_' && !dialect.gnu) continue;
