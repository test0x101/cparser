...
static const char *prefix_arg(const char *prefix, options_state_t *s) {
  const char *option = &s->argv[s->i][1];
  if (!strstart(option, prefix)) return NULL;

  const size_t prefix_len = strlen(prefix);
  const char *def = &option[prefix_len];
  if (def[0] != '\0') return def;
...
char const *spaced_arg(char const *const arg, options_state_t *const s) {
  ...
  if (streq(option, arg)) {
    if (s->i + 1 < s->argc) return s->argv[++s->i];
...
static bool f_yesno_arg(char const *const arg, options_state_t const *const s) {
  ...
  if (option[2] == 'n' && option[3] == 'o' && option[4] == '-') option += 3;
...
static bool accept_prefix(options_state_t *const s, char const *const prefix,
  ...
  assert(option[0] == '-');
  assert(prefix[0] == '-');
  if (!strstart(&option[1], &prefix[1])) return false;
...
static compilation_unit_type_t get_unit_type_from_string(const char *string) {
  ...
  if (streq(string, "assembler-with-cpp")) return COMPILATION_UNIT_ASSEMBLER;
  if (streq(string, "none")) return COMPILATION_UNIT_AUTODETECT;
...
bool options_parse_linker(options_state_t *s) {
  ...
  if ((arg = prefix_arg("l", s)) != NULL) {
    ...
    driver_add_flag(&ldflags_obst, "-L%s", arg);
  } else if (simple_arg("static", s) || simple_arg("no-pie", s) ||
             simple_arg("nopie", s) || simple_arg("nodefaultlibs", s) ||
             simple_arg("nostartfiles", s) || simple_arg("nostdlib", s) ||
             simple_arg("pie", s) || simple_arg("rdynamic", s) ||
             simple_arg("s", s) || simple_arg("shared", s) ||
             simple_arg("shared-libgcc", s) || simple_arg("static-libgcc", s) ||
...
bool options_parse_codegen(options_state_t *s) {
  ...
  if ((arg = equals_arg("falign-loops", s)) != NULL ||
    ...
    warningf(WARN_COMPAT_OPTION, NULL, "ignoring gcc option '%s'", full_option);
    ...
    *codegen_options_anchor = cg_option;
    ...
    if ((fopt = f_no_arg(&truth_value, s)) != NULL) {
      if (f_yesno_arg("-ffast-math", s)) {
        ...
        warningf(WARN_COMPAT_OPTION, NULL, "ignoring gcc option '-f%s'", fopt);
    ...
    if ((arg = equals_arg("fmessage-length", s)) != NULL) {
      ...
      if ((fopt = f_no_arg(&truth_value, s)) != NULL) {
        if (f_yesno_arg("-fdiagnostics-show-option", s)) {
          ...
          diagnostic_enable_color(truth_value ? (colorterm != 0 ? colorterm : 8)
                                              : 0);
        } else {
          return false;
        }
...
bool parse_target_triple(char const *const arg) {
  ...
  if (triple == NULL) {
    errorf(NULL,
           "target-triple '%s' is not in the form "
...
bool options_parse_early_target(options_state_t *s) {
  ...
  if (simple_arg("pthread", s)) {
    ...
    if (!parse_target_triple(arg)) s->argument_errors = true;
