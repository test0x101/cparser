...
static const char *get_tempdir(void) {
  ...
  if (tmpdir == NULL) tmpdir = try_dir(getenv("TMPDIR"));
  if (tmpdir == NULL) tmpdir = try_dir(getenv("TMP"));
  if (tmpdir == NULL) tmpdir = try_dir(getenv("TEMP"));

#ifdef P_tmpdir
  if (tmpdir == NULL) tmpdir = try_dir(P_tmpdir);
#endif

  if (tmpdir == NULL) tmpdir = try_dir("/tmp");
  if (tmpdir == NULL) tmpdir = try_dir("/var/tmp");
  if (tmpdir == NULL) tmpdir = try_dir("/usr/tmp");
...
static char *mkdtemp(char *templ) {
  ...
  if (res && mkdir(templ, 0700) != 0) res = NULL;
  return res;
}
#endif
...
FILE *make_temp_file(const char *name_orig, const char **name_result) {
  if (tempsubdir == NULL) tempsubdir = make_tempsubdir(get_tempdir());
