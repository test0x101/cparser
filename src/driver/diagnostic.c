...
static void diagnosticvf(position_t const *const pos,
  ...
  if (pos) {
    if (pos->colno != 0 && show_column) {
      fprintf(out, "%s:%u:%u: ", pos->input_name, pos->lineno,
              (unsigned)pos->colno);
  ...
  done_flags:;
    ...
    if (!flag_high) flag_high = strchr("EKNQTYk", *f);

    if (flag_high) fputs(colors.highlight, out);
    switch (*f++) {
      case '%':
        fputc('%', out);
        break;
    ...
    if (flag_high) fputs(colors.reset_highlight, out);
  }
  fputs(fmt, out);  // Print rest.
}

static void errorvf(const position_t *pos, const char *const fmt, va_list ap) {
  ++error_count;
  diagnosticvf(pos, colors.error, "error", fmt, ap);
  fputc('\n', stderr);
  if (is_warn_on(WARN_FATAL_ERRORS)) exit(EXIT_FAILURE);
}
...
bool warningf(warning_t const warn, position_t const *pos,
