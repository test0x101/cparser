...
void timer_init(void) { timers_inited = 1; }
...
void timer_push(ir_timer_t *timer) {
  if (timers_inited) ir_timer_push(timer);
}

void timer_pop(ir_timer_t *timer) {
  if (timers_inited) ir_timer_pop(timer);
}

void timer_start(ir_timer_t *timer) {
  if (timers_inited) ir_timer_start(timer);
}

void timer_stop(ir_timer_t *timer) {
  if (timers_inited) ir_timer_stop(timer);
}
