...
FILE *open_temp_file(const char *basename, const char *extension,
  ...
  snprintf(uniquenum_extension, sizeof(uniquenum_extension), "-%u%s", nextnum++,
           extension);
...
bool open_input(compilation_unit_t *unit) {
  ...
  if (unit->input != NULL) return true;
...
bool process_unit(compilation_env_t *env, compilation_unit_t *unit) {
  ...
  for (;;) {
    ...
    if (!handler) panic("incomplete handler chain on '%s'", unit->name);
    if (!handler(env, unit)) {
      res = false;
      break;
    } else if (stop_after[type]) {
      res = true;
      break;
    }
...
bool process_all_units(compilation_env_t *env) {
  ...
  for (compilation_unit_t *unit = units; unit != NULL; unit = unit->next) {
    ...
    if (unit->original_name && env->outname &&
        streq(unit->original_name, env->outname)) {
      errorf(NULL, "output file '%s' is the same as the input file",
             env->outname);
      return false;
    }
...
void end_statistics(void) {
  if (stat_ev_enabled) stat_ev_end();
}
...
compilation_unit_type_t autodetect_input(char const *const path) {
  ...
  if (suffix == name) return COMPILATION_UNIT_OBJECT;
  ++suffix;
  return streq(suffix, "S")     ? COMPILATION_UNIT_ASSEMBLER
         ...
         : streq(suffix, "s")
             ...
             : COMPILATION_UNIT_OBJECT; /* gcc behavior: unknown file extension
                                           means object file */
}
...
static int detect_color_terminal(void) {
  ...
  if (!isatty(STDOUT_FILENO) || !isatty(STDERR_FILENO)) return 0;

  char *term = getenv("TERM");
  if (term == NULL || streq(term, "dumb")) return 0;
  if (strstr(term, "256color") != 0) return 256;
  return 8;
}
...
void exit_driver(void) { obstack_free(&file_obst, NULL); }
