...
static void print_expression_prec(const expression_t *expression,
                                  unsigned prec);
...
void print_indent(void) {
  for (int i = 0; i < indent; ++i) print_char('\t');
}
...
static void print_string_literal(
    string_literal_expression_t const *const literal, char const delimiter) {
...
static void print_funcname(const funcname_expression_t *funcname) {
  ...
  switch (funcname->kind) {
    case FUNCNAME_FUNCTION:
      s = dialect.c99 ? "__func__" : "__FUNCTION__";
      break;
...
static void print_binary_expression(const binary_expression_t *binexpr) {
  ...
  switch (binexpr->base.kind) {
    ...
    case EXPR_BINARY_SHIFTRIGHT_ASSIGN:
      ...
      break;
    default:
      panic("invalid binexpression found");
  }
  print_string(op);
...
static void print_unary_expression(const unary_expression_t *unexpr) {
  ...
  switch (unexpr->base.kind) {
    ...
    case EXPR_UNARY_REAL:
      print_string("__real__ ");
      break;
    case EXPR_UNARY_IMAG:
      print_string("__imag__ ");
      break;
 ...
 * @param ref   the reference expression
 */
static void print_label_address_expression(
    const label_address_expression_t *le) {
...
static void print_typeprop_expression(const typeprop_expression_t *expression) {
  switch (expression->base.kind) {
    case EXPR_SIZEOF:
      ...
      break;
    case EXPR_ALIGNOF:
      print_string(dialect.c11 ? "_Alignof" : "__alignof__");
      break;
    default:
      panic("invalid typeprop kind");
  }
 ...
 */
static void print_builtin_constant(
...
static void print_select(const select_expression_t *expression) {
   ...
   */
  if (expression->compound_entry->base.symbol == NULL) return;
...
static bool needs_parentheses(expression_t const *const expr,
                              unsigned const top_prec) {
  if (expr->base.parenthesized) return true;

  if (top_prec > get_expression_precedence(expr->kind)) return true;
 ...
 */
static void print_expression_prec(expression_t const *expr,
                                  unsigned const top_prec) {
  if (expr->kind == EXPR_UNARY_CAST && expr->base.implicit &&
      !print_implicit_casts) {
    expr = expr->unary.value;
  }

  bool const parenthesized = needs_parentheses(expr, top_prec);

  if (parenthesized) print_char('(');
  switch (expr->kind) {
    case EXPR_ALIGNOF:
    case EXPR_SIZEOF:
      print_typeprop_expression(&expr->typeprop);
      break;
    case EXPR_ARRAY_ACCESS:
      print_array_expression(&expr->array_access);
      break;
    case EXPR_BINARY_CASES:
      print_binary_expression(&expr->binary);
      break;
    case EXPR_BUILTIN_CONSTANT_P:
      print_builtin_constant(&expr->builtin_constant);
      break;
    case EXPR_BUILTIN_TYPES_COMPATIBLE_P:
      print_builtin_types_compatible(&expr->builtin_types_compatible);
      break;
    case EXPR_CALL:
      print_call_expression(&expr->call);
      break;
    case EXPR_CLASSIFY_TYPE:
      print_classify_type_expression(&expr->classify_type);
      break;
    case EXPR_COMPOUND_LITERAL:
      print_compound_literal(&expr->compound_literal);
      break;
    case EXPR_CONDITIONAL:
      print_conditional(&expr->conditional);
      break;
    case EXPR_ERROR:
      print_string("$error$");
      break;
    case EXPR_FUNCNAME:
      print_funcname(&expr->funcname);
      break;
    case EXPR_LABEL_ADDRESS:
      print_label_address_expression(&expr->label_address);
      break;
    case EXPR_LITERAL_CASES:
      print_literal(&expr->literal);
      break;
    case EXPR_LITERAL_CHARACTER:
      print_string_literal(&expr->string_literal, '\'');
      break;
    case EXPR_OFFSETOF:
      print_offsetof_expression(&expr->offsetofe);
      break;
    case EXPR_REFERENCE:
    case EXPR_ENUM_CONSTANT:
      print_reference_expression(&expr->reference);
      break;
    case EXPR_SELECT:
      print_select(&expr->select);
      break;
    case EXPR_STATEMENT:
      print_statement_expression(&expr->statement);
      break;
    case EXPR_STRING_LITERAL:
      print_string_literal(&expr->string_literal, '"');
      break;
    case EXPR_UNARY_CASES:
      print_unary_expression(&expr->unary);
      break;
    case EXPR_VA_ARG:
      print_va_arg(&expr->va_arge);
      break;
    case EXPR_VA_COPY:
      print_va_copy(&expr->va_copye);
      break;
    case EXPR_VA_START:
      print_va_start(&expr->va_starte);
      break;
  }
  if (parenthesized) print_char(')');
}
...
static void print_indented_statement(statement_t const *const stmt) {
  switch (stmt->kind) {
    ...
    case STATEMENT_CASE_LABEL:
      for (int i = 0; i != case_indent; ++i) print_char('\t');
      break;
...
static void print_compound_statement(const compound_statement_t *block) {
  ...
  for (statement_t const *stmt = block->statements; stmt;
       stmt = stmt->base.next) {
 ...
 */
static void print_expression_statement(
 ...
 */
static void print_computed_goto_statement(
    computed_goto_statement_t const *const stmt) {
  print_string("goto *");
...
static bool is_generated_entity(const entity_t *entity) {
  if (is_declaration(entity)) return entity->declaration.implicit;
...
static void print_declaration_statement(
  ...
  for (; entity != end; entity = entity->base.next) {
    if (entity->kind == ENTITY_ENUM_VALUE) continue;
    if (is_generated_entity(entity)) continue;
...
static void print_asm_arguments(entity_t const *const arguments) {
  ...
  for (entity_t const *i = arguments; i; i = i->base.next) {
    ...
    if (symbol != NULL) print_format("[%s] ", symbol->string);
...
static void print_asm_statement(asm_statement_t const *const stmt) {
  ...
  unsigned const n = stmt->labels          ? 4
                     : stmt->clobbers      ? 3
                     : stmt->inputs        ? 2
...
void print_statement(statement_t const *const stmt) {
  switch (stmt->kind) {
    case STATEMENT_ASM:
      print_asm_statement(&stmt->asms);
      break;
    case STATEMENT_BREAK:
      print_string("break;");
      break;
    case STATEMENT_CASE_LABEL:
      print_case_label(&stmt->case_label);
      break;
    case STATEMENT_COMPOUND:
      print_compound_statement(&stmt->compound);
      break;
    case STATEMENT_COMPUTED_GOTO:
      print_computed_goto_statement(&stmt->computed_goto);
      break;
    case STATEMENT_CONTINUE:
      print_string("continue;");
      break;
    case STATEMENT_DECLARATION:
      print_declaration_statement(&stmt->declaration);
      break;
    case STATEMENT_DO_WHILE:
      print_do_while_statement(&stmt->do_while);
      break;
    ...
    case STATEMENT_EXPRESSION:
      print_expression_statement(&stmt->expression);
      break;
    case STATEMENT_FOR:
      print_for_statement(&stmt->fors);
      break;
    case STATEMENT_GOTO:
      print_goto_statement(&stmt->gotos);
      break;
    case STATEMENT_IF:
      print_if_statement(&stmt->ifs);
      break;
    case STATEMENT_LABEL:
      print_label_statement(&stmt->label);
      break;
    case STATEMENT_RETURN:
      print_return_statement(&stmt->returns);
      break;
    case STATEMENT_SWITCH:
      print_switch_statement(&stmt->switchs);
      break;
  }
}
...
expression_classification_t is_constant_initializer(
  ...
  switch (initializer->kind) {
    ...
    case INITIALIZER_LIST: {
      ...
      for (size_t i = 0; i < initializer->list.len; ++i) {
        ...
        expression_classification_t const cur =
            is_constant_initializer(sub_initializer);
        all = MIN(all, cur);
      }
      return all;
    }
  ...
  switch (expression->kind) {
    ...
    case EXPR_SELECT: {
      ...
      if (is_type_pointer(base_type)) {
        ...
        return is_object_with_linker_constant_address(
            expression->select.compound);
      }
    }

    case EXPR_ARRAY_ACCESS: {
      expression_classification_t const ref =
          is_linker_constant(expression->array_access.array_ref);
      expression_classification_t const idx =
    ...
    case EXPR_REFERENCE: {
      ...
      if (!is_declaration(entity)) return EXPR_CLASS_VARIABLE;

      switch (entity->declaration.storage_class) {
        case STORAGE_CLASS_NONE:
        case STORAGE_CLASS_EXTERN:
        case STORAGE_CLASS_STATIC:
          return entity->kind != ENTITY_VARIABLE ||
...
expression_classification_t is_linker_constant(const expression_t *expression) {
  switch (expression->kind) {
    ...
    case EXPR_UNARY_DEREFERENCE: {
      type_t *const type = skip_typeref(
    ...
    case EXPR_UNARY_CAST: {
      ...
      if (!is_type_pointer(dest) &&
          (dest->kind != TYPE_ATOMIC ||
           !(get_atomic_type_flags(dest->atomic.akind) &
             ATOMIC_TYPE_FLAG_INTEGER) ||
           get_atomic_type_size(dest->atomic.akind) <
               get_ctype_size(type_void_ptr)))
    ...
    case EXPR_REFERENCE: {
      ...
      if (is_type_function(type)) return EXPR_CLASS_CONSTANT;
    ...
    case EXPR_ARRAY_ACCESS: {
      ...
      if (!is_type_array(type)) return EXPR_CLASS_VARIABLE;
    ...
    case EXPR_CONDITIONAL: {
      ...
      if (cclass < EXPR_CLASS_CONSTANT) return cclass;
 ...
 */
static expression_classification_t is_builtin_const_call(
    const expression_t *expression) {
  expression_t *function = expression->call.function;
  if (function->kind != EXPR_REFERENCE) return EXPR_CLASS_VARIABLE;
  reference_expression_t *ref = &function->reference;
  if (ref->entity->kind != ENTITY_FUNCTION) return EXPR_CLASS_VARIABLE;
...
static expression_classification_t is_constant_pointer(
    const expression_t *expression) {
  expression_classification_t const expr_class =
      is_constant_expression(expression);
  if (expr_class != EXPR_CLASS_VARIABLE) return expr_class;
...
static expression_classification_t is_object_with_constant_address(
    const expression_t *expression) {
  switch (expression->kind) {
    case EXPR_SELECT: {
      expression_t *compound = expression->select.compound;
    ...
    case EXPR_ARRAY_ACCESS: {
      ...
      expression_classification_t const idx_class =
          is_constant_expression(array_access->index);
      if (idx_class < EXPR_CLASS_CONSTANT) return idx_class;
      expression_classification_t const ref_addr =
          is_object_with_constant_address(array_access->array_ref);
      expression_classification_t const ref_ptr =
...
expression_classification_t is_constant_expression(
    const expression_t *expression) {
  ...
  switch (expression->kind) {
    ...
    case EXPR_LITERAL_MS_NOOP:
...

      {
        ...
        case EXPR_CLASSIFY_TYPE:
          type = skip_typeref(
              expression->classify_type.type_expression->base.type);
          goto check_type;
        ...
        check_type:
          return is_type_valid(type) ? EXPR_CLASS_INTEGER_CONSTANT
                                     : EXPR_CLASS_ERROR;
      }
    ...
    case EXPR_BUILTIN_CONSTANT_P: {
      expression_classification_t const c =
          is_constant_expression(expression->builtin_constant.value);
      return c != EXPR_CLASS_ERROR ? EXPR_CLASS_INTEGER_CONSTANT
                                   : EXPR_CLASS_ERROR;
    }
    ...
    case EXPR_BINARY_MOD: {
      ...
      if (iconst && tarval_is_null(fold_expression(right)))
        return EXPR_CLASS_VARIABLE;
      expression_classification_t const l =
          is_constant_expression(expression->binary.left);
      return MIN(l, r);
    }
    ...
    case EXPR_BINARY_ISUNORDERED: {
      expression_classification_t const l =
          is_constant_expression(expression->binary.left);
      expression_classification_t const r =
    ...
    case EXPR_BINARY_LOGICAL_AND: {
      ...
      if (lcls < EXPR_CLASS_CONSTANT) return lcls;
      expression_classification_t const rcls =
          is_constant_expression(expression->binary.right);
      if (lcls == EXPR_CLASS_INTEGER_CONSTANT &&
          rcls == EXPR_CLASS_INTEGER_CONSTANT)
        return EXPR_CLASS_INTEGER_CONSTANT;
      if (!fold_expression_to_bool(left)) return EXPR_CLASS_CONSTANT;
    ...
    case EXPR_COMPOUND_LITERAL: {
      ...
      if (is_type_scalar(skipped)) {
        expression_classification_t const cls =
            is_constant_initializer(expression->compound_literal.initializer);
        return MIN(cls, EXPR_CLASS_CONSTANT);
    ...
    case EXPR_CONDITIONAL: {
      ...
      if (ccls < EXPR_CLASS_CONSTANT) return ccls;
      expression_t const *const t = expression->conditional.true_expression;
      expression_classification_t const tcls =
          t == NULL ? ccls : is_constant_expression(t);
      expression_classification_t const fcls =
          is_constant_expression(expression->conditional.false_expression);
      if (ccls >= EXPR_CLASS_CONSTANT && tcls == EXPR_CLASS_INTEGER_CONSTANT &&
          fcls == EXPR_CLASS_INTEGER_CONSTANT)
        return EXPR_CLASS_INTEGER_CONSTANT;
      expression_classification_t const cls =
          fold_expression_to_bool(cond) ? tcls : fcls;
      return MIN(cls, EXPR_CLASS_CONSTANT);
    }
...
type_t *get_string_type(string_encoding_t const enc) {
  ...
  switch (enc) {
    ...
    case STRING_ENCODING_UTF8:
      return warn ? type_const_char_ptr : type_char_ptr;
    case STRING_ENCODING_CHAR16:
      return warn ? type_char16_t_const_ptr : type_char16_t_ptr;
    case STRING_ENCODING_CHAR32:
      return warn ? type_char32_t_const_ptr : type_char32_t_ptr;
    case STRING_ENCODING_WIDE:
      return warn ? type_const_wchar_t_ptr : type_wchar_t_ptr;
  }
...
type_t *revert_automatic_type_conversion(const expression_t *expression) {
  switch (expression->kind) {
    ...
    case EXPR_UNARY_DEREFERENCE: {
      ...
      if (!is_type_pointer(type)) return type_error_type;
    ...
    case EXPR_ARRAY_ACCESS: {
      ...
      if (!is_type_pointer(type_left)) return type_error_type;
...
bool function_is_inline_only(function_t const *const function) {
  ...
  if (function->base.modifiers & DM_GNU_INLINE) goto gnu_c90_inline;
