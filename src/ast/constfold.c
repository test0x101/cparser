...
static ir_tarval *get_type_size_tarval(type_t *const type,
                                       ir_mode *const mode) {
...
static ir_tarval *fold_expression_to_address(expression_t const *const expr) {
  switch (expr->kind) {
    case EXPR_SELECT: {
      ...
      ir_tarval *const base_addr =
          is_type_pointer(type) ? fold_expression(sel->compound)
                                : fold_expression_to_address(sel->compound);
      ir_mode *const mode = get_tarval_mode(base_addr);
      ir_mode *const mode_offset = get_reference_offset_mode(mode);
      ir_tarval *const offset = new_tarval_from_long(
          sel->compound_entry->compound_member.offset, mode_offset);
      return tarval_add(base_addr, offset);
    }

    case EXPR_ARRAY_ACCESS: {
      ir_tarval *const base_addr =
          fold_expression_to_address(expr->array_access.array_ref);
      ...
      type_t *const elem_type =
          skip_typeref(expr->array_access.array_ref->base.type);
...
ir_relation get_relation(const expression_kind_t kind) {
  switch (kind) {
    ...
    case EXPR_BINARY_NOTEQUAL:
      return ir_relation_unordered_less_greater;
...
static ir_tarval *fold_binary_comparison(
  ...
  assert(amode ==
         get_ir_mode_arithmetic(skip_typeref(binexpr->right->base.type)));
...
static ir_tarval *fold_call_builtin(call_expression_t const *const call) {
  ...
  switch (ref->entity->function.btk) {
    case BUILTIN_INF:
      return fold_builtin_inf(function_type);
    case BUILTIN_NAN:
      return fold_builtin_nan(call, function_type);
    default:
...
static ir_tarval *literal_to_tarval_(const literal_expression_t *literal,
  ...
  switch (literal->base.kind) {
    ...
    case EXPR_LITERAL_FLOATINGPOINT:
      return new_tarval_from_str(literal->value->begin, literal->value->size,
                                 mode);
...
static ir_tarval *char_literal_to_tarval(
    string_literal_expression_t const *literal) {
...
void determine_enum_values(enum_t *const enume) {
  ...
  for (entity_t *entry = enume->first_value;
    ...
    if (init != NULL) {
      ...
      if (!is_type_valid(init_type)) continue;
...
ir_tarval *get_enum_value(const enum_value_t *enum_value) {
  if (enum_value->tv == NULL) determine_enum_values(enum_value->enume);
  return enum_value->tv;
}

static ir_tarval *enum_constant_to_tarval(
    reference_expression_t const *const ref) {
...
static ir_tarval *offsetof_to_tarval(
    offsetof_expression_t const *const expression) {
...
static unsigned get_object_alignment(expression_t const *const expr) {
  ...
  switch (expr->kind) {
    case EXPR_ARRAY_ACCESS:
      return get_address_alignment(expr->array_access.array_ref);
    case EXPR_UNARY_DEREFERENCE:
      return get_address_alignment(expr->unary.value);
    case EXPR_REFERENCE:
      ent = expr->reference.entity;
      break;
    case EXPR_SELECT:
      ent = expr->select.compound_entry;
      break;
    default:
      return get_ctype_alignment(expr->base.type);
  }
...
static ir_tarval *builtin_constant_to_tarval(
  ...
  ir_mode *const mode = get_ir_mode_storage(expression->base.type);
  bool const v =
      is_constant_expression(expression->value) != EXPR_CLASS_VARIABLE;
...
static ir_tarval *classify_type_to_tarval(
  ...
  for (;;) {
    ...
    switch (type->kind) {
      ...
      case TYPE_POINTER:
        tc = pointer_type_class;
        goto make_const;
      case TYPE_COMPOUND_STRUCT:
        tc = record_type_class;
        goto make_const;
      case TYPE_COMPOUND_UNION:
        tc = union_type_class;
        goto make_const;
  ...
  switch (expr->kind) {
    case EXPR_CONDITIONAL: {
      ...
      if (cond->true_expression != NULL) {
        ...
        expression_t *res =
            condval ? cond->true_expression : cond->false_expression;
        return fold_expression(res);
      }
      ir_tarval *const val = fold_expression(cond->condition);
      return tarval_is_null(val)     ? fold_expression(cond->false_expression)
    ...
    case EXPR_SIZEOF: {
      ...
      ir_mode *const mode =
          get_ir_mode_arithmetic(skip_typeref(expr->base.type));
      return get_type_size_tarval(type, mode);
    }

    case EXPR_ALIGNOF:
      return alignof_to_tarval(&expr->typeprop);
    case EXPR_BUILTIN_CONSTANT_P:
      return builtin_constant_to_tarval(&expr->builtin_constant);
    case EXPR_BUILTIN_TYPES_COMPATIBLE_P:
      return builtin_types_compatible_to_tarval(
          &expr->builtin_types_compatible);
    case EXPR_CLASSIFY_TYPE:
      return classify_type_to_tarval(&expr->classify_type);
    case EXPR_ENUM_CONSTANT:
      return enum_constant_to_tarval(&expr->reference);
    case EXPR_LITERAL_CASES:
      return literal_to_tarval(&expr->literal);
    case EXPR_LITERAL_CHARACTER:
      return char_literal_to_tarval(&expr->string_literal);
    case EXPR_OFFSETOF:
      return offsetof_to_tarval(&expr->offsetofe);
    case EXPR_UNARY_TAKE_ADDRESS:
      return fold_expression_to_address(expr->unary.value);

    case EXPR_UNARY_NEGATE:
      return tarval_neg(fold_expression(expr->unary.value));
    case EXPR_UNARY_PLUS:
      return fold_expression(expr->unary.value);
    case EXPR_UNARY_COMPLEMENT:
      return tarval_not(fold_expression(expr->unary.value));
    ...
    case EXPR_BINARY_LOGICAL_AND: {
      bool const c = !tarval_is_null(fold_expression(expr->binary.left)) &&
...
static complex_constant fold_complex_add(complex_constant const left,
  ...
  return (complex_constant){tarval_add(left.real, right.real),
                            tarval_add(left.imag, right.imag)};
}

static complex_constant fold_complex_sub(complex_constant const left,
                                         complex_constant const right) {
  return (complex_constant){tarval_sub(left.real, right.real),
...
static complex_constant fold_complex_mul(complex_constant const left,
  ...
  return (complex_constant){tarval_sub(op1, op2), tarval_add(op3, op4)};
}
...
static complex_constant fold_complex_div(complex_constant const left,
  ...
  return (complex_constant){tarval_div(real_dividend, real_divisor),
                            tarval_div(imag_dividend, imag_divisor)};
}

static complex_constant convert_complex_constant(const complex_constant cnst,
                                                 ir_mode *mode) {
  if (get_tarval_mode(cnst.real) == mode) return cnst;
  return (complex_constant){tarval_convert_to(cnst.real, mode),
...
static complex_constant fold_complex_negate(
  ...
  return (complex_constant){tarval_neg(value.real), tarval_neg(value.imag)};
}
...
static complex_constant fold_complex_complement(
  ...
  ir_mode *mode = get_complex_mode_arithmetic(expr->base.type);
  value = convert_complex_constant(value, mode);
  return (complex_constant){value.real, tarval_neg(value.imag)};
}

complex_constant fold_complex_literal(
  ...
  return (complex_constant){zero, litvalue};
}

static complex_constant fold_complex_conditional(
    conditional_expression_t const *const cond) {
  type_t const *const condition_type = skip_typeref(cond->condition->base.type);
  if (!is_type_complex(condition_type)) {
    bool condval = fold_expression_to_bool(cond->condition);
    expression_t *to_fold =
        condval ? cond->true_expression : cond->false_expression;
    return fold_complex(to_fold);
  }
  complex_constant const val = fold_complex(cond->condition);
  return tarval_is_null(val.real) && tarval_is_null(val.imag)
             ? fold_complex(cond->false_expression)
         : cond->true_expression ? fold_complex(cond->true_expression)
                                 : val;
}

static complex_constant fold_complex_cast(
