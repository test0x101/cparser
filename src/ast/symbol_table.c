...
static inline void init_symbol_table_entry(symbol_t *entry,
                                           const char *string) {
  entry->string = string;
...
#define InitData(this, value, key)                                         \
  ((void)((value) =                                                        \
              (ValueType)obstack_alloc(&symbol_obstack, sizeof(symbol_t)), \
          init_symbol_table_entry((value), key)))
#define Hash(this, key) hash_string(key)
#define KeysEqual(this, key1, key2) (streq(key1, key2))
#define SetRangeEmpty(ptr, size) \
  memset(ptr, 0, (size) * sizeof(symbol_table_hash_entry_t))
...
void symbol_table_iterator_init_(symbol_table_iterator_t *iterator,
