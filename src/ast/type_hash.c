...
static bool function_types_equal(const function_type_t *type1,
  ...
  if (type1->return_type != type2->return_type) return false;
  if (type1->variadic != type2->variadic) return false;
  if (type1->unspecified_parameters != type2->unspecified_parameters)
    return false;
  if (type1->kr_style_parameters != type2->kr_style_parameters) return false;
  if (type1->linkage != type2->linkage) return false;
  if (type1->modifiers != type2->modifiers) return false;
  if (type1->calling_convention != type2->calling_convention) return false;
...
static bool array_types_equal(const array_type_t *type1,
  ...
  if (type1->element_type != type2->element_type) return false;
  if (type1->is_variable != type2->is_variable) return false;
  if (type1->is_static != type2->is_static) return false;
  if (type1->size_constant != type2->size_constant) return false;

  /* never identify vla types, because we need them for caching calculated
   * sizes later in ast2firm */
  if (type1->is_vla || type2->is_vla) return false;
...
static bool typeof_types_equal(const typeof_type_t *type1,
  ...
  if (type1->expression != type2->expression) return false;
  if (type1->typeof_type != type2->typeof_type) return false;
...
void init_typehash(void) { _typehash_init(&typehash); }

void exit_typehash(void) { _typehash_destroy(&typehash); }
