...
static void print_char_file(const char c) { fputc(c, out); }

static void print_string_file(const char *str) { fputs(str, out); }
...
static void print_char_obstack(const char c) { obstack_1grow(obst, c); }
...
static void print_char_buffer(const char c) {
  if (buffer_pos == buffer_end) return;
  *buffer_pos++ = c;
}
...
void printer_push(void) { /* TODO */ }

void printer_pop(void) { /* TODO */ }
