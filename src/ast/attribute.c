...
static void handle_attribute_aligned(const attribute_t *attribute,
  ...
  if (!is_po2(alignment)) {
    errorf(&attribute->pos, "alignment must be a power of 2 but is %d",
           alignment);
    return;
  }
  if (alignment <= 0) {
    errorf(&attribute->pos, "alignment must be bigger than 0 but is %d",
           alignment);
    return;
  }
  ...
  switch (entity->kind) {
    ...
    default:
      warningf(WARN_OTHER, &attribute->pos,
               "alignment attribute specification on %N ignored", entity);
      break;
  }
}

static string_t const *get_argument_string(
    attribute_argument_t const *const argument) {
  if (argument == NULL) return NULL;
  if (argument->kind != ATTRIBUTE_ARGUMENT_EXPRESSION) return NULL;
  expression_t *expression = argument->v.expression;
  if (expression->kind != EXPR_STRING_LITERAL) return NULL;
...
static void handle_attribute_visibility(const attribute_t *attribute,
  ...
  if (string == NULL) {
    errorf(&attribute->pos,
           "__attribute__((visibility(X))) requires a string argument");
    return;
  }
  ...
  switch (entity->kind) {
    ...
    default:
      warningf(WARN_OTHER, &attribute->pos,
               "visibility attribute specification on %N ignored", entity);
      break;
  }
}

static void warn_arguments(const attribute_t *attribute) {
  if (attribute->a.arguments == NULL) return;
...
static void handle_attribute_packed_e(const attribute_t *attribute,
  ...
  if (entity->kind == ENTITY_STRUCT) {
    ...
    for (entity_t *member = compound->members.first_entity; member != NULL;
      ...
      if (member->kind != ENTITY_COMPOUND_MEMBER) continue;
    ...
    position_t const *const pos = &attribute->pos;
    warningf(WARN_OTHER, pos, "packed attribute on entity %N ignored", entity);
    return;
  }
}
  ...
  for (; attribute != NULL; attribute = attribute->next) {
    switch (attribute->kind) {
      case ATTRIBUTE_GNU_CONST:
        modifiers |= DM_CONST;
        break;
      case ATTRIBUTE_GNU_DEPRECATED:
        modifiers |= DM_DEPRECATED;
        break;
      case ATTRIBUTE_GNU_NOINLINE:
        modifiers |= DM_NOINLINE;
        break;
      case ATTRIBUTE_GNU_NAKED:
        modifiers |= DM_NAKED;
        break;
      case ATTRIBUTE_GNU_PURE:
        modifiers |= DM_PURE;
        break;
      case ATTRIBUTE_GNU_ALWAYS_INLINE:
        modifiers |= DM_FORCEINLINE;
        break;
      case ATTRIBUTE_GNU_CONSTRUCTOR:
        modifiers |= DM_CONSTRUCTOR;
        break;
      case ATTRIBUTE_GNU_DESTRUCTOR:
        modifiers |= DM_DESTRUCTOR;
        break;
      case ATTRIBUTE_GNU_TRANSPARENT_UNION:
        modifiers |= DM_TRANSPARENT_UNION;
        break;
      case ATTRIBUTE_GNU_USED:
        modifiers |= DM_USED;
        break;
      case ATTRIBUTE_GNU_UNUSED:
        modifiers |= DM_UNUSED;
        break;
      case ATTRIBUTE_GNU_DLLIMPORT:
        modifiers |= DM_DLLIMPORT;
        break;
      case ATTRIBUTE_GNU_DLLEXPORT:
        modifiers |= DM_DLLEXPORT;
        break;
      case ATTRIBUTE_GNU_WEAK:
        modifiers |= DM_WEAK;
        break;
      case ATTRIBUTE_GNU_WEAK_IMPORT:
        modifiers |= DM_WEAK;
        break;
      case ATTRIBUTE_GNU_LEAF:
        modifiers |= DM_LEAF;
        break;
      case ATTRIBUTE_GNU_GNU_INLINE:
        modifiers |= DM_GNU_INLINE;
        break;

      case ATTRIBUTE_MS_DLLIMPORT:
        modifiers |= DM_DLLIMPORT;
        break;
      case ATTRIBUTE_MS_DLLEXPORT:
        modifiers |= DM_DLLEXPORT;
        break;
      case ATTRIBUTE_MS_NAKED:
        modifiers |= DM_NAKED;
        break;
      case ATTRIBUTE_MS_NOINLINE:
        modifiers |= DM_NOINLINE;
        break;
      case ATTRIBUTE_MS_THREAD:
        modifiers |= DM_THREAD;
        break;
      case ATTRIBUTE_MS_DEPRECATED:
        modifiers |= DM_DEPRECATED;
        break;
      case ATTRIBUTE_MS_RESTRICT:
        modifiers |= DM_RESTRICT;
        break;
      case ATTRIBUTE_MS_NOALIAS:
        modifiers |= DM_NOALIAS;
        break;
...
static type_t *change_calling_convention(type_t *type, cc_kind_t cconv) {
  ...
  if (type->function.calling_convention == cconv) return type;
...
static type_t *add_modifiers(type_t *type, decl_modifiers_t modifiers) {
  ...
  if ((type->function.modifiers & modifiers) == modifiers) return type;
...
static bool property_attribute_equal(
    const attribute_property_argument_t *prop1,
...
static bool attribute_argument_equal(const attribute_argument_t *arg1,
  ...
  if (arg1->kind != arg2->kind) return false;
...
static bool attribute_arguments_equal(const attribute_argument_t *args1,
  ...
  for (; args1 != NULL && args2 != NULL;
    ...
    if (!attribute_argument_equal(args1, args2)) return false;
  }
...
bool attributes_equal(const attribute_t *attr1, const attribute_t *attr2) {
  if (attr1->kind != attr2->kind) return false;
  ...
  switch (attr1->kind) {
    ...
    default:
      return attribute_arguments_equal(attr1->a.arguments, attr2->a.arguments);
  }
}

attribute_t const *get_attribute(attribute_t const *const attributes,
                                 attribute_kind_t const kind) {
