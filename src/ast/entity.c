 ...
 */
entity_t *allocate_entity_zero(entity_kind_t const kind,
                               entity_namespace_t const namespc,
...
entity_t *skip_unnamed_bitfields(entity_t *entry) {
  for (; entry != NULL; entry = entry->base.next) {
    ...
    if (!entry->compound_member.bitfield || entry->base.symbol != NULL) break;
  }
  return entry;
}
...
unsigned get_declaration_alignment(const declaration_t *declaration) {
  ...
  if (declaration->modifiers & DM_PACKED) return 1;
...
entity_t *find_compound_entry(compound_t *compound, symbol_t *symbol) {
  for (entity_t *iter = compound->members.first_entity; iter != NULL;
       iter = iter->base.next) {
    if (iter->kind != ENTITY_COMPOUND_MEMBER) continue;
