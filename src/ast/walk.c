...
static void walk_type(type_t *const type, const walk_env_t *const env) {
  if (pset_find_ptr(env->visited_types, type) != NULL) return;
  ...
  switch (type->kind) {
    ...
    case TYPE_TYPEOF: {
      ...
      if (expr) walk_expression(expr, env);
      return;
    }
    case TYPE_TYPEDEF:
...
static void walk_declarations(entity_t *entity, entity_t *const last,
                              const walk_env_t *const env) {
...
static void walk_scope(const scope_t *const scope,
                       const walk_env_t *const env) {
  walk_declarations(scope->first_entity, NULL, env);
}

static void walk_statement(statement_t *const stmt,
                           const walk_env_t *const env) {
  ...
  switch (stmt->kind) {
    ...
    case STATEMENT_FOR:
      ...
      if (stmt->fors.step != NULL) walk_expression(stmt->fors.step, env);
...
void walk_translation_unit(translation_unit_t *unit,
  ...
  walk_env_t walk_env = {
      ...
      expression_func != NULL ? expression_func : null_expression_func, env};
  walk_scope(&unit->scope, &walk_env);
  del_pset(walk_env.visited_types);
}

void walk_statements(statement_t *statement, statement_callback func,
                     void *env) {
  walk_env_t walk_env = {pset_new_ptr_default(), null_declaration_func, func,
                         null_expression_func, env};
