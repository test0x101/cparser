...
void print_type_qualifiers(type_qualifiers_t const qualifiers,
                           QualifierSeparators const q) {
  ...
  if (sep == 0 && q & QUAL_SEP_END) print_char(' ');
}
...
static void print_function_type_pre(const function_type_t *type) {
  switch (type->linkage) {
    case LINKAGE_C:
      if (dialect.cpp) print_string("extern \"C\" ");
      break;
...
restart:
  switch (cc) {
    case CC_CDECL:
      print_string(" __cdecl");
      break;
    case CC_STDCALL:
      print_string(" __stdcall");
      break;
    case CC_FASTCALL:
      print_string(" __fastcall");
      break;
    case CC_THISCALL:
      print_string(" __thiscall");
      break;
    case CC_DEFAULT:
...
static void print_modifiers(decl_modifiers_t const modifiers) {
  ...
  if (!sep_at_first(&sep)) print_string("))");
}
...
void print_compound_definition(const compound_t *compound) {
  ...
  for (entity_t const *entity = compound->members.first_entity; entity != NULL;
       entity = entity->base.next) {
    if (entity->kind != ENTITY_COMPOUND_MEMBER) continue;
 ...
 */
static void print_compound_type(char const *const kind,
                                compound_type_t const *const type) {
  print_type_qualifiers(type->base.qualifiers, QUAL_SEP_END);
  print_string(kind);
...
static void intern_print_type_pre(const type_t *const type) {
  switch (type->kind) {
    ...
    case TYPE_COMPOUND_STRUCT:
      print_compound_type("struct ", &type->compound);
      return;
    case TYPE_COMPOUND_UNION:
      print_compound_type("union ", &type->compound);
      return;
    case TYPE_ENUM:
    ...
    case TYPE_TYPEOF:
      ...
      return;
    case TYPE_VOID:
      print_void_type_pre(type);
      return;
    case TYPE_BUILTIN_TEMPLATE:
      print_template_type_pre(type);
      return;
  }
...
type_t *get_unqualified_type(type_t *type) {
  ...
  if (type->base.qualifiers == TYPE_QUALIFIER_NONE) return type;
...
type_t *get_qualified_type(type_t *orig_type, type_qualifiers_t const qual) {
  ...
  if (is_type_array(type)) {
    ...
    if (qual_element_type == element_type) return orig_type;

    copy = duplicate_type(type);
    copy->array.element_type = qual_element_type;
  } else if (is_type_valid(type)) {
    if ((type->base.qualifiers & qual) == qual) return orig_type;
...
bool is_type_integer(const type_t *type) {
  assert(!is_typeref(type));
  if (!is_type_arithmetic(type)) return false;
...
bool is_type_float(const type_t *type) {
  ...
  if (type->kind != TYPE_ATOMIC) return false;
...
static bool function_types_compatible(const function_type_t *func1,
  ...
  if (!types_compatible(ret1, ret2)) return false;

  if (func1->linkage != func2->linkage) return false;

  cc_kind_t cc1 = func1->calling_convention;
  if (cc1 == CC_DEFAULT) cc1 = default_calling_convention;
  cc_kind_t cc2 = func2->calling_convention;
  if (cc2 == CC_DEFAULT) cc2 = default_calling_convention;

  if (cc1 != cc2) return false;

  if (func1->variadic != func2->variadic) return false;
  ...
  check_promoted_types:
    ...
    for (function_parameter_t *parameter = func1->parameters; parameter != NULL;
         parameter = parameter->next) {
      type_t *const parameter_type = skip_typeref(parameter->type);
      type_t *const promoted = get_default_promoted_type(parameter_type);
      if (promoted != parameter_type) return false;
    }
    return true;
  }
  ...
  for (; parameter1 != NULL && parameter2 != NULL;
    ...
    if (!types_compatible_ignore_qualifiers(parameter1_type, parameter2_type))
      return false;
  }
...
static bool array_types_compatible(const array_type_t *array1,
  ...
  if (!types_compatible(element_type1, element_type2)) return false;

  if (!array1->size_constant || !array2->size_constant) return true;
...
bool types_compatible_ignore_qualifiers(const type_t *type1,
  ...
  if (type1->kind != type2->kind) {
    ...
    if (!is_type_valid(type1) || !is_type_valid(type2)) return true;
    return false;
  }
...
bool types_compatible(const type_t *type1, const type_t *type2) {
  ...
  /* shortcut: the same type is always compatible */
  if (type1 == type2) return true;
  if (type1->base.qualifiers != type2->base.qualifiers) return false;
...
static bool function_types_same(const function_type_t *func0,
  ...
  if (!types_same(func0->return_type, func1->return_type)) return false;
  for (function_parameter_t *param0 = func0->parameters,
    ...
    if (param0 == NULL || param1 == NULL) return false;
    if (!types_same(param0->type, param1->type)) return false;
  }
...
static bool array_types_same(const array_type_t *array0,
  ...
  if (!types_same(array0->element_type, array1->element_type)) return false;
...
bool types_same(type_t *type0, type_t *type1) {
  ...
  if (skipped0 == skipped1) return true;
  if (!type_base_same(&skipped0->base, &skipped1->base)) return false;
  ...
  switch (type->kind) {
    ...
    case TYPE_TYPEDEF: {
      unsigned const alignment =
          get_ctype_alignment(type->typedeft.typedefe->type);
...
unsigned get_type_alignment_compound(type_t const *const type) {
  switch (type->kind) {
    ...
    case TYPE_TYPEDEF: {
      unsigned const alignment =
          get_type_alignment_compound(type->typedeft.typedefe->type);
...
atomic_type_kind_t find_signed_int_atomic_type_kind_for_size(unsigned size) {
  static const atomic_type_kind_t possible_kinds[] = {
      ATOMIC_TYPE_SCHAR, ATOMIC_TYPE_SHORT, ATOMIC_TYPE_INT, ATOMIC_TYPE_LONG,
      ATOMIC_TYPE_LONGLONG};
...
atomic_type_kind_t find_unsigned_int_atomic_type_kind_for_size(unsigned size) {
  static const atomic_type_kind_t possible_kinds[] = {
      ATOMIC_TYPE_UCHAR, ATOMIC_TYPE_USHORT, ATOMIC_TYPE_UINT,
      ATOMIC_TYPE_ULONG, ATOMIC_TYPE_ULONGLONG};
  for (size_t i = 0; i < ARRAY_SIZE(possible_kinds); ++i) {
    if (get_atomic_type_size(possible_kinds[i]) == size) {
      return possible_kinds[i];
    }
...
type_t *promote_integer(type_t *type) {
  ...
  if (get_akind_rank(akind) < get_akind_rank(ATOMIC_TYPE_INT)) type = type_int;
...
void layout_compound(compound_t *const compound) {
  ...
  for (entity_t *entry = compound->members.first_entity; entry;
    ...
    if (!is_type_valid(m_type)) continue;
 ...
 */
static __attribute__((unused)) void dbg_type(const type_t *type) {
