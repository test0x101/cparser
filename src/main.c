...
static bool warn_unused_input(compilation_env_t *env,
                              compilation_unit_t *unit) {
  (void)env;
  warningf(WARN_UNUSED_OPTION, NULL,
...
static bool warn_no_linking(compilation_env_t *env, compilation_unit_t *units) {
  ...
  for (compilation_unit_t *unit = units; unit != NULL; unit = unit->next) {
    ...
    if (unit->original_name != unit->name) continue;
    warningf(WARN_UNUSED_OPTION, NULL,
             "ignoring input '%s' because no linking is performed", unit->name);
  }
  return true;
}
...
static void set_handlers(compile_mode_t mode) {
  ...
  switch (mode) {
    ...
    case MODE_PREPROCESS_ONLY:
      ...
      return;
    case MODE_PRINT_AST:
      set_unit_handler(COMPILATION_UNIT_LEXER_TOKENS_C, parse_ignore_errors,
                       false);
      set_unit_handler(COMPILATION_UNIT_LEXER_TOKENS_CXX, parse_ignore_errors,
                       false);
    ...
    case MODE_COMPILE:
      ...
      return;
    case MODE_JITTEST:
      set_unit_handler(COMPILATION_UNIT_INTERMEDIATE_REPRESENTATION, jittest,
                       true);
      set_unused_after(MODE_COMPILE);
      return;
    case MODE_COMPILE_ASSEMBLE:
      set_unit_handler(COMPILATION_UNIT_PREPROCESSED_ASSEMBLER, assemble_final,
                       true);
      return;
    case MODE_GENERATE_DEPENDENCIES:
      set_unit_handler(COMPILATION_UNIT_LEXER_TOKENS_C, generate_dependencies,
                       true);
      set_unit_handler(COMPILATION_UNIT_LEXER_TOKENS_CXX, generate_dependencies,
                       true);
      set_unit_handler(COMPILATION_UNIT_LEXER_TOKENS_ASSEMBLER,
...
int action_compile(const char *argv0) {
  ...
  if (!process_all_units(&env)) result = EXIT_FAILURE;
  ...
  if (do_timing) timer_term(print_timing ? stderr : NULL);
  end_statistics();
  return result;
}
...
int main(int argc, char **argv) {
  ...
  for (state.i = 1; state.i < argc; ++state.i) {
    if (state.argv[state.i] == NULL) continue;
    if (options_parse_assembler(&state) || options_parse_c_dialect(&state) ||
        options_parse_codegen(&state) || options_parse_diagnostics(&state) ||
        parse_compile_mode_options(&state) || options_parse_driver(&state) ||
        options_parse_help(&state) || options_parse_linker(&state) ||
  ...
  if (!target_setup()) return EXIT_FAILURE;
