...
#define POP_CURRENT_ENTITY()                     \
  (assert(current_entity == new_current_entity), \
   (void)(current_entity = old_current_entity))
...
#define POP_PARENT() \
  (assert(current_parent == new_parent), (void)(current_parent = old_parent))

#define PUSH_SCOPE(scope)               \
  size_t const top = environment_top(); \
  scope_t *const new_scope = (scope);   \
  scope_t *const old_scope = (new_scope ? scope_push(new_scope) : NULL)
#define PUSH_SCOPE_STATEMENT(scope) \
  PUSH_SCOPE(dialect.c99 || dialect.cpp ? (scope) : NULL)
#define POP_SCOPE()                                                      \
  (new_scope ? assert(current_scope == new_scope), scope_pop(old_scope), \
   environment_pop_to(top) : (void)0)
...
#define POP_EXTENSION() ((void)(in_gcc_extension = old_gcc_extension))
...
static size_t get_expression_struct_size(expression_kind_t kind) {
  static const size_t sizes[] = {
      ...
      [EXPR_BUILTIN_TYPES_COMPATIBLE_P] =
          sizeof(builtin_types_compatible_expression_t),
...
static initializer_t *allocate_initializer_list(size_t len,
  ...
  size_t size =
      sizeof(initializer_list_t) + len * sizeof(result->list.initializers[0]);
 ...
 */
static size_t environment_top(void) { return ARR_LEN(environment_stack); }

/**
 * Returns the index of the top element of the global label stack.
 */
static size_t label_top(void) { return ARR_LEN(label_stack); }
...
static inline bool peek(token_kind_t const kind) { return token.kind == kind; }
...
static void eat_until_matching_token(token_kind_t const type) {
  ...
  while (!peek(end_token) || parenthesis_count != 0 || brace_count != 0 ||
         bracket_count != 0) {
    switch (token.kind) {
      case T_EOF:
        return;
      ...
      check_stop:
        if (peek(end_token) && parenthesis_count == 0 && brace_count == 0 &&
            bracket_count == 0)
          return;
        break;
...
#endif
    void
    parse_error_expected(const char *message, ...) {
  va_list ap;
  va_start(ap, message);
  if (message) {
...
static void expect(token_kind_t const expected) {
  if (skip_till(expected, NULL)) eat(expected);
}

static symbol_t *expect_identifier(char const *const context,
                                   position_t *const pos) {
  if (!skip_till(T_IDENTIFIER, context)) return sym_anonymous;
...
static scope_t *scope_push(scope_t *new_scope) {
  if (current_scope != NULL) new_scope->depth = current_scope->depth + 1;
 ...
 */
static void scope_pop(scope_t *old_scope) { current_scope = old_scope; }
...
entity_t *get_entity(const symbol_t *const symbol, entity_namespace_t namespc) {
  for (entity_t *entity = symbol->entity; entity != NULL;
    ...
    if (entity->base.namespc == namespc) return entity;
  }
...
static void reset_symbol(symbol_t *symbol, entity_namespace_t namespc) {
  ...
  for (anchor = &symbol->entity;; anchor = &iter->base.symbol_next) {
    ...
    if (iter == NULL) return;
    /* replace an entry? */
    if (iter->base.namespc == namespc) break;
  }
 ...
 * possible. */
static entity_t *get_tag(symbol_t const *const symbol, entity_kind_t const kind,
                         position_t const *const pos) {
...
static enum_t *determine_enum(expression_t *expression) {
  ...
  if (is_type_enum(type)) return type->enumt.enume;
  return NULL;
}
...
static void warn_enum_conversion(type_t *dest_type, expression_t *expression) {
  ...
  if (expression_enum == NULL) return;
...
static expression_t *create_implicit_cast(expression_t *expression,
  ...
  if (source_type == skipped_dest) return expression;

  if (is_type_enum(skipped_dest)) warn_enum_conversion(dest_type, expression);
...
static void report_assign_error(assign_error_t error, type_t *orig_type_left,
  ...
  switch (error) {
    ...
    case ASSIGN_ERROR_INCOMPATIBLE:
      errorf(pos, "destination type '%T' in %s is incompatible with type '%T'",
    ...
    case ASSIGN_ERROR_POINTER_QUALIFIER_MISSING: {
      ...
      warningf(WARN_OTHER, pos,
               "destination type '%T' in %s from type '%T' lacks qualifiers "
               "'%Q' in pointer target type",
...
static assign_error_t semantic_assign(type_t *orig_type_left,
  ...
  if (is_type_pointer(type_left)) {
    if (is_null_pointer_constant(right)) {
      ...
      unsigned missing_qualifiers =
          points_to_right->base.qualifiers & ~points_to_left->base.qualifiers;
      if (missing_qualifiers != 0) res = ASSIGN_ERROR_POINTER_QUALIFIER_MISSING;

      if (is_type_void(points_to_left)) return res;

      if (is_type_void(points_to_right)) /* ISO/IEC 14882:1998(E) §C.1.2:6 */
      ...
      if (!types_compatible_ignore_qualifiers(points_to_left,
        ...
        if (is_type_integer(points_to_left) &&
            ...
            get_ctype_size(points_to_left) == get_ctype_size(points_to_right))
...
static expression_t *parse_integer_constant_expression(
  ...
  switch (is_constant_expression(result)) {
    ...
    case EXPR_CLASS_CONSTANT:
      if (!is_type_integer(skip_typeref(result->base.type))) {
        errorf(&result->base.pos, "%s '%E' has non-integer type", description,
               result);
        result->base.type = type_error_type;
      } else {
        warningf(WARN_PEDANTIC, &result->base.pos,
                 "%s '%E' is not an integer constant expression", description,
                 result);
      }
      break;
    ...
    do {
      ...
      if (new_enc != enc && new_enc != STRING_ENCODING_CHAR) {
        if (enc == STRING_ENCODING_CHAR) {
          ...
          errorf(HERE, "concatenating string literals with encodings %s and %s",
...
static attribute_t *parse_attribute_gnu_single(void) {
  ...
  for (kind = ATTRIBUTE_GNU_FIRST;; ++kind) {
    if (kind > ATTRIBUTE_GNU_LAST) {
      ...
      warningf(WARN_ATTRIBUTE, HERE, "unknown attribute '%hs' ignored", name);
    ...
    if (attribute_name != NULL && streq_underscore(attribute_name, name)) break;
  }
  ...
  for (;;) {
    ...
    switch (token.kind) {
      ...
      case '.':
        ...
        if (designator->symbol == sym_anonymous) return NULL;
        break;
      default:
        expect('=');
        return result;
...
static initializer_t *initializer_from_expression(type_t *orig_type,
  ...
  if (error == ASSIGN_ERROR_INCOMPATIBLE) return NULL;
  position_t const *const pos = &expression->base.pos;
...
static initializer_t *parse_scalar_initializer(type_t *type,
  ...
  if (peek('{')) {
    do {
      if (braces == (unsigned)may_have_braces && is_initializable_type(type))
  ...
  if (must_be_constant && !is_linker_constant(expression))
    errorf(&expression->base.pos,
           "initialisation expression '%E' is not constant", expression);
...
struct type_path_entry_t {
  type_t *type; /**< the upper top type. restored to path->top_tye if this entry
                   is popped. */
  union {
    size_t index; /**< For array types: the current index. */
    declaration_t
        *compound_entry; /**< For compound types: the current declaration. */
  } v;
};
...
static __attribute__((unused)) void debug_print_type_path(
  ...
  for (size_t i = 0; i < len; ++i) {
    ...
    if (is_type_compound(type)) {
      ...
      fprintf(stderr, ".%s", entry->v.compound_entry->base.symbol->string);
...
static void descend_into_subtype(type_path_t *path) {
  ...
  if (!is_initializable_type(top_type)) return;
...
static bool walk_designator(type_path_t *path, const designator_t *designator,
  ...
  for (; designator != NULL; designator = designator->next) {
    ...
    if (designator->symbol != NULL) {
      ...
      if (!is_type_compound(type)) {
        if (is_type_valid(type))
          errorf(&designator->pos,
                 "'.%Y' designator used for non-compound type '%T'", symbol,
                 orig_type);
        ...
        for (;;) {
          ...
          if (!member) {
            errorf(&designator->pos, "'%T' has no member named '%Y'", orig_type,
                   symbol);
            return false;
          }
          assert(member->kind == ENTITY_COMPOUND_MEMBER);
        ...
        if (used_in_offsetof && member->compound_member.bitfield) {
          errorf(&designator->pos,
                 "offsetof designator '%Y' must not specify bitfield", symbol);
          return false;
        }
      ...
      if (array_index->base.type == type_error_type) return true;

      if (!is_type_array(type)) {
        if (is_type_valid(type))
          errorf(&designator->pos,
                 "[%E] designator used for non-array type '%T'", array_index,
                 orig_type);
      ...
      if (!used_in_offsetof) {
        if (index_long < 0) {
          errorf(&designator->pos, "array index [%E] must be positive",
                 array_index);
        } else if (type->array.size_constant) {
          long array_size = type->array.size;
          if (index_long >= array_size)
            errorf(&designator->pos,
                   "designator [%E] (%ld) exceeds array size %ld", array_index,
                   index_long, array_size);
        }
      }
    ...
    if (designator->next != NULL) descend_into_subtype(path);
  }
  return true;
}
...
static void advance_current_object(type_path_t *path, size_t top_path_level) {
  ...
  if (is_type_union(type)) {
    ...
    if (!type->array.size_constant || top->v.index < type->array.size) return;
  } else {
...
static initializer_t *parse_sub_initializer(type_path_t *path,
  ...
  /* empty initializer? */
  if (peek('}')) return allocate_initializer_list(0, &env->pos);
  ...
  while (true) {
    ...
    if (peek('.') || peek('[')) {
      ...
      if (!GNU_MODE) errorf(HERE, "designator with ':' is a GCC extension");
      designator = allocate_ast_zero(sizeof(designator[0]));
      designator->pos = *HERE;
    ...
    finish_designator:
      ...
      if (!walk_designator(path, designator, false)) goto end_error;
    ...
    if (peek('{')) {
      if (type != NULL && is_type_scalar(type)) {
        ...
        if (type == NULL) {
          if (env->entity != NULL) {
            errorf(HERE, "extra brace group at end of initializer for %N",
                   env->entity);
          } else {
      ...
      if (type == NULL) {
        ...
        if (outer_type == NULL) goto error_parse_next;
      ...
      excess_elements:;
        position_t const *const pos = &expression->base.pos;
        if (env->entity != NULL) {
          warningf(WARN_OTHER, pos, "excess elements in initializer for %N",
                   env->entity);
        } else {
      ...
      while (true) {
        ...
        if (sub != NULL) break;
        if (!is_initializable_type(type)) goto end_error;
        if (is_type_scalar(type)) {
          errorf(&expression->base.pos,
                 "expression '%E' doesn't match expected type '%T'", expression,
                 orig_type);
          goto end_error;
        }

        descend_into_subtype(path);
        if (path->top_type == NULL) goto excess_elements;
      }
    }
...
end_error:
  skip_initializers();
  if (result == NULL) result = allocate_initializer_list(0, &env->pos);
out:
...
static type_t *parse_compound_type_specifier(bool const is_struct) {
  ...
  if (peek(T___attribute__)) attributes = parse_attributes(NULL);
  ...
  if (peek(T_IDENTIFIER)) {
    ...
    char const *const msg =
        is_struct ? "struct type specifier" : "union type specifier";
  ...
  if (entity == NULL) {
    ...
    if (symbol != NULL) environment_push(entity);
...
static void parse_enum_entries(enum_t *const enume) {
  ...
  do {
    ...
    entity_t *const entity =
        allocate_entity_zero(ENTITY_ENUM_VALUE, NAMESPACE_NORMAL, symbol, &pos);
    entity->enum_value.enume = enume;
    if (enume->first_value == NULL) enume->first_value = entity;
    rem_anchor_token('=');
...
static type_t *parse_enum_specifier(void) {
  ...
  if (peek('{')) {
    ...
    if (!has_negative_value) entity->enume.akind = ATOMIC_TYPE_UINT;
...
static entity_t *create_error_entity(symbol_t *const symbol,
  ...
  if (kind != ENTITY_COMPOUND_MEMBER) record_entity(entity, false);
  return entity;
}
...
static attribute_t *parse_attribute_ms_property(attribute_t *attribute) {
  ...
  do {
    ...
    if (streq(prop_sym->string, "put")) {
      ...
      errorf(&pos, "expected put or get in property declspec, but got '%Y'",
             prop_sym);
    }
...
static attribute_t *parse_microsoft_extended_decl_modifier_single(void) {
  ...
  /* parse arguments */
  if (accept('(')) attribute->a.arguments = parse_attribute_arguments();
...
static attribute_t *parse_microsoft_extended_decl_modifier(attribute_t *first) {
  ...
  if (!peek(')')) {
    ...
    do {
      ...
      attribute_t *attribute = parse_microsoft_extended_decl_modifier_single();
...
#define MATCH_STORAGE_CLASS(token, class)                                 \
  case token:                                                             \
    ...
    if (specifiers->thread_local) goto check_thread_storage_class;        \
    eat(token);                                                           \
    break;
        ...
        check_thread_storage_class:
          switch (specifiers->storage_class) {
            ...
            case STORAGE_CLASS_AUTO:
              wrong = "auto";
              goto wrong_thread_storage_class;
  ...
  case token:                                  \
    ...
    break
        ...
        { /* type specifiers */
          ...
          case T__Bool:
            specifier = SPECIFIER_BOOL;
            goto check_specifier;
          case T__Complex:
            specifier = SPECIFIER_COMPLEX;
            goto check_specifier;
          case T__Imaginary:
            specifier = SPECIFIER_IMAGINARY;
            goto check_specifier;
          case T__int16:
            specifier = SPECIFIER_INT16;
            goto check_specifier;
          case T__int32:
            specifier = SPECIFIER_INT32;
            goto check_specifier;
          case T__int64:
            specifier = SPECIFIER_INT64;
            goto check_specifier;
          case T__int8:
            specifier = SPECIFIER_INT8;
            goto check_specifier;
          case T_bool:
            specifier = SPECIFIER_BOOL;
            goto check_specifier;
          case T_char:
            ...
            goto check_specifier;
          case T_double:
            specifier = SPECIFIER_DOUBLE;
            goto check_specifier;
          case T_float:
            specifier = SPECIFIER_FLOAT;
            goto check_specifier;
          case T_int:
            specifier = SPECIFIER_INT;
            goto check_specifier;
          case T_short:
            specifier = SPECIFIER_SHORT;
            goto check_specifier;
          case T_signed:
...
#define CHECK_DOUBLE_TYPE()                                                \
  (type != NULL ? errorf(HERE, "multiple types in declaration specifiers") \
                : (void)0)
      ...
      case T_IDENTIFIER: {
        ...
        if (type != NULL || type_specifiers != 0) {
          ...
          switch (look_ahead(1)->kind) {
            ...
            case '&':
            case '*':
              errorf(HERE, "discarding stray %K in declaration specifier",
                     &token);
              eat(T_IDENTIFIER);
              continue;
        ...
        if (!type) goto finish_specifiers;
...
finish_specifiers:
  ...
  if (type_specifiers == SPECIFIER_NONE) {
    ...
    switch (type_specifiers & ~(SPECIFIER_COMPLEX | SPECIFIER_IMAGINARY)) {
      ...
      case SPECIFIER_LONG | SPECIFIER_FLOAT:
        warningf(WARN_OTHER, pos, "'%hs' is a pre-ANSI spelling for '%hs'",
                 "long float", "double");
        /* FALLTHROUGH */
    ...
    switch (type_specifiers & (SPECIFIER_COMPLEX | SPECIFIER_IMAGINARY)) {
      case SPECIFIER_COMPLEX:
        type = allocate_type_zero(TYPE_COMPLEX);
        break;
      case SPECIFIER_IMAGINARY:
  ...
  if (newtype) type = identify_new_type(type);
...
static void parse_identifier_list(scope_t *scope) {
  ...
  do {
    entity_t *const entity = allocate_entity_zero(
        ENTITY_PARAMETER, NAMESPACE_NORMAL, token.base.symbol, HERE);
    /* a K&R parameter has no type, yet */
    eat(T_IDENTIFIER);

    if (scope != NULL) append_entity(scope, entity);
...
static entity_t *parse_parameter(void) {
  ...
  entity_t *entity =
      parse_declarator(&specifiers, DECL_MAY_BE_ABSTRACT | DECL_IS_PARAMETER);
...
static bool has_parameters(void) {
  ...
  if (!peek_ahead(')')) return true;
  if (peek(T_IDENTIFIER)) {
    entity_t const *const entity =
        get_entity(token.base.symbol, NAMESPACE_NORMAL);
    if (entity == NULL) return true;
    if (entity->kind != ENTITY_TYPEDEF) return true;
    type_t const *const type = skip_typeref(entity->declaration.type);
    if (!is_type_void(type)) return true;
    if (dialect.cpp) {
      /* ISO/IEC 14882:1998(E) §8.3.5:2  It must be literally (void).  A typedef
       * is not allowed. */
      errorf(HERE,
             "empty parameter list defined with a typedef of 'void' not "
             "allowed in C++");
...
static void parse_parameters(function_type_t *type, scope_t *scope) {
  ...
  if (peek(T_IDENTIFIER) && !is_typedef_symbol(token.base.symbol) &&
      (peek_ahead(',') || peek_ahead(')'))) {
    type->kr_style_parameters = true;
    parse_identifier_list(scope);
  } else if (peek(')')) {
    /* ISO/IEC 14882:1998(E) §C.1.6:1 */
    if (!dialect.cpp) type->unspecified_parameters = true;
...
static construct_type_t *allocate_declarator_zero(
    construct_type_kind_t const kind, size_t const size) {
...
static construct_type_t *parse_pointer_declarator(void) {
  construct_type_t *const cons =
      allocate_declarator_zero(CONSTRUCT_POINTER, sizeof(parsed_pointer_t));
  eat('*');
...
static construct_type_t *parse_reference_declarator(void) {
  if (!dialect.cpp) errorf(HERE, "references are only available for C++");

  construct_type_t *const cons =
      allocate_declarator_zero(CONSTRUCT_REFERENCE, sizeof(parsed_reference_t));
  eat('&');
...
static construct_type_t *parse_array_declarator(void) {
  construct_type_t *const cons =
      allocate_declarator_zero(CONSTRUCT_ARRAY, sizeof(parsed_array_t));
  ...
  if (!is_static) is_static = accept(T_static);
  ...
  if (peek('*') && peek_ahead(']')) {
    ...
    if (!is_type_integer(type) && is_type_valid(type))
      errorf(&size->base.pos,
             "array size '%E' must have integer type but has type '%T'", size,
             orig_type);
...
static construct_type_t *parse_function_declarator(scope_t *scope) {
  construct_type_t *const cons = allocate_declarator_zero(
      CONSTRUCT_FUNCTION, sizeof(construct_function_type_t));
...
ptr_operator_end:;
      ...
      switch (la1->kind) {
        case T_IDENTIFIER:
          if (is_typedef_symbol(la1->base.symbol)) {
            ...
            case T__stdcall:
            case T___thiscall:
            case T___attribute__: /* FIXME __attribute__ might also introduce a
                                     parameter of a function declarator. */
  ...
  for (;;) {
    ...
    switch (token.kind) {
      case '(': {
        scope_t *scope = NULL;
        if (!env->must_be_abstract) scope = &env->parameters;
    ...
    type->base.next = *p;
    *p = type;
    if (anchor == p) anchor = &type->base.next;
    env->must_be_abstract = true;
  }
  ...
  for (; iter != NULL; iter = iter->base.next) {
    ...
    switch (iter->kind) {
      case CONSTRUCT_FUNCTION: {
        ...
        if (is_type_function(skipped_return_type)) {
          ...
          warningf(WARN_IGNORED_QUALIFIERS, pos,
                   "type qualifiers in return type of function type are "
                   "meaningless");
        }
      ...
      case CONSTRUCT_ARRAY: {
        ...
        if (size_expression) {
          array_type->array.size_expression =
          ...
          switch (is_constant_expression(size_expression)) {
            case EXPR_CLASS_INTEGER_CONSTANT: {
              ...
              if (tarval_is_constant(size_tv) &&
                  get_tarval_highest_bit(size_tv) < 31) {
                ...
                if (size < 0) {
                  ...
                  errorf(&size_expression->base.pos,
                         "size of array must be greater than zero (zero length "
                         "arrays are a GCC extension)");
                }
              } else {
...
static entity_t *parse_declarator(const declaration_specifiers_t *specifiers,
  ...
  if (construct_type != NULL) obstack_free(&temp_obst, construct_type);
  ...
  *anchor = specifiers->attributes;
  ...
  if (storage_class == STORAGE_CLASS_NONE &&
      (current_function || flags & DECL_IS_PARAMETER))
    storage_class = STORAGE_CLASS_AUTO;
  entity->declaration.storage_class = storage_class;

  if (attributes != NULL) handle_entity_attributes(attributes, entity);
...
static void check_main(const entity_t *entity) {
  ...
  if (!types_compatible(skip_typeref(ret_type), type_int))
    warningf(WARN_MAIN, pos,
             "return type of 'main' should be 'int', but is '%T'", ret_type);
  ...
  if (parm != NULL) {
    ...
    if (parm != NULL) {
      ...
      if (!types_compatible_ignore_qualifiers(second_type, type_char_ptr_ptr))
        warningf(WARN_MAIN, pos,
      ...
      if (parm != NULL) {
        ...
        if (!types_compatible_ignore_qualifiers(third_type, type_char_ptr_ptr))
          warningf(WARN_MAIN, pos,
                   "third argument of 'main' should be 'char**', but is '%T'",
                   parm->type);

        parm = parm->next;
        if (parm != NULL) goto warn_arg_count;
      }
    } else {
    warn_arg_count:
      warningf(WARN_MAIN, pos,
...
static bool contains_attribute(const attribute_t *list,
                               const attribute_t *attr) {
  for (const attribute_t *tattr = list; tattr != NULL; tattr = tattr->next) {
    if (attributes_equal(tattr, attr)) return true;
  }
  return false;
}
...
static bool has_new_attributes(const attribute_t *old_list,
  ...
  for (const attribute_t *attr = new_list; attr != NULL; attr = attr->next) {
    if (!contains_attribute(old_list, attr)) return true;
  }
  return false;
}
...
static void merge_in_attributes(declaration_t *decl, attribute_t *attributes) {
  ...
  for (attribute_t *attr = attributes; attr != NULL; attr = next) {
    next = attr->next;
    if (contains_attribute(decl->attributes, attr)) continue;
...
static bool is_main(const entity_t *entity) {
  if (entity->base.symbol != symbol_main) return false;
  /* must be in outermost scope */
  if (entity->base.parent_scope != file_scope) return false;
...
static bool is_function_definition(const entity_t *entity) {
  ...
  return entity->function.body != NULL || entity->function.alias.symbol != NULL;
}

static void warn_missing_declaration(const entity_t *entity,
                                     bool is_definition) {
  if (entity->kind == ENTITY_FUNCTION) {
    if (is_definition &&
  ...
  } else if (entity->kind == ENTITY_VARIABLE && current_scope == file_scope &&
...
entity_t *record_entity(entity_t *entity, const bool is_definition) {
  ...
  if (kind == ENTITY_FUNCTION) {
    ...
    if (is_main(entity)) check_main(entity);
  }
    ...
    if (previous->base.parent_scope == current_scope) {
      ...
      if (kind == ENTITY_TYPEDEF) {
        ...
        if (dialect.cpp) {
          ...
          if (types_same(type, prev_type)) goto finish;
        } else {
      ...
      if (!types_compatible(type, prev_type)) {
        errorf(pos, "declaration '%#N' is incompatible with '%#N'", entity,
               previous);
        note_prev_decl(previous);
      } else {
        unsigned old_storage_class = prev_decl->storage_class;
        if (is_definition && !prev_decl->used &&
        ...
        warn_redundant_declaration:;
          bool has_new_attrs =
              has_new_attributes(prev_decl->attributes, decl->attributes);
          if (has_new_attrs) {
            merge_in_attributes(decl, prev_decl->attributes);
          } else if (!is_definition && is_type_valid(prev_type)) {
            if (warningf(WARN_REDUNDANT_DECLS, pos,
                         "redundant declaration for %N", entity))
              note_prev_decl(previous);
          }
          ...
          if (old_storage_class != STORAGE_CLASS_STATIC &&
            ...
            if (dialect.cpp) goto error_redeclaration;
      ...
      if (is_definition) previous->base.pos = entity->base.pos;
...
static void parse_init_declarator_rest(entity_t *entity) {
  ...
  if (entity->kind == ENTITY_TYPEDEF) {
    errorf(&entity->base.pos, "%N is initialized (use __typeof__ instead)",
           entity);
  } else {
    assert(is_declaration(entity));
  ...
  if (entity->kind == ENTITY_VARIABLE && entity->variable.alias.symbol != NULL)
    errorf(&entity->base.pos,
           "%N has an initializer but is declared as an alias for '%Y'", entity,
  ...
  if (is_type_function(type)) {
    errorf(&entity->base.pos, "%N is initialized like a variable", entity);
    orig_type = type_error_type;
  } else if (is_type_array(type)) {
    if (type->array.is_vla)
      errorf(&entity->base.pos, "variable sized %N may not be initialized",
             entity);
  } else if (!is_type_complete(type) && is_type_valid(type)) {
    errorf(&entity->base.pos, "%N has incomplete type '%T' but is initialized",
           entity, orig_type);
  }
...
static void check_variable_type_complete(entity_t *ent) {
  if (ent->kind != ENTITY_VARIABLE) return;
  ...
  type_t *const type = skip_typeref(decl->type);
  if (is_type_complete(type)) return;
...
static void parse_declaration_rest(entity_t *ndeclaration,
  ...
  while (true) {
    ...
    if (peek('=')) {
      ...
      if (entity->variable.alias.symbol != NULL && is_common_variable(entity))
        errorf(&entity->base.pos,
               "%N is a definition but declared as an alias for '%Y'", entity,
               entity->variable.alias.symbol);
    }
...
static entity_t *finished_kr_declaration(entity_t *entity, bool is_definition) {
  ...
  if (previous_entity == NULL ||
    ...
    errorf(&entity->base.pos,
           "expected declaration of a function parameter, found '%Y'", symbol);
    return entity;
  }

  if (is_definition) errorf(HERE, "%N is initialised", entity);
...
static void parse_kr_declaration_list(entity_t *entity) {
  if (entity->kind != ENTITY_FUNCTION) return;

  type_t *type = skip_typeref(entity->declaration.type);
  assert(is_type_function(type));
  if (!type->function.kr_style_parameters) return;
...
decl_list_end:
  ...
  for (entity_t *parameter = entity->function.parameters.first_entity;
       parameter != NULL;
       parameter = parameter->base.next,
                proto_parameter =
                    proto_parameter == NULL ? NULL : proto_parameter->next) {
    if (parameter->kind != ENTITY_PARAMETER) continue;
...
static void check_labels(void) {
  ...
  if (!address_taken && cgoto_first)
    errorf(&cgoto_first->base.pos,
           "computed goto in function without address-taken label");
}
...
static void warn_unused_entity(warning_t const why, entity_t *entity,
  ...
  for (; entity != end; entity = entity->base.next) {
    if (!is_declaration(entity)) continue;

    declaration_t *declaration = &entity->declaration;
    if (declaration->implicit) continue;
...
static bool expression_returns(expression_t const *const expr) {
  switch (expr->kind) {
    case EXPR_CALL: {
      ...
      if (type->kind == TYPE_POINTER) {
        type_t const *const points_to = skip_typeref(type->pointer.points_to);
        if (points_to->kind == TYPE_FUNCTION &&
            points_to->function.modifiers & DM_NORETURN)
          return false;
      }

      if (!expression_returns(func)) return false;

      for (call_argument_t const *arg = expr->call.arguments; arg != NULL;
           arg = arg->next) {
        if (!expression_returns(arg->expression)) return false;
      }
    ...
    case EXPR_CONDITIONAL:
      ...
      if (!expression_returns(expr->conditional.condition)) return false;
  ...
  switch (init->kind) {
    ...
    case INITIALIZER_LIST:
      for (initializer_t *const *i = init->list.initializers,
                                *const *end = i + init->list.len;
           i != end; ++i) {
        if (!initializer_returns(*i)) return false;
      }
      return true;
...
static void check_reachable(statement_t *const stmt) {
  if (stmt->base.reachable) return;
  if (stmt->kind != STATEMENT_DO_WHILE) stmt->base.reachable = true;
  ...
  switch (stmt->kind) {
    ...
    case STATEMENT_COMPOUND:
      ...
      if (next == NULL) goto fallthrough;
      break;

    case STATEMENT_RETURN: {
      expression_t const *const val = stmt->returns.value;
      if (val == NULL || expression_returns(val)) noreturn_candidate = false;
      return;
    }
    ...
    case STATEMENT_IF: {
      ...
      if (!expression_returns(cond)) return;

      int const val = determine_truth(cond);

      if (val >= 0) check_reachable(ifs->true_statement);
    ...
    case STATEMENT_SWITCH: {
      ...
      if (!expression_returns(expr)) return;
      ...
      if (is_constant_expression(expr) >= EXPR_CLASS_CONSTANT) {
        ...
        for (case_label_statement_t *i = switchs->first_case; i != NULL;
             i = i->next) {
          if (i->expression == NULL) has_default = true;
    ...
    case STATEMENT_COMPUTED_GOTO: {
      if (!expression_returns(stmt->computed_goto.expression)) return;
    ...
    case STATEMENT_FOR: {
      ...
      if (fors->condition_reachable) return;
      ...
      if (val >= 0) check_reachable(fors->body);
  ...
  while (next == NULL) {
    ...
    switch (next->kind) {
      ...
      continue_do_while:
        ...
        if (val >= 0) check_reachable(dw->body);
...
static void check_unreachable(statement_t *const stmt, void *const env) {
  ...
  switch (stmt->kind) {
    ...
    case STATEMENT_COMPOUND:
      if (stmt->compound.statements != NULL) return;
      goto warn_unreachable;
    ...
    case STATEMENT_DECLARATION: {
      ...
      if (ent != NULL) {
        for (;; ent = ent->base.next) {
          if (ent->kind == ENTITY_VARIABLE && ent->variable.initializer != NULL)
            goto warn_unreachable;
...
static void parse_external_declaration(void) {
  ...
  if (!is_type_function(type)) {
    if (is_type_valid(type))
      errorf(HERE, "declarator '%#N' has a body but is not a function type",
             ndeclaration);
    eat_block();
    return;
  }
  ...
  if (alias != NULL)
    errorf(HERE, "'%#N' has a body but is declared as an alias for '%Y'",
           ndeclaration, alias);

  position_t const *const pos = &ndeclaration->base.pos;
  if (current_scope != file_scope) {
    if (GNU_MODE) {
      warningf(WARN_PEDANTIC, pos, "nested function %N is a GCC extension",
               ndeclaration);
    } else {
  ...
  if (is_typeref(orig_type)) /* §6.9.1:2 */
  ...
  check_labels();
  check_declarations();
  if (is_warn_on(WARN_RETURN_TYPE) || is_warn_on(WARN_UNREACHABLE_CODE) ||
    ...
    if (noreturn_candidate && !(function->base.modifiers & DM_NORETURN))
      warningf(WARN_MISSING_NORETURN, &body->base.pos,
               "function '%#N' is candidate for attribute 'noreturn'", entity);
  }
  ...
  if (is_main(entity)) {
    ...
    if (dialect.enable_main_collect2_hack) prepare_main_collect2(entity);
  }
...
static void check_deprecated(const position_t *pos, const entity_t *entity) {
  if (!is_declaration(entity)) return;
  declaration_t const *const decl = &entity->declaration;
  if (!(decl->modifiers & DM_DEPRECATED)) return;
  ...
  if (is_type_integer(skipped)) {
    ...
    if (size->base.type == type_error_type) {
      ...
      if (size_long < 0) {
        ...
        assert(size_long < (1 << sizeof(entity->compound_member.bit_size) * 8));
        if (is_type_enum(skipped) && is_warn_on(WARN_BITFIELD_SIZE)) {
          ...
          if (!enum_bitfield_big_enough(enume, skipped, (unsigned)size_long))
            warningf(WARN_BITFIELD_SIZE, pos,
                     "bitfield '%Y' is too small for enum '%T'", user_symbol,
                     type);
        }
      }
    }
    ...
    errorf(&entity->base.pos, "bitfield base type '%T' is not an integer type",
           type);
  }
}

static void parse_compound_declarators(
    compound_t *compound, const declaration_specifiers_t *specifiers) {
  add_anchor_token(';');
  add_anchor_token(',');
  do {
    entity_t *const entity = parse_declarator(
        specifiers, DECL_MAY_BE_ABSTRACT | DECL_CREATE_COMPOUND_MEMBER);
    position_t const *const pos = &entity->base.pos;
    if (entity->kind == ENTITY_TYPEDEF) {
...
static void parse_compound_type_entries(compound_t *compound) {
  ...
  for (;;) {
    switch (token.kind) {
      ...
      case ';':
        warningf(WARN_PEDANTIC, HERE, "extra ';' in %N", (entity_t *)compound);
        eat(';');
        break;
...
static type_t *parse_typename(void) {
  declaration_specifiers_t specifiers;
  parse_declaration_specifiers(&specifiers);
  if (specifiers.storage_class != STORAGE_CLASS_NONE || specifiers.thread_local)
...
static void check_number_suffix(expression_t *const expr,
  ...
  for (char const *c = suffix; *c != '\0'; ++c) {
    ...
    if (spec & add) goto error;
    spec |= add;
  }

  if (!(spec & SPECIFIER_FLOAT) && is_float) spec |= SPECIFIER_DOUBLE;

  if (!(spec & (SPECIFIER_FLOAT | SPECIFIER_DOUBLE)) == is_float) goto error;
  ...
  switch (spec & ~SPECIFIER_COMPLEX) {
    ...
    case SPECIFIER_DOUBLE:
      type = type_double;
      break;
    case SPECIFIER_DOUBLE | SPECIFIER_LONG:
...
static expression_t *parse_number_literal(void) {
  ...
  for (;; ++i) {
    ...
    if (digit >= 10 && base != 16) goto done_mantissa;
...
done_mantissa:
  ...
  switch (base) {
    case 2:
      if (is_float) errorf(HERE, "binary floating %K not allowed", &token);
      break;
  ...
  expression_t *const expr = allocate_expression_zero(
      is_float ? EXPR_LITERAL_FLOATINGPOINT : EXPR_LITERAL_INTEGER);
...
static expression_t *parse_character_constant(void) {
  expression_t *const literal =
      allocate_expression_zero(EXPR_LITERAL_CHARACTER);
  ...
  switch (token.literal.string->encoding) {
    ...
    case STRING_ENCODING_CHAR16:
      literal->base.type = type_char16_t;
      goto warn_multi;
    case STRING_ENCODING_CHAR32:
      literal->base.type = type_char32_t;
      goto warn_multi;
    case STRING_ENCODING_WIDE:
      literal->base.type = type_wchar_t;
      goto warn_multi;
    warn_multi:
      if (size > 1)
        warningf(WARN_MULTICHAR, HERE, "multi-character character constant");
...
static entity_t *create_implicit_function(symbol_t *symbol,
  ...
  if (current_scope != NULL) record_entity(entity, false);
...
static entity_t *lookup_entity(const scope_t *scope, symbol_t *symbol,
                               entity_namespace_t namespc) {
  if (scope == NULL) return get_entity(symbol, namespc);
...
static entity_t *parse_qualified_identifier(void) {
  ...
  if (accept(T_COLONCOLON)) lookup_scope = &unit->scope;
  ...
  while (true) {
    ...
    switch (entity->kind) {
      ...
      default:
        errorf(&pos,
               "'%Y' must be a namespace, class, struct or union (but is a %s)",
...
static bool semantic_cast(expression_t *cast) {
   ...
   * than for utility. */
  if (is_type_void(dst_type)) return true;
  ...
  if (is_type_pointer(src_type) && is_type_pointer(dst_type)) {
    ...
    unsigned missing_qualifiers = src->base.qualifiers & ~dst->base.qualifiers;
...
static expression_t *parse_cast(void) {
  ...
  if (peek('{')) return parse_compound_literal(&pos, type);
  ...
  if (!semantic_cast(cast)) cast->base.type = type_error_type;
  return cast;
}
...
static void semantic_statement_expression(statement_expression_t *expr) {
  ...
  if (expr->statement->compound.statements == NULL) {
    ...
    if (expression != NULL) type = expression->base.type;
  }
  expr->base.type = type;
}
...
static designator_t *parse_designator(void) {
  ...
  if (result->symbol == sym_anonymous) return NULL;
...
static expression_t *parse_offsetof(void) {
  ...
  if (is_type_scalar(type_skipped)) {
    errorf(&expression->base.pos, "offsetof called on scalar type '%T'", type);
...
static void parse_intrinsic_call(expression_t **const args, unsigned const n) {
  ...
  if (i != n) {
    errorf(&pos, "wrong number of arguments for '%Y', expected %u, got %u", ctx,
           n, i);
    if (i < n) {
...
static bool is_last_parameter(expression_t *const param) {
  if (param->kind == EXPR_REFERENCE) {
    ...
    if (entity->kind == ENTITY_PARAMETER && !entity->base.next &&
        entity->base.parent_scope == &current_function->parameters)
      return true;
  }

  if (!is_type_valid(skip_typeref(param->base.type))) return true;
...
static expression_t *parse_va_start(void) {
  ...
  if (!current_function) {
    ...
    errorf(&args[1]->base.pos,
           "second argument of 'va_start' must be last parameter of the "
           "current function");
  } else {
...
static expression_t *parse_builtin_constant(void) {
  expression_t *const expression =
      allocate_expression_zero(EXPR_BUILTIN_CONSTANT_P);
  expression_t *args[1];
  parse_intrinsic_call(args, ARRAY_SIZE(args));
...
static label_t *get_label(char const *const context) {
  ...
  if (sym == sym_anonymous) return NULL;
  if (!current_function) {
...
static expression_t *parse_label_address(void) {
  if (!GNU_MODE) errorf(HERE, "taking address of a label is a GCC extension");

  position_t const pos = *HERE;
  eat(T_ANDAND);

  label_t *const label = get_label("label address");
  if (!label) return create_error_expression();
...
static expression_t *parse_primary_expression(void) {
  switch (token.kind) {
    case T_false:
      return parse_boolean_literal(false);
    case T_true:
      return parse_boolean_literal(true);
    case T_NUMBER:
    ...
    case T___func__:
      return parse_function_keyword(FUNCNAME_FUNCTION);
    case T___PRETTY_FUNCTION__:
      return parse_function_keyword(FUNCNAME_PRETTY_FUNCTION);
    case T___FUNCSIG__:
      return parse_function_keyword(FUNCNAME_FUNCSIG);
    case T___FUNCDNAME__:
      return parse_function_keyword(FUNCNAME_FUNCDNAME);
    case T___builtin_isgreater:
      return parse_compare_builtin(EXPR_BINARY_ISGREATER);
    case T___builtin_isgreaterequal:
      return parse_compare_builtin(EXPR_BINARY_ISGREATEREQUAL);
    case T___builtin_isless:
      return parse_compare_builtin(EXPR_BINARY_ISLESS);
    case T___builtin_islessequal:
      return parse_compare_builtin(EXPR_BINARY_ISLESSEQUAL);
    case T___builtin_islessgreater:
      return parse_compare_builtin(EXPR_BINARY_ISLESSGREATER);
    case T___builtin_isunordered:
      return parse_compare_builtin(EXPR_BINARY_ISUNORDERED);
    case T___builtin_offsetof:
    ...
    case T___builtin_types_compatible_p:
      return parse_builtin_types_compatible();
    case T__assume:
      return parse_assume();
    case T_ANDAND:
    ...
    case T___noop:
      return parse_noop_expression();
    case T___imag__:
      return parse_complex_extract_expression(EXPR_UNARY_IMAG);
    case T___real__:
      return parse_complex_extract_expression(EXPR_UNARY_REAL);
...
static expression_t *parse_array_expression(expression_t *left) {
  ...
  check_idx:
    ...
    if (is_type_valid(type_left) && is_type_valid(type_inside))
      errorf(&expr->base.pos, "invalid types '%T[%T]' for array access",
...
static expression_t *parse_typeprop(expression_kind_t const kind) {
  ...
  if (!is_type_complete(type)) {
    if (!is_type_void(type) || !GNU_MODE) wrong_type = "incomplete";
  ...
  if (wrong_type && is_type_valid(type)) {
    char const *const what = kind == EXPR_SIZEOF ? "sizeof" : "alignof";
    errorf(&tp_expression->base.pos,
           "operand of %s expression must not be of %s type '%T'", what,
           wrong_type, orig_type);
  }

  return tp_expression;
}

static expression_t *parse_sizeof(void) { return parse_typeprop(EXPR_SIZEOF); }
...
static expression_t *parse_select_expression(expression_t *addr) {
  ...
  if (symbol == sym_anonymous) return create_error_expression();
  ...
  if (is_type_pointer(type)) {
    if (!select_left_arrow) {
      errorf(&pos,
             "request for member '%Y' in something not a struct or union, but "
             "'%T'",
             symbol, orig_type);
      saw_error = true;
    }
  ...
  if (!compound->complete) {
    errorf(&pos, "request for member '%Y' in incomplete type '%T'", symbol,
           type_left);
    return create_error_expression();
  }
...
static void check_call_argument(type_t *expected_type,
  ...
  if (is_type_union(expected_type_skip) &&
    ...
    if (best_type != NULL) expected_type = best_type;
  }
    ...
    case BUILTIN_FIRM:
      switch (entity->function.b.firm_builtin_kind) {
        ...
        case ir_bk_prefetch:
          ...
          if (locality != NULL &&
              is_constant_expression(locality->expression) ==
                  EXPR_CLASS_VARIABLE)
            errorf(&call->base.pos,
...
static type_t *check_generic_parameter(type_t **const replacement_ref,
  ...
  if (parameter_type->kind == TYPE_BUILTIN_TEMPLATE) {
    ...
    if (replacement == NULL) {
      if (argument_type == NULL) return NULL;
    ...
    if (argument_type != NULL) {
      ...
      if (argument_type->kind != TYPE_POINTER) /* TODO: error message */
        return NULL;
      argument_points_to = skipped->pointer.points_to;
    }
    type_t *old_points_to = parameter_type->pointer.points_to;
    type_t *new_points_to = check_generic_parameter(
        replacement_ref, old_points_to, argument_points_to);
    if (old_points_to != new_points_to)
      return make_pointer_type(new_points_to, parameter_type->base.qualifiers);
  }

  return parameter_type;
}

static function_type_t *handle_builtin_overload(
    call_expression_t *const call, function_type_t *const function_type,
    expression_t *const function) {
  if (!function_type->typegeneric) return function_type;
  ...
  for (; parameter != NULL && argument != NULL;
    ...
    type_t *new_type = check_generic_parameter(
        &template_replacement, parameter_type, argument->expression->base.type);
  ...
  new_function_type->function.return_type = check_generic_parameter(
      &template_replacement, function_type->return_type, NULL);

  if (parameter != NULL) {
    errorf(&call->base.pos, "too few arguments to function '%E'", function);
    return NULL;
...
static void check_library_call_ctype(const call_expression_t *call) {
  if (dialect.freestanding || !is_warn_on(WARN_CHAR_CTYPE)) return;
  expression_t *const function = call->function;
  if (function->kind != EXPR_REFERENCE) return;
  ...
  if (is_type_atomic(type, ATOMIC_TYPE_CHAR) ||
    ...
    warningf(WARN_CHAR_CTYPE, &expression->base.pos,
             "Argument of type %T to %Y gives undefined behaviour on negative "
             "values != EOF",
             orig_type, symbol);
  }
}
...
static void semantic_call(call_expression_t *call) {
  ...
  if (function_type == NULL) {
    if (is_type_valid(type))
      errorf(&call->base.pos,
             "called object '%E' (type '%T') is not a function or function "
             "pointer",
             function, orig_type);
    goto error;
  }
  ...
  if (function_type->typegeneric) {
    ...
    if (function_type == NULL) goto error;
  ...
  for (; argument != NULL; argument = argument->next) {
    ...
    if (!is_type_complete(arg_type) && is_type_valid(arg_type))
      errorf(&argument->expression->base.pos,
             "call argument '%E' has incomplete type", argument->expression);

    type_t *const promoted_arg_type = get_default_promoted_type(orig_arg_type);
    argument->expression =
        create_implicit_cast(argument->expression, promoted_arg_type);
  }
...
static bool same_compound_type(const type_t *type1, const type_t *type2) {
  return is_type_compound(type1) && type1->kind == type2->kind &&
...
static expression_t const *get_reference_address(expression_t const *expr) {
  ...
  for (;;) {
    ...
    if (expr->kind != EXPR_UNARY_DEREFERENCE) break;

    expr = expr->unary.value;
  }

  if (expr->kind != EXPR_REFERENCE) return NULL;
  ...
  if (!regular_take_address && expr->reference.entity->kind != ENTITY_FUNCTION)
    return NULL;
...
static void warn_assignment_in_condition(const expression_t *const expr) {
  if (expr->kind != EXPR_BINARY_ASSIGN) return;
  if (expr->base.parenthesized) return;
  position_t const *const pos = &expr->base.pos;
...
static expression_t *parse_conditional_expression(expression_t *expression) {
  ...
  if (is_type_void(true_type) || is_type_void(false_type)) {
    ...
    if (is_null_pointer_constant(other_expression)) {
      ...
      if (is_type_void(to1) || is_type_void(to2)) {
        ...
        warningf(WARN_OTHER, pos,
                 "pointer types '%T' and '%T' in conditional expression are "
                 "incompatible",
      ...
      warningf(WARN_OTHER, pos,
               "pointer/integer type mismatch in conditional expression ('%T' "
               "and '%T')",
               true_type, false_type);
      result_type = pointer_type;
    } else {
  ...
  types_incompatible:
    if (is_type_valid(true_type) && is_type_valid(false_type))
      type_error_incompatible("while parsing conditional", pos, true_type,
                              false_type);
...
static expression_t *parse_delete(void) {
  ...
  if (!is_type_pointer(type)) {
    if (is_type_valid(type))
      errorf(&value->base.pos, "operand of delete must have pointer type");
...
static expression_t *parse_throw(void) {
  ...
  switch (token.kind) {
    ...
    case T_IDENTIFIER: {
      ...
      type_t *const type = skip_typeref(orig_type);
      if (!is_type_complete(type)) {
        errorf(&value->base.pos, "cannot throw object of incomplete type '%T'",
               orig_type);
...
static bool check_pointer_arithmetic(const position_t *pos,
  ...
  if (!is_type_complete(points_to)) {
    if (!GNU_MODE || !is_type_void(points_to)) {
      errorf(pos, "arithmetic with pointer to incomplete type '%T' not allowed",
             orig_pointer_type);
      return false;
    } else {
      warningf(WARN_POINTER_ARITH, pos,
               "pointer of type '%T' used in arithmetic", orig_pointer_type);
    }
  } else if (is_type_function(points_to)) {
    if (!GNU_MODE) {
      errorf(pos, "arithmetic with pointer to function type '%T' not allowed",
             orig_pointer_type);
      return false;
    } else {
      warningf(WARN_POINTER_ARITH, pos,
  ...
  switch (expression->kind) {
    ...
    default: {
      ...
      return is_type_reference(type)
             /* Claim it is an lvalue, if the type is invalid.  There was a
              * parse error before, which maybe prevented properly recognizing
              * it as lvalue. */
             || !is_type_valid(type);
    }
  }
...
static void semantic_incdec(unary_expression_t *expression) {
  ...
  if (is_type_pointer(type)) {
    if (!check_pointer_arithmetic(&expression->base.pos, type, orig_type))
      return;
  } else if (!is_type_real(type) && (!GNU_MODE || !is_type_complex(type)) &&
             is_type_valid(type)) {
  ...
  if (!is_lvalue(value)) /* TODO: improve error message */
...
static void set_address_taken(expression_t *expression, bool may_be_register) {
  if (expression->kind != EXPR_REFERENCE) return;
...
CREATE_UNARY_EXPRESSION_PARSER('+', EXPR_UNARY_PLUS, semantic_unexpr_plus)
CREATE_UNARY_EXPRESSION_PARSER('!', EXPR_UNARY_NOT, semantic_not)
CREATE_UNARY_EXPRESSION_PARSER('*', EXPR_UNARY_DEREFERENCE,
                               semantic_dereference)
CREATE_UNARY_EXPRESSION_PARSER('&', EXPR_UNARY_TAKE_ADDRESS, semantic_take_addr)
CREATE_UNARY_EXPRESSION_PARSER('~', EXPR_UNARY_COMPLEMENT, semantic_complement)
...
CREATE_UNARY_POSTFIX_EXPRESSION_PARSER(T_PLUSPLUS, EXPR_UNARY_POSTFIX_INCREMENT,
                                       semantic_incdec)
...
static atomic_type_kind_t semantic_arithmetic_(atomic_type_kind_t kind_left,
  ...
  if (kind_left == kind_right) return kind_left;
  ...
  if (u_rank >= s_rank) return u_kind;
...
static void semantic_binexpr_arithmetic(binary_expression_t *expression) {
  ...
  if (is_type_arithmetic(type_left) && is_type_arithmetic(type_right)) {
    ...
    position_t const *const pos = &expression->base.pos;
    errorf(pos,
           "operands of binary expression must have arithmetic types, but are "
           "'%T' and '%T'",
...
static void semantic_binexpr_integer(binary_expression_t *const expression) {
  ...
  if (is_type_integer(type_left) && !is_type_complex(type_left) &&
    ...
    if (is_type_valid(type_left) && is_type_valid(type_right)) {
      position_t const *const pos = &expression->base.pos;
      errorf(pos,
             "operands of binary expression must have integer types, but are "
             "'%T' and '%T'",
             orig_type_left, orig_type_right);
    }
  }
...
static void warn_div_by_zero(binary_expression_t const *const expression) {
  if (!is_type_integer(expression->base.type)) return;
...
static bool semantic_shift(binary_expression_t *expression) {
  ...
  if (!is_type_integer(type_left) || !is_type_integer(type_right)) {
    if (is_type_valid(type_left) && is_type_valid(type_right)) {
      position_t const *const pos = &expression->base.pos;
      errorf(pos,
             "operands of shift expression must have integer types, but are "
             "'%T' and '%T'",
             orig_type_left, orig_type_right);
    }
    return false;
...
static void semantic_shift_op(binary_expression_t *expression) {
  ...
  if (!semantic_shift(expression)) return;
...
static void semantic_add(binary_expression_t *expression) {
  ...
  if (is_type_arithmetic(type_left) && is_type_arithmetic(type_right)) {
    ...
    check_pointer_arithmetic(&expression->base.pos, type_left, orig_type_left);
    expression->base.type = type_left;
    ...
    errorf(&expression->base.pos, "invalid operands to binary + ('%T', '%T')",
           orig_type_left, orig_type_right);
  }
}
...
static void semantic_sub(binary_expression_t *expression) {
  ...
  if (is_type_arithmetic(type_left) && is_type_arithmetic(type_right)) {
    ...
    if (!types_compatible_ignore_qualifiers(left_pt, right_pt)) {
      errorf(pos, "subtracting pointers to incompatible types '%T' and '%T'",
             orig_type_left, orig_type_right);
...
static void warn_string_literal_address(expression_t const *expr) {
  while (expr->kind == EXPR_UNARY_TAKE_ADDRESS) {
    expr = expr->unary.value;
    if (expr->kind != EXPR_UNARY_DEREFERENCE) return;
    expr = expr->unary.value;
  }
...
static void semantic_comparison(binary_expression_t *expression,
  ...
  if (is_type_arithmetic(type_left) && is_type_arithmetic(type_right)) {
    ...
    if (is_type_integer(arithmetic_type) && !is_type_signed(arithmetic_type)) {
    ...
    if (is_relational &&
      ...
      type_error_incompatible("invalid operands for relational operator", pos,
    ...
    if (!is_type_void(skipped_pt_left) && !is_type_void(skipped_pt_right) &&
        !types_compatible_ignore_qualifiers(skipped_pt_left, skipped_pt_right))
    ...
    if (is_relational || !is_null_pointer_constant(right)) goto int_ptr_warning;
  } else if (is_type_pointer(type_right) && is_type_integer(type_left)) {
    ...
    type_error_incompatible("invalid operands in comparison", pos, type_left,
                            type_right);
  }
  expression->base.type = dialect.cpp ? type_bool : type_int;
}
...
static bool has_const_fields(const compound_type_t *type) {
  ...
  for (entity_t const *entry = compound->members.first_entity; entry != NULL;
       entry = entry->base.next) {
    if (!is_declaration(entry) || entry->kind == ENTITY_TYPEDEF) continue;

    const type_t *decl_type = skip_typeref(entry->declaration.type);
    if (decl_type->base.qualifiers & TYPE_QUALIFIER_CONST) return true;
  }
...
static bool is_valid_assignment_lhs(expression_t const *const left) {
  ...
  if (type_left->base.qualifiers & TYPE_QUALIFIER_CONST) {
    errorf(&left->base.pos, "assignment to read-only location '%E' (type '%T')",
           left, orig_type_left);
    return false;
  }
  if (!is_type_complete(type_left)) {
    if (is_type_valid(type_left))
      errorf(&left->base.pos,
             "left-hand side '%E' of assignment has incomplete type '%T'", left,
             orig_type_left);
    return false;
  }
...
static void semantic_arithmetic_assign(binary_expression_t *expression) {
  ...
  if (!is_valid_assignment_lhs(left)) return;
...
static void semantic_arithmetic_addsubb_assign(
  ...
  if (is_type_arithmetic(type_left) && is_type_arithmetic(type_right)) {
    ...
    errorf(&expression->base.pos,
           "incompatible types '%T' and '%T' in assignment", orig_type_left,
           orig_type_right);
  }
}
...
static void warn_logical_and_within_or(const expression_t *const expr) {
  if (expr->kind != EXPR_BINARY_LOGICAL_AND) return;
  if (expr->base.parenthesized) return;
  position_t const *const pos = &expr->base.pos;
  warningf(WARN_PARENTHESES_LOGICAL, pos,
...
static bool is_ambiguous_unary_expression_kind(
    expression_kind_t expression_kind) {
...
static void semantic_binexpr_assign(binary_expression_t *expression) {
  ...
  if (expression->base.kind == EXPR_BINARY_ASSIGN &&
    ...
    warningf(WARN_NOT_COMPOUND_ASSIGN, &expression->base.pos,
             "use of unary operator that may be intended as compound "
             "assignment (%hs%E)",
             "=", expression->right);
...
static bool expression_has_effect(const expression_t *const expr) {
  switch (expr->kind) {
    ...
    case EXPR_CALL: {
      ...
      if (call->function->kind != EXPR_REFERENCE) return true;
...
CREATE_BINEXPR_PARSER('*', EXPR_BINARY_MUL, PREC_CAST,
...
CREATE_BINEXPR_PARSER(T_LESSLESS, EXPR_BINARY_SHIFTLEFT, PREC_ADDITIVE,
                      semantic_shift_op)
CREATE_BINEXPR_PARSER(T_GREATERGREATER, EXPR_BINARY_SHIFTRIGHT, PREC_ADDITIVE,
                      semantic_shift_op)
...
CREATE_BINEXPR_PARSER(T_LESSEQUAL, EXPR_BINARY_LESSEQUAL, PREC_SHIFT,
                      semantic_relational)
CREATE_BINEXPR_PARSER(T_GREATEREQUAL, EXPR_BINARY_GREATEREQUAL, PREC_SHIFT,
                      semantic_relational)
CREATE_BINEXPR_PARSER(T_EXCLAMATIONMARKEQUAL, EXPR_BINARY_NOTEQUAL,
                      PREC_RELATIONAL, semantic_equality)
CREATE_BINEXPR_PARSER(T_EQUALEQUAL, EXPR_BINARY_EQUAL, PREC_RELATIONAL,
                      semantic_equality)
CREATE_BINEXPR_PARSER('&', EXPR_BINARY_BITWISE_AND, PREC_EQUALITY,
                      semantic_binexpr_integer)
CREATE_BINEXPR_PARSER('^', EXPR_BINARY_BITWISE_XOR, PREC_AND,
                      semantic_binexpr_integer)
CREATE_BINEXPR_PARSER('|', EXPR_BINARY_BITWISE_OR, PREC_XOR,
                      semantic_binexpr_integer)
CREATE_BINEXPR_PARSER(T_ANDAND, EXPR_BINARY_LOGICAL_AND, PREC_OR,
                      semantic_logical_op)
CREATE_BINEXPR_PARSER(T_PIPEPIPE, EXPR_BINARY_LOGICAL_OR, PREC_LOGICAL_AND,
                      semantic_logical_op)
CREATE_BINEXPR_PARSER('=', EXPR_BINARY_ASSIGN, PREC_ASSIGNMENT,
                      semantic_binexpr_assign)
CREATE_BINEXPR_PARSER(T_PLUSEQUAL, EXPR_BINARY_ADD_ASSIGN, PREC_ASSIGNMENT,
                      semantic_arithmetic_addsubb_assign)
CREATE_BINEXPR_PARSER(T_MINUSEQUAL, EXPR_BINARY_SUB_ASSIGN, PREC_ASSIGNMENT,
                      semantic_arithmetic_addsubb_assign)
CREATE_BINEXPR_PARSER(T_ASTERISKEQUAL, EXPR_BINARY_MUL_ASSIGN, PREC_ASSIGNMENT,
                      semantic_arithmetic_assign)
CREATE_BINEXPR_PARSER(T_SLASHEQUAL, EXPR_BINARY_DIV_ASSIGN, PREC_ASSIGNMENT,
                      semantic_divmod_assign)
CREATE_BINEXPR_PARSER(T_PERCENTEQUAL, EXPR_BINARY_MOD_ASSIGN, PREC_ASSIGNMENT,
                      semantic_divmod_assign)
CREATE_BINEXPR_PARSER(T_LESSLESSEQUAL, EXPR_BINARY_SHIFTLEFT_ASSIGN,
                      PREC_ASSIGNMENT, semantic_shift_assign)
...
CREATE_BINEXPR_PARSER(T_ANDEQUAL, EXPR_BINARY_BITWISE_AND_ASSIGN,
                      PREC_ASSIGNMENT, semantic_integer_assign)
...
static expression_t *parse_subexpression(precedence_t precedence) {
  ...
  while (true) {
    ...
    if (parser->infix_parser == NULL) break;
    if (parser->infix_precedence < precedence) break;
...
static void init_expression_parsers(void) {
  ...
  register_infix_parser(parse_EXPR_UNARY_POSTFIX_INCREMENT, T_PLUSPLUS,
                        PREC_POSTFIX);
  register_infix_parser(parse_EXPR_UNARY_POSTFIX_DECREMENT, T_MINUSMINUS,
                        PREC_POSTFIX);
  ...
  register_infix_parser(parse_EXPR_BINARY_SHIFTRIGHT, T_GREATERGREATER,
                        PREC_SHIFT);
  ...
  register_infix_parser(parse_EXPR_BINARY_LESSEQUAL, T_LESSEQUAL,
                        PREC_RELATIONAL);
  register_infix_parser(parse_EXPR_BINARY_GREATEREQUAL, T_GREATEREQUAL,
                        PREC_RELATIONAL);
  register_infix_parser(parse_EXPR_BINARY_EQUAL, T_EQUALEQUAL, PREC_EQUALITY);
  register_infix_parser(parse_EXPR_BINARY_NOTEQUAL, T_EXCLAMATIONMARKEQUAL,
                        PREC_EQUALITY);
  ...
  register_infix_parser(parse_EXPR_BINARY_LOGICAL_AND, T_ANDAND,
                        PREC_LOGICAL_AND);
  register_infix_parser(parse_EXPR_BINARY_LOGICAL_OR, T_PIPEPIPE,
                        PREC_LOGICAL_OR);
  ...
  register_infix_parser(parse_EXPR_BINARY_ADD_ASSIGN, T_PLUSEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_SUB_ASSIGN, T_MINUSEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_MUL_ASSIGN, T_ASTERISKEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_DIV_ASSIGN, T_SLASHEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_MOD_ASSIGN, T_PERCENTEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_SHIFTLEFT_ASSIGN, T_LESSLESSEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_SHIFTRIGHT_ASSIGN,
                        T_GREATERGREATEREQUAL, PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_BITWISE_AND_ASSIGN, T_ANDEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_BITWISE_OR_ASSIGN, T_PIPEEQUAL,
                        PREC_ASSIGNMENT);
  register_infix_parser(parse_EXPR_BINARY_BITWISE_XOR_ASSIGN, T_CARETEQUAL,
                        PREC_ASSIGNMENT);
...
static void semantic_asm_argument(asm_argument_t *argument, bool is_out) {
  ...
  if (asm_flags & ASM_CONSTRAINT_FLAG_NO_SUPPORT) {
    errorf(&argument->base.base.pos,
           "some constraints in '%s' are not supported on target", constraints);
    return;
  }
  ...
  if (is_out) {
      ...
      if (kind == TYPE_ATOMIC || kind == TYPE_POINTER) {
        ...
        do {
          ...
          if (value_kind == TYPE_ATOMIC) {
            ...
            value_flags =
                get_atomic_type_flags(value_akind) & ~ATOMIC_TYPE_FLAG_SIGNED;
          ...
          if (value_flags != flags || value_size != size) break;
    ...
    if (!is_lvalue(expression))
      errorf(&expression->base.pos, "asm output argument is not an lvalue");
  } else if (asm_flags & ASM_CONSTRAINT_FLAG_MODIFIER_WRITE) {
    errorf(&argument->base.base.pos,
           "constraints '%s' for input operand indicate write", constraints);
  }

  if (asm_flags & ASM_CONSTRAINT_FLAG_MODIFIER_WRITE) {
    if (asm_flags & (ASM_CONSTRAINT_FLAG_SUPPORTS_REGISTER |
...
static entity_t *parse_asm_arguments(bool const is_out) {
  ...
  if (peek(T_STRING_LITERAL) || peek('[')) {
    ...
    do {
      ...
      entity_t *argument = allocate_entity_zero(
          ENTITY_ASM_ARGUMENT, NAMESPACE_ASM_ARGUMENT, symbol, &pos);
  ...
  if (peek(T_IDENTIFIER)) {
    ...
    do {
      ...
      if (label) {
        ...
        entity_t *const asm_label = allocate_entity_zero(
            ENTITY_ASM_LABEL, NAMESPACE_ASM_ARGUMENT, label->base.symbol, &pos);
...
static unsigned set_asm_operand_entities(unsigned pos,
                                         entity_t *const operands) {
...
static void reset_asm_operand_entities(entity_t *const operands) {
  for (entity_t *i = operands; i; i = i->base.next) {
    ...
    if (symbol) reset_symbol(symbol, NAMESPACE_ASM_ARGUMENT);
  }
}
...
static void normalize_asm_text(asm_statement_t *asm_statement) {
  if (!asm_statement->has_arguments) {
    ...
    for (const char *c = asm_statement->asm_text->begin; *c != '\0'; ++c) {
      if (*c == '%') obstack_1grow(&string_obst, '%');
...
static statement_t *parse_gcc_asm_statement(void) {
  ...
  if (accept(':')) asm_statement->inputs = parse_asm_arguments(false);
  ...
  if (accept(':')) {
    ...
    if (asm_statement->labels) asm_statement->is_volatile = true;
  } else if (asm_goto)
  ...
  if (asm_statement->outputs == NULL) asm_statement->is_volatile = true;
...
static statement_t *parse_label_inner_statement(statement_t const *const label,
  ...
  switch (token.kind) {
    case '}':
      warningf(WARN_LABEL_AT_END_OF_BLOCK, &label->base.pos,
               "%s at end of compound statement", label_kind);
    ...
    default:
      ...
      if (inner_stmt->kind == STATEMENT_DECLARATION && !dialect.cpp)
        warningf(WARN_DECLARATION_AFTER_LABEL, &inner_stmt->base.pos,
                 "declaration after %s", label_kind);
      break;
  }
  return inner_stmt;
}
...
static statement_t *parse_case_statement(void) {
  ...
  rem_anchor_token(T_DOTDOTDOT);
  if (accept(T_DOTDOTDOT)) {
    if (!GNU_MODE) errorf(pos, "case ranges are a GCC extension");
  ...
  if (current_switch != NULL) {
    if (!statement->case_label.is_bad) {
      ...
      for (case_label_statement_t *l = current_switch->first_case; l != NULL;
           l = l->next) {
        if (l->is_bad || l->is_empty_range || l->expression == NULL) continue;
...
static statement_t *parse_label_statement(void) {
  ...
  label_t *const label =
      get_label(NULL /* Cannot fail, token is T_IDENTIFIER. */);
...
static void check_enum_cases(const switch_statement_t *statement) {
  ...
  for (const entity_t *entry = enumt->enume->first_value;
    ...
    for (const case_label_statement_t *l = statement->first_case; l != NULL;
         l = l->next) {
      if (l->expression == NULL) continue;
      if (l->first_case == l->last_case && l->first_case != value) continue;
...
static statement_t *parse_switch(void) {
  ...
  if (is_type_integer(type)) {
    ...
    if (get_akind_rank(get_arithmetic_akind(type)) >=
        get_akind_rank(ATOMIC_TYPE_LONG)) {
      warningf(WARN_TRADITIONAL, &expr->base.pos,
               "'%T' switch expression not converted to '%T' in ISO C", type,
               type_int);
    }
  } else if (is_type_valid(type)) {
...
static statement_t *parse_goto(void) {
  ...
  if (peek_ahead('*')) {
    if (!GNU_MODE) errorf(HERE, "computed goto is a GCC extension");
    ...
    if (!cgoto_first) cgoto_first = statement;
  } else {
    ...
    if (label) {
      ...
      statement->gotos.label =
          &allocate_entity_zero(ENTITY_LABEL, NAMESPACE_LABEL, sym_anonymous,
                                &builtin_position)
               ->label;
    }
  }
...
static statement_t *parse_continue(void) {
  if (current_loop == NULL) errorf(HERE, "continue statement not within loop");
...
static bool is_local_variable(const entity_t *entity) {
  if (entity->kind != ENTITY_VARIABLE) return false;
  ...
  switch (expr->kind) {
    case EXPR_COMPOUND_LITERAL:
      return is_type_array(
          skip_typeref(revert_automatic_type_conversion(expr)));

    case EXPR_UNARY_CAST:
      if (!expr->base.implicit) return false;
...
static void parse_namespace_definition(void) {
  ...
  if (entity == NULL) {
    entity =
        allocate_entity_zero(ENTITY_NAMESPACE, NAMESPACE_NORMAL, symbol, HERE);
    entity->base.parent_scope = current_scope;
  }

  if (peek('=')) /* TODO: parse namespace alias */
...
static statement_t *parse_compound_statement(bool inside_expression_statement) {
  ...
  while (!peek('}') && !peek(T_EOF)) {
    ...
    if (sub_statement->kind == STATEMENT_ERROR) break;
  ...
  if (is_warn_on(WARN_UNUSED_VALUE)) {
    for (statement_t *sub_statement = statement->compound.statements;
      ...
      if (sub_statement->kind != STATEMENT_EXPRESSION) continue;
...
static void mark_vars_read(expression_t *const expr, entity_t *lhs_ent) {
  switch (expr->kind) {
    case EXPR_REFERENCE: {
      ...
      if (lhs_ent != entity && lhs_ent != ENT_ANY) entity->variable.read = true;
      return;
    }
    ...
    case EXPR_VA_COPY:
      if (lhs_ent == ENT_ANY) lhs_ent = NULL;
    ...
    case EXPR_UNARY_CAST:
      ...
      if (is_type_void(skip_typeref(expr->base.type))) lhs_ent = NULL;
      goto unary;

    case EXPR_UNARY_THROW:
      if (expr->unary.value == NULL) return;
      /* FALLTHROUGH */
    case EXPR_UNARY_DEREFERENCE:
    case EXPR_UNARY_DELETE:
    case EXPR_UNARY_DELETE_ARRAY:
    ...
    case EXPR_STATEMENT: {
      ...
      if (last != NULL) mark_vars_read(last, lhs_ent);
      return;
    }
...
static void check_unused_globals(void) {
  ...
  for (entity_t const *entity = file_scope->first_entity; entity != NULL;
       entity = entity->base.next) {
    if (!is_declaration(entity) || entity->kind == ENTITY_TYPEDEF) continue;

    const declaration_t *declaration = &entity->declaration;
    if (declaration->used || declaration->modifiers & DM_UNUSED ||
    ...
    if (entity->kind == ENTITY_FUNCTION) {
      ...
      if (entity->function.is_inline) continue;
...
static void parse_externals(void) {
  ...
  while (!peek(T_EOF) && !peek('}')) {
    ...
    for (int i = 0; i < T_LAST_TOKEN; ++i) {
      ...
      if (count != 0) /* the anchor set and its copy differs */
        panic("Leaked anchor token");
    }
    if (in_gcc_extension) /* an gcc extension scope was not closed */
...
static void resolve_aliases(void) {
  for (size_t i = 0, n_aliases = ARR_LEN(alias_entities); i < n_aliases; ++i) {
    entity_t *entity = alias_entities[i];
    assert(entity->kind == ENTITY_VARIABLE || entity->kind == ENTITY_FUNCTION);
    ...
    if (def == NULL) {
      errorf(&entity->base.pos, "%N aliased to unknown entity '%Y'", entity,
             symbol);
    } else {
...
void start_parsing(void) {
  ...
  create_gnu_builtins();
  if (dialect.ms) create_microsoft_intrinsics();
...
static void complete_incomplete_arrays(void) {
  ...
  for (size_t i = 0; i != n; ++i) {
    ...
    if (is_type_complete(type)) continue;
...
void exit_parser(void) { obstack_free(&temp_obst, NULL); }
