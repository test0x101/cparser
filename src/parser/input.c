...
typedef size_t (*decode_func)(input_t *input, utf32 *buffer,
                              size_t buffer_size);

typedef enum { INPUT_FILE, INPUT_STRING } input_kind_t;
  ...
  for (;;) {
    ...
    while (src != end) {
      ...
      one_more:
        ...
        if (decoded < min_code || decoded > 0x10FFFF ||
    ...
    if (buffer != dst) return dst - buffer;
  }
}
...
static bool strcaseeq(char const *s1, char const *s2) {
  for (; tolower((unsigned char)*s1) == tolower((unsigned char)*s2);
       ++s1, ++s2) {
    if (*s1 == '\0') return true;
  }
  return false;
...
input_decoder_t *input_get_decoder(const char *encoding) {
  for (named_decoder_t const *i = decoders; i->name != NULL; ++i) {
    if (strcaseeq(encoding, i->name)) return i->decoder;
  }
  return NULL;
}
...
FILE *input_get_file(const input_t *const input) {
  if (input->kind != INPUT_FILE) return NULL;
...
void input_free(input_t *input) { free(input); }
