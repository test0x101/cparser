...
static const char *get_length_modifier_name(
...
static format_length_modifier_t parse_length_modifier(
    char const **const c_inout) {
  ...
  char const *const *const e = dialect.ms
                                   ? endof(format_length_modifier_names)
                                   : format_length_modifier_names + FMT_MOD_w;
  for (; i != e; ++i) {
...
static void warn_invalid_length_modifier(const position_t *pos,
  ...
  warningf(WARN_FORMAT, pos,
           "invalid length modifier '%s' for conversion specifier '%%%c'", lmod,
           conversion);
}

static void check_argument_type(format_env_t *const env,
                                type_t *const spec_type,
                                char const *const spec_begin,
                                char const *const spec_end) {
  if (!spec_type) {
  ...
  if (is_type_void(spec_skip)) return;
  ...
  if (is_type_pointer(spec_skip)) {
    if (is_type_pointer(arg_skip)) {
      ...
      if ((arg_to->base.qualifiers & ~spec_to->base.qualifiers) == 0 &&
          (types_compatible_ignore_qualifiers(arg_to, spec_to) ||
           is_type_void(spec_to)))
        return;
    }
    ...
    switch (arg->kind) {
      ...
      case EXPR_SELECT:
        if (arg_skip == type_int) {
          ...
          unprom_type = skip_typeref(
              arg->select.compound_entry->compound_member.base.type);
          goto check_type;
        }
        break;
      ...
      case EXPR_ENUM_CONSTANT:
        if (spec_skip->kind == TYPE_ENUM &&
            spec_skip->enumt.enume == arg->reference.entity->enum_value.enume)
          return;
        break;
  ...
  if (!is_type_valid(arg_skip) || !is_type_valid(spec_skip)) return;
  position_t const *const apos = &arg->base.pos;
  int const slen = spec_end - spec_begin;
  warningf(WARN_FORMAT, apos,
           "conversion '%%%.*s' at position %u specifies type '%T' but the "
           "argument has type '%T'",
           slen, spec_begin, env->num_fmt, spec_type, arg_type);
}

static bool check_digits_or_star(format_env_t *const env, char const **const pc,
                                 char const *const ctx) {
  if (accept(pc, '*')) {
    expression_t const *const arg = get_next_arg(env);
    if (!arg) {
      warningf(WARN_FORMAT, env->pos,
               "missing argument for '*' %s in conversion specification %u",
               ctx, env->num_fmt);
      return false;
    }
    type_t *const arg_type = arg->base.type;
    if (!types_compatible_ignore_qualifiers(skip_typeref(arg_type), type_int))
      warningf(WARN_FORMAT, env->pos,
               "argument for '*' %s in conversion specification %u is not an "
               "'int', but an '%T'",
               ctx, env->num_fmt, arg_type);
  } else {
    while (is_digit(**pc)) {
      ++*pc;
    }
...
static char const *check_printf_format(format_env_t *const env, char const *c) {
  ...
  if (accept(&c, '0')) fmt_flags |= FMT_FLAG_ZERO;
      ...
      switch (fmt) {
        ...
        case ' ':
          if (fmt_flags & FMT_FLAG_PLUS)
            warningf(
                WARN_FORMAT, env->pos,
                "' ' is overridden by prior '+' in conversion specification %u",
                env->num_fmt);
          flag = FMT_FLAG_SPACE;
          break;
        ...
        case '+':
          if (fmt_flags & FMT_FLAG_SPACE)
            warningf(WARN_FORMAT, env->pos,
                     "'+' overrides prior ' ' in conversion specification %u",
                     env->num_fmt);
          flag = FMT_FLAG_PLUS;
          break;

        default:
          goto break_fmt_flags;
      }
      if (fmt_flags & flag)
        warningf(WARN_FORMAT, env->pos,
                 "repeated flag '%c' in conversion specification %u", fmt,
                 env->num_fmt);
      fmt_flags |= flag;
      ++c;
    }
  break_fmt_flags:
    ...
    /* minimum field width */
    if (!check_digits_or_star(env, &c, "field width")) return NULL;
  }
  ...
  if (accept(&c, '.')) {
    if (fmt_flags & FMT_FLAG_ZERO)
      warningf(WARN_FORMAT, env->pos,
               "'0' flag ignored with precision in conversion specification %u",
               env->num_fmt);
    if (!check_digits_or_star(env, &c, "precision")) return NULL;
  }
  ...
  switch (fmt) {
    ...
    case 'i':
      switch (fmt_mod) {
        case FMT_MOD_NONE:
          ...
          break;
        case FMT_MOD_hh:
          expected_type = type_signed_char;
          break;
        case FMT_MOD_h:
          expected_type = type_short;
          break;
        case FMT_MOD_l:
          expected_type = type_long;
          break;
        case FMT_MOD_ll:
          expected_type = type_long_long;
          break;
        case FMT_MOD_j:
          expected_type = type_intmax_t;
          break;
        case FMT_MOD_z:
          expected_type = type_ssize_t;
          break;
        case FMT_MOD_t:
          expected_type = type_ptrdiff_t;
          break;
        case FMT_MOD_I:
          expected_type = type_ptrdiff_t;
          break;
        case FMT_MOD_I32:
          expected_type = type_int32;
          break;
        case FMT_MOD_I64:
          expected_type = type_int64;
          break;
        default:
          goto bad_len_mod;
      }
      allowed_flags =
          FMT_FLAG_MINUS | FMT_FLAG_SPACE | FMT_FLAG_PLUS | FMT_FLAG_ZERO;
      break;
    ...
    eval_fmt_mod_unsigned:
      switch (fmt_mod) {
        case FMT_MOD_NONE:
          expected_type = type_unsigned_int;
          break;
        case FMT_MOD_hh:
          expected_type = type_unsigned_char;
          break;
        case FMT_MOD_h:
          expected_type = type_unsigned_short;
          break;
        case FMT_MOD_l:
          expected_type = type_unsigned_long;
          break;
        case FMT_MOD_ll:
          expected_type = type_unsigned_long_long;
          break;
        case FMT_MOD_j:
          expected_type = type_uintmax_t;
          break;
        case FMT_MOD_z:
          expected_type = type_size_t;
          break;
        case FMT_MOD_t:
          expected_type = type_uptrdiff_t;
          break;
        case FMT_MOD_I:
          expected_type = type_size_t;
          break;
        case FMT_MOD_I32:
          expected_type = type_unsigned_int32;
          break;
        case FMT_MOD_I64:
          expected_type = type_unsigned_int64;
          break;
        default:
          goto bad_len_mod;
      }
    ...
    case 'g':
      switch (fmt_mod) {
        ...
        case FMT_MOD_NONE:
          ...
          break;
        case FMT_MOD_L:
          expected_type = type_long_double;
          break;
        default:
          goto bad_len_mod;
      }
      allowed_flags = FMT_FLAG_MINUS | FMT_FLAG_SPACE | FMT_FLAG_PLUS |
                      FMT_FLAG_HASH | FMT_FLAG_ZERO;
      break;

    case 'C':
      if (fmt_mod != FMT_MOD_NONE) goto bad_len_mod;
    ...
    case 'c':
      ...
      switch (fmt_mod) {
        case FMT_MOD_NONE:
          expected_type = type_int;
          break; /* TODO promoted char */
        case FMT_MOD_l:
          expected_type = type_wint_t;
          break;
        case FMT_MOD_w:
          expected_type = type_wchar_t;
          break;
    ...
    case 's':
      switch (fmt_mod) {
        case FMT_MOD_NONE:
          expected_type = type_const_char_ptr;
          break;
        case FMT_MOD_l:
          expected_type = type_const_wchar_t_ptr;
          break;
    ...
    case 'n':
      switch (fmt_mod) {
        case FMT_MOD_NONE:
          expected_type = type_int_ptr;
          break;
        case FMT_MOD_hh:
          expected_type = type_signed_char_ptr;
          break;
        case FMT_MOD_h:
          expected_type = type_short_ptr;
          break;
        case FMT_MOD_l:
          expected_type = type_long_ptr;
          break;
        case FMT_MOD_ll:
          expected_type = type_long_long_ptr;
          break;
        case FMT_MOD_j:
          expected_type = type_intmax_t_ptr;
          break;
        case FMT_MOD_z:
          expected_type = type_ssize_t_ptr;
          break;
        case FMT_MOD_t:
          expected_type = type_ptrdiff_t_ptr;
          break;
    ...
    default:
      warningf(WARN_FORMAT, env->pos,
               "encountered unknown conversion specifier '%%%c' at position %u",
               fmt, env->num_fmt);
    bad_spec:
  ...
  if (wrong_flags != FMT_FLAG_NONE) {
    ...
    *p = '\0';
    ...
    warningf(WARN_FORMAT, env->pos,
             "invalid format flags \"%s\" in conversion specification %%%c at "
             "position %u",
             wrong, fmt, env->num_fmt);
  }
...
static char const *check_scanf_format(format_env_t *const env, char const *c) {
  ...
  if (is_digit(*c)) {
    ...
    if (width == 0)
      warningf(WARN_FORMAT, env->pos, "field width is zero at format %u",
               env->num_fmt);
  }
  ...
  switch (fmt) {
    ...
    case 'i':
      switch (fmt_mod) {
        ...
        case FMT_MOD_I:
          ...
          break;
        case FMT_MOD_I32:
          expected_type = type_int32_ptr;
          break;
        case FMT_MOD_I64:
          expected_type = type_int64_ptr;
          break;
        default:
          goto bad_len_mod;
      }
    ...
    case 'u':
      switch (fmt_mod) {
        case FMT_MOD_NONE:
          expected_type = type_unsigned_int_ptr;
          break;
        case FMT_MOD_hh:
          expected_type = type_unsigned_char_ptr;
          break;
        case FMT_MOD_h:
          expected_type = type_unsigned_short_ptr;
          break;
        case FMT_MOD_l:
          expected_type = type_unsigned_long_ptr;
          break;
        case FMT_MOD_ll:
          expected_type = type_unsigned_long_long_ptr;
          break;
        case FMT_MOD_j:
          expected_type = type_uintmax_t_ptr;
          break;
        case FMT_MOD_z:
          expected_type = type_size_t_ptr;
          break;
        case FMT_MOD_t:
          expected_type = type_uptrdiff_t_ptr;
          break;
        case FMT_MOD_I:
          expected_type = type_size_t_ptr;
          break;
        case FMT_MOD_I32:
          expected_type = type_unsigned_int32_ptr;
          break;
        case FMT_MOD_I64:
          expected_type = type_unsigned_int64_ptr;
          break;
        default:
          goto bad_len_mod;
      }
    ...
    case 'g':
      switch (fmt_mod) {
        case FMT_MOD_l:
          expected_type = type_double_ptr;
          break;
        case FMT_MOD_NONE:
          expected_type = type_float_ptr;
          break;
        case FMT_MOD_L:
          expected_type = type_long_double_ptr;
          break;
        default:
          goto bad_len_mod;
      }
    ...
    check_c_width:
      ...
      if (!suppress_assignment && env->arg) {
        type_t *const type = skip_typeref(
            revert_automatic_type_conversion(env->arg->expression));
        if (is_type_array(type) && type->array.size_constant &&
            width > type->array.size) {
          warningf(
              WARN_FORMAT, env->pos,
              "target buffer '%T' is too small for %u characters at format %u",
              type, width, env->num_fmt);
        }
      }
      break;
    }
    ...
    case '[': {
      ...
      if (!suppress_assignment && width != 0 && env->arg) {
        ...
        if (is_type_array(type) && type->array.size_constant &&
            width >= type->array.size) {
          warningf(WARN_FORMAT, env->pos,
                   "target buffer '%T' is too small for %u characters and \\0 "
                   "at format %u",
                   type, width, env->num_fmt);
        }
      }
    ...
    case 'n': {
      if (suppress_assignment)
        warningf(WARN_FORMAT, env->pos,
                 "conversion '%n' cannot be suppressed with '*' at format %u",
                 env->num_fmt);
    ...
    default:
      warningf(WARN_FORMAT, env->pos,
               "encountered unknown conversion specifier '%%%c' at format %u",
               fmt, env->num_fmt);
    bad_spec:
      expected_type = type_error_type;
      break;
...
static char const *check_custom_format(format_env_t *const env, char const *c) {
  attribute_format_argument_t const *const fmt =
      (attribute_format_argument_t const *)env->ctx;
  ...
  if (*c == '\0') {
    ...
    for (attribute_format_specifier_t const *s = fmt->specifiers; s;
         s = s->next) {
...
static int do_check_format(string_literal_expression_t const *const fmt_string,
                           call_argument_t const *const arg,
                           check_func_t *const check_func,
...
static int internal_check_format_recursive(expression_t const *const fmt_expr,
                                           call_argument_t const *const arg,
                                           check_func_t *const check_func,
                                           void const *const ctx) {
  switch (fmt_expr->kind) {
    case EXPR_CONDITIONAL: {
      ...
      int const nf = internal_check_format_recursive(c->false_expression, arg,
                                                     check_func, ctx);
      return MAX(nt, nf);
    }
    ...
    case EXPR_UNARY_CAST:
      return internal_check_format_recursive(fmt_expr->unary.value, arg,
                                             check_func, ctx);
...
static void internal_check_format(unsigned const fmt_idx,
                                  unsigned const arg_idx,
                                  call_argument_t const *arg,
                                  check_func_t *const check_func,
                                  void const *const ctx) {
  unsigned idx = 0;
  ...
  if (num_args > (unsigned)num_fmt) {
    ...
    warningf(WARN_FORMAT, pos,
             "%u argument%s given but only %u argument%s used", num_args,
             num_args != 1 ? "s" : "", num_fmt, num_fmt != 1 ? "s" : "");
  }
}
...
void check_format(const call_expression_t *const call) {
  if (!is_warn_on(WARN_FORMAT)) return;

  const expression_t *const func_expr = call->function;
  if (func_expr->kind != EXPR_REFERENCE) return;
  ...
  for (format_spec_t const *i = builtin_table; i != endof(builtin_table); ++i) {
    if (streq(name, i->name)) {
      switch (i->fmt_kind) {
        case FORMAT_PRINTF:
          internal_check_format(i->fmt_idx, i->arg_idx, arg,
                                &check_printf_format, NULL);
          break;
        case FORMAT_SCANF:
          internal_check_format(i->fmt_idx, i->arg_idx, arg,
                                &check_scanf_format, NULL);
          break;
  ...
  attribute_t const *const fmt_attr = get_attribute(
      entity->declaration.attributes, ATTRIBUTE_CPARSER_CUSTOM_FORMAT);
  if (fmt_attr) {
    ...
    if (is_constant_expression(fmt->fmt_idx) == EXPR_CLASS_INTEGER_CONSTANT) {
      ...
      internal_check_format(idx, idx + 1 /* TODO make more flexible */, arg,
                            &check_custom_format, fmt);
    }
  }
}
