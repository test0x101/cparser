...
static symbol_t *intern_register_token(token_kind_t id, const char *string) {
  ...
  if (token_symbols[id] == NULL) token_symbols[id] = symbol;
  return symbol;
}
...
static void register_token(lang_features_t mode, token_kind_t id,
  ...
  if (dialect.features & mode) {
    ...
    if (is_keyword) symbol->ID = id;
  }
}

static void register_pp_token(pp_token_kind_t const id,
...
void init_tokens(void) {
  ...
  if (tokens_initialized) return;
  tokens_initialized = true;

  memset(token_symbols, 0, T_LAST_TOKEN * sizeof(token_symbols[0]));

#define T(mode, x, str, val, is_keyword) \
  register_token(mode, x, str, is_keyword);
#include "tokens.h"
#undef T
...
void print_token(FILE *const f, token_t const *const token, bool const simple) {
  ...
  switch (token->kind) {
    ...
    default: {
      val =
          (token->base.symbol ? token->base.symbol : token_symbols[token->kind])
              ->string;
      if (!simple) {
        fprintf(f, "'%s'", val);
        return;
  ...
  if (simple) {
    ...
    fprintf(f, "%s %s%s%s%s", token_symbols[token->kind]->string, enc, delim,
            val, delim);
  }
}
...
bool tokens_would_paste(token_kind_t token1, token_kind_t token2) {
  ...
  switch (token1) {
    ...
    case T_IDENTIFIER:
      return token2 == T_CHARACTER_CONSTANT || token2 == T_IDENTIFIER ||
             token2 == T_NUMBER || token2 == T_STRING_LITERAL; /* L */

    case T_NUMBER:
      return token2 == T_IDENTIFIER || token2 == T_NUMBER || token2 == '.' ||
             token2 == '+' || token2 == '-';
