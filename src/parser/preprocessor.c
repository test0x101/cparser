...
void switch_pp_input(FILE *const stream, char const *const input_name,
                     searchpath_entry_t *const path,
                     bool const is_system_header) {
...
static bool try_switch_input(char const *const name,
                             searchpath_entry_t *const path,
                             bool const is_system_header) {
...
static void push_input(void) {
  pp_input_t *const saved_input =
      obstack_copy(&input_obstack, &input, sizeof(input));
...
static bool is_universal_char_valid(utf32 const v) {
  /* C11 §6.4.3:2 */
  if (v < 0xA0U && v != 0x24 && v != 0x40 && v != 0x60) return false;
...
static utf32 parse_universal_char(unsigned const n_digits) {
  ...
  for (unsigned k = n_digits; k != 0; --k) {
    ...
    if (digit_value >= 0) {
      v = 16 * v + digit_value;
      if (!resolve_escape_sequences) obstack_1grow(&symbol_obstack, input.c);
      next_char();
    } else {
      errorf(&input.pos,
             "short universal character name, expected %u more digits", k);
      break;
    }
  }
  if (!is_universal_char_valid(v)) {
    errorf(&input.pos, "\\%c%0*X is not a valid universal character name",
...
static bool is_universal_char_valid_identifier_c99(utf32 const v) {
  ...
  for (size_t i = 0; i < sizeof(ranges) / sizeof(ranges[0]); ++i) {
    if (ranges[i][0] <= v && v <= ranges[i][1]) return true;
  }
  for (size_t i = 0; i < sizeof(single_chars) / sizeof(single_chars[0]); ++i) {
    if (v == single_chars[i]) return true;
  }
  return false;
}
...
static utf32 parse_escape_sequence(void) {
  ...
  switch (ec) {
    ...
    case 'U':
      return parse_universal_char(8);
    case 'u':
      return parse_universal_char(4);
...
static utf32 get_string_encoding_limit(string_encoding_t const enc) {
  switch (enc) {
    ...
    case STRING_ENCODING_WIDE:
      return 0xFFFFFFFF;  // FIXME depends on settings
  }
...
static void parse_string(utf32 const delimiter, token_kind_t const kind,
  ...
  while (true) {
    switch (input.c) {
      ...
      case NEWLINE:
        warningf(WARN_OTHER, &pp_token.base.pos, "newline while parsing %s",
                 context);
        goto end_of_string;

      case UTF32_EOF:
        warningf(WARN_OTHER, &pp_token.base.pos, "EOF while parsing %s",
                 context);
        goto end_of_string;
...
static void push_function_expansion(pp_definition_t *function) {
  ...
  for (size_t i = 0; i < n_parameters; ++i) {
    ...
    pp_argument_t arg = {parameter->list_len, parameter->token_list,
...
static void push_macro_call(void) {
  if (current_call.macro == NULL) return;
...
static void finish_argument(void) {
  ...
  parameter->token_list =
      obstack_copy(&pp_obstack, tokens, len * sizeof(tokens[0]));
...
static void start_function_macro_expansion(const macro_call_t *call) {
  ...
  if (current_call.macro->is_pragma) {
    ...
    warningf(WARN_UNKNOWN_PRAGMAS, &pp_token.base.pos,
             "_Pragma is not currently supported");
  }
...
static void grow_escaped(struct obstack *obst, const char *string,
                         size_t size) {
  if (resolve_escape_sequences) {
...
static token_t stringify(const pp_definition_t *definition, bool space_before) {
  ...
  for (size_t p = 0; p < list_len; ++p) {
    ...
    if (p > 0 && saved->base.space_before) obstack_1grow(&string_obst, ' ');
...
static string_encoding_t identify_encoding_prefix(symbol_t *const sym) {
  ...
  switch (sym->pp_ID) {
    case TP_L:
      return STRING_ENCODING_WIDE;
    case TP_U:
      enc = STRING_ENCODING_CHAR32;
      break;
    case TP_u:
      enc = STRING_ENCODING_CHAR16;
      break;
    case TP_u8:
      enc = STRING_ENCODING_UTF8;
      break;
    default:
      break;
  }
...
static bool is_identifierlike_token(token_t const *const t) {
  ...
  if (t->kind == T_IDENTIFIER) return true;
...
static bool concat_identifier(const token_t *token0, const token_t *token1) {
  ...
  switch (token1->kind) {
    ...
    case T_CHARACTER_CONSTANT: {
      string_encoding_t const enc =
          identify_encoding_prefix(token0->base.symbol);
      if (enc != STRING_ENCODING_CHAR &&
  ...
  if (symbol->string != string) obstack_free(&symbol_obstack, string);
...
static bool concat_tokens(const position_t *pos, const token_t *token0,
...
static inline void set_digraph(token_kind_t const kind,
...
static bool concat_tokens(const position_t *pos, const token_t *token0,
  ...
  if (token1->kind == T_MACRO_PARAMETER &&
      (token0->kind != ',' || !token1->macro_parameter.def->is_variadic)) {
  ...
  switch (token0->kind) {
    ...
    case '&':
      if (kind1 == '&') {
        set_punctuator(T_ANDAND);
        return true;
      }
      if (kind1 == '=') {
        set_punctuator(T_ANDEQUAL);
        return true;
      }
      break;
    case '*':
      if (kind1 == '=') {
        set_punctuator(T_ASTERISKEQUAL);
        return true;
      }
      break;
    case '+':
      if (kind1 == '+') {
        set_punctuator(T_PLUSPLUS);
        return true;
      }
      if (kind1 == '=') {
        set_punctuator(T_PLUSEQUAL);
        return true;
      }
      break;
    case '-':
      if (kind1 == '-') {
        set_punctuator(T_MINUSMINUS);
        return true;
      }
      if (kind1 == '=') {
        set_punctuator(T_MINUSEQUAL);
        return true;
      }
      if (kind1 == '>') {
        set_punctuator(T_MINUSGREATER);
        return true;
      }
      break;
    case '/':
      if (kind1 == '=') {
        set_punctuator(T_SLASHEQUAL);
        return true;
      }
      break;
    case ':':
    ...
    case '<':
      switch (kind1) {
        case '%':
          set_digraph('{', symbol_lesspercent);
          return true;
        case ':':
          set_digraph('[', symbol_lesscolon);
          return true;
        case '<':
          set_punctuator(T_LESSLESS);
          return true;
        case T_LESSEQUAL:
          set_punctuator(T_LESSLESSEQUAL);
          return true;
        case '=':
          set_punctuator(T_LESSEQUAL);
          return true;
        default:
          break;
      }
    ...
    case '>':
      switch (kind1) {
        case '=': {
          set_punctuator(T_GREATEREQUAL);
          return true;
        }
        case '>': {
          set_punctuator(T_GREATERGREATER);
          return true;
        }
        case T_GREATEREQUAL: {
    ...
    case '^':
      if (kind1 == '=') {
        set_punctuator(T_CARETEQUAL);
        return true;
      }
      break;
    case '|':
      if (kind1 == '=') {
        set_punctuator(T_PIPEEQUAL);
        return true;
      }
      if (kind1 == '|') {
        set_punctuator(T_PIPEPIPE);
        return true;
      }
      break;
    case '.':
    ...
    case T_NUMBER:
      if (concat_number(token0, token1)) return true;
      break;

    default:
      if (is_identifierlike_token(token0)) {
        case T_IDENTIFIER:
          if (concat_identifier(token0, token1)) return true;
      }
      break;
  }
...
more_concat:
  if (pos < current_expansion->list_len) {
    ...
    if (next_kind == T_HASHHASH && pos + 1 < current_expansion->list_len &&
      ...
      if (pp_token.kind == T_MACRO_PARAMETER) {
        ...
        if (list_len > 1) {
          if (!current_expansion->expand_parameter_before_concat) {
            ...
            pos -= 1;
            current_expansion->expand_parameter_before_concat = true;
            goto finished;
            ...
            current_expansion->expand_parameter_before_concat = false;
          }
        }
      }
      ...
      size_t advance = 2;
      token_t tmp;
      if (next_but_one->kind == '#' && pos + 2 < current_expansion->list_len) {
        const token_t *next_next_but_one =
            &current_expansion->token_list[pos + 2];
        if (next_next_but_one->kind == T_MACRO_PARAMETER) {
          pp_definition_t *def = next_next_but_one->macro_parameter.def;
...
static token_kind_t peek_expansion(bool may_pop) {
  for (size_t i = ARR_LEN(expansion_stack); i-- > 0;) {
    ...
    if (e->pos < e->list_len) return e->token_list[e->pos].kind;
    if (!may_pop) return T_EOF;
  }
  return T_EOF;
}
  ...
  while (true) {
    switch (input.c) {
      ...
      case '/':
        ...
        /* FALLTHROUGH */
      default:
        if (stop_at_non_whitespace) return false;
        res = true;
        next_char();
        continue;
        ...
        switch (input.c) {
          {
            ...
            universal:
              ...
              if (!is_universal_char_valid_identifier(v)) {
                if (is_universal_char_valid(v))
                  errorf(&pos,
                         "universal character '\\%c%0*X' is not valid in an "
                         "identifier",
                         n == 4 ? 'u' : 'U', (int)n, v);
              } else if (obstack_object_size(&symbol_obstack) == 0 &&
                         is_universal_char_invalid_identifier_start(v)) {
                errorf(&pos,
                       "universal character '\\%c%0*X' is not valid as start "
                       "of an identifier",
                       n == 4 ? 'u' : 'U', (int)n, v);
...
restart:
  ...
  switch (input.c) {
    ...
    case '.':
      MAYBE_PROLOG
      MAYBE('.', T_DOTDOTDOT)
      ELSE_CODE(put_back('.'); set_punctuator('.'); return;)
      ELSE('.')
    case '&':
      MAYBE_PROLOG
      MAYBE('&', T_ANDAND)
    ...
    case '%':
      ...
      ELSE_CODE(put_back('%'); goto digraph_percentcolon;)
      ELSE_CODE(digraph_percentcolon
                : set_digraph('#', symbol_percentcolon);
                return;)
      ELSE('%')
    case '<':
...
static bool emit_newlines(void) {
  ...
  if (!info.at_line_begin) return false;
...
void preprocessor_print_dependencies(FILE *output, bool show_system_headers,
  ...
  for (const include_t *i = includes; i != NULL; i = i->next) {
    if (i->is_system_header && !show_system_headers) continue;
    fputc(' ', output);
...
void emit_pp_token(void) {
  ...
  if (had_newlines) {
    ...
    if (pp_token.kind == '#' && whitespace == 0) ++whitespace;
    ...
    if (whitespace == 0 && pp_token.base.space_before) ++whitespace;
    for (unsigned i = whitespace; i-- > 0;) fputc(' ', out);
...
static bool pp_tokens_equal(const token_t *token1, const token_t *token2) {
  if (token1->kind != token2->kind) return false;
...
static bool pp_definitions_equal(const pp_definition_t *const definition1,
  ...
  if (n_parameters != definition2->n_parameters) return false;
  ...
  if (len != definition2->list_len) return false;
  for (size_t t = len; t-- > 0;) {
    const token_t *const t1 = &definition1->token_list[t];
    const token_t *const t2 = &definition2->token_list[t];
    if (!pp_tokens_equal(t1, t2)) return false;
    if (t > 0 && t1->base.space_before != t2->base.space_before) return false;
  }
...
static pp_definition_t *add_define_(char const *const name,
  ...
  if (sym->string != string) obstack_free(&symbol_obstack, string);
...
void add_define_string(char const *const name, char const *const val,
  ...
  token_t const stringtok = {.literal = {
...
static void add_define_dynamic_string(char const *const name,
                                      update_func update) {
...
static void add_define_dynamic_number(char const *const name,
                                      update_func update) {
  pp_definition_t *const def = add_define_(name, true);
...
static bool is_defineable_token(char const *const context,
  ...
  if (!is_identifierlike_token(&pp_token)) {
    errorf(&pp_token.base.pos, "expected identifier after %s, got %K", context,
           &pp_token);
    return false;
  }
...
static void parse_define_directive(void) {
  ...
  if (!is_defineable_token("#define", false)) goto error_out;
      ...
      switch (pp_token.kind) {
        ...
        case T_IDENTIFIER: {
          ...
          if (pp_token.kind == ',') {
            if (is_variadic) {
              errorf(&input.pos,
                     "'...' parameter must be last in macro argument list");
              goto error_out;
            }
            eat_token(',');
            break;
          ...
          if (pp_token.kind != ')') {
            errorf(&pp_token.base.pos,
                   "expected ',' or ')' after identifier, got %K", &pp_token);
            goto error_out;
          }
          break;
        }
  ...
  finish_argument_list:
    ...
    size_t size = obstack_object_size(&pp_obstack);
    new_definition->n_parameters = size / sizeof(new_definition->parameters[0]);
    ...
    for (size_t i = 0; i < new_definition->n_parameters; ++i) {
      ...
      if (previous != NULL && previous->function_definition == new_definition) {
  ...
  while (pp_token.kind != T_NEWLINE) {
    ...
    if (first) pp_token.base.space_before = false;
    obstack_grow(&pp_obstack, &pp_token, sizeof(pp_token));
    next_must_be_param = new_definition->has_parameters && pp_token.kind == '#';
    next_input_token();
    first = false;
  }
  if (next_must_be_param) error_missing_macro_param();
  ...
  if (old_definition != NULL) {
    if (old_definition->standard_define) {
      ...
      if (warningf(WARN_OTHER, &new_definition->pos,
                   "multiple definitions of macro '%Y'", macro_symbol))
        notef(&old_definition->pos, "macro '%Y' first defined here",
              macro_symbol);
    } else {
        ...
        while (true) {
          switch (input.c) {
            ...
            case NEWLINE: {
              ...
              errorf(&pp_token.base.pos, "header name without closing '%c'",
                     (char)delimiter);
              return NULL;
            }
...
static bool do_include(bool const bracket_include, bool const include_next,
  ...
  for (; entry != NULL; entry = entry->next) {
    ...
    if (path[len - 1] != '/') obstack_1grow(&symbol_obstack, '/');
    obstack_grow(&symbol_obstack, headername, headername_size);
...
static void parse_include_directive(bool const include_next) {
  ...
  if (res) {
    ...
    errorf(&pp_token.base.pos, "failed including %c%s%c: %s", ldelim,
           headername, rdelim, strerror(errno));
    pop_restore_input();
    goto exit_skip;
  }
}
...
static ir_tarval *parse_pp_operand(void) {
  ...
  switch (kind) {
    ...
    case T_NUMBER: {
      ...
      char const *i = str;
      while (hex_digit_value(*i) >= 0 || *i == 'X' || *i == 'x') ++i;
      ...
      for (;;) {
        if (*i == 'L' || *i == 'l') {
          if (is_long) goto error;
          is_long = true;
          if (*i == i[1]) ++i;
        } else if (*i == 'U' || *i == 'u') {
          if (is_unsigned) goto error;
          is_unsigned = true;
        } else {
          break;
        }
      ...
      error:
        errorf(
            &pp_token.base.pos,
            "invalid suffix '%s' on integer constant in preprocessor condition",
            suffix);
        res = tarval_bad;
      } else {
        ...
        if (res == tarval_bad) {
          errorf(&pp_token.base.pos, "invalid %K in preprocessor condition",
                 &pp_token);
        }
      }
      next_condition_token();
    ...
    case '!': {
      ...
      switch (kind) {
        ...
        case '~':
          return tarval_not(v);
        case '!':
          return tarval_is_null(v) ? pp_one : pp_null;
        default:
    ...
    default:
      if (!is_identifierlike_token(&pp_token)) {
        errorf(&pp_token.base.pos, "unexpected %K in preprocessor condition",
               &pp_token);
        return tarval_bad;
        ...
        if (!expand_next()) {
          ...
          if (pp_token.kind == '\n') {
            errorf(&pp_token.base.pos,
                   "unexpected end of preprocessor condition, expected '(' or "
                   "identifier");
            return tarval_bad;
          }
          if (pp_token.kind == '(') {
          ...
          has_paren:
            ...
            if (pp_token.kind == '\n') {
              errorf(&pp_token.base.pos,
                     "unexpected end of preprocessor condition, expected "
                     "identifier");
              return tarval_bad;
            }
          }
        ...
        restart:
          do {
            if (!expand_next()) goto read_input;
      ...
      next:;
        ...
        if (is_identifierlike_token(&pp_token)) {
          ...
          errorf(&pp_token.base.pos,
                 "unexpected %K in preprocessor condition, expected identifier",
                 &pp_token);
          res = tarval_bad;
        }
        ...
        if (has_paren) {
          if (pp_token.kind == '\n') {
            ...
            errorf(&pp_token.base.pos,
                   "unexpected %K in preprocessor condition, expected ')'",
                   &pp_token);
          }
        }
        ...
        warningf(WARN_UNDEF, &pp_token.base.pos, "'%s' is not defined",
                 pp_token.base.symbol->string);
        next_condition_token();
        return pp_null;
      }
  }
...
static ir_tarval *parse_pp_expression(precedence_t const prev_prec) {
  ...
  for (;;) {
    ...
    if ((res != tarval_bad && right != tarval_bad) || kind == '?') {
      ...
      switch (kind) {
        case '*':
          res = tarval_mul(res, right);
          break;
        case '/':
          res = tarval_div(res, right);
          break;
        case '%':
          res = tarval_mod(res, right);
          break;
        case '+':
          res = tarval_add(res, right);
          break;
        case '-':
          res = tarval_sub(res, right);
          break;
        case T_LESSLESS:
          res = tarval_shl(res, right);
          break;
        case T_GREATERGREATER:
          res = (mode_is_signed(mode) ? tarval_shrs : tarval_shr)(res, right);
          break;
        ...
        case '&':
          res = tarval_and(res, right);
          break;
        case '^':
          res = tarval_eor(res, right);
          break;
        case '|':
          res = tarval_or(res, right);
          break;
        case T_ANDAND:
          res =
              !tarval_is_null(res) && !tarval_is_null(right) ? pp_one : pp_null;
          break;
        case T_PIPEPIPE:
          res =
              !tarval_is_null(res) || !tarval_is_null(right) ? pp_one : pp_null;
...
static void parse_else_directive(void) {
  ...
  if (!conditional) {
    ...
    if (!conditional->skip) skip_mode = conditional->condition;
    conditional->pos = pp_token.base.pos;
  }
  eat_pp(TP_else);
...
static void parse_endif_directive(void) {
  ...
  if (!conditional) {
    ...
    if (!conditional->skip) skip_mode = false;
    pop_conditional();
  }
  eat_pp(TP_endif);
...
static void parse_line_directive(void) {
  if (pp_token.kind != T_NUMBER) {
    if (!skip_mode) parse_error("expected integer");
  } else {
    char *end;
      ...
      case T_NUMBER: {
...

        {
          ...
          string:;
            ...
            obstack_printf(&pp_obstack, "%s%c%s%c", enc, delim, str->begin,
                           delim);
            break;
        }
...
static void parse_ident_directive(char const *const ctx) {
  ...
  if (pp_token.kind == T_STRING_LITERAL &&
    ...
    errorf(&pp_token.base.pos,
           "expected string literal in %s directive, got %K", ctx, &pp_token);
    eat_pp_directive();
  }
}
...
static void parse_preprocessing_directive(void) {
  ...
  if (pp_token.kind == '\n') {
    ...
    switch (pp_token.base.symbol->pp_ID) {
      ...
      case TP_endif:
        ...
        break;
      case TP_error:
        parse_diagnostic_directive(true);
        break;
      case TP_ident:
        parse_ident_directive("#ident");
        break;
      case TP_if:
        parse_if_directive();
        break;
      case TP_ifdef:
        parse_ifdef_ifndef_directive(true);
        break;
      case TP_ifndef:
        parse_ifdef_ifndef_directive(false);
        break;
      case TP_include:
        parse_include_directive(false);
        break;
      case TP_include_next:
        parse_include_directive(true);
        break;
      case TP_line:
        next_input_token();
        goto line_directive;
      case TP_pragma:
        ...
        break;
      case TP_sccs:
        parse_ident_directive("#sccs");
        break;
      case TP_undef:
        parse_undef_directive();
        break;
      case TP_warning:
        parse_diagnostic_directive(false);
        break;
      default:
        goto skip;
    }
  ...
  skip:
    if (!skip_mode)
      errorf(&pp_token.base.pos, "invalid preprocessing directive #%#K",
             &pp_token);
    eat_pp_directive();
  }
      ...
      if (pp_definition != NULL && !pp_token.base.expansion_forbidden) {
        ...
        if (pp_definition->is_expanding && !pp_definition->may_recurse) {
          ...
          if (current_expansion != NULL) {
            ...
            if (next_token != '(') {
              if (next_token == T_EOF && may_pop) goto try_input;
              goto have_token;
            }
          } else {
          try_input:;
          ...
          if (current_expansion == NULL) expansion_pos = pp_token.base.pos;
          push_macro_call();
  ...
  if (current_call.macro != NULL) {
    if (kind == '(') {
      ...
      if (current_call.parameter_idx >= current_call.macro->n_parameters) {
        errorf(&pp_token.base.pos, "too many arguments passed for macro '%Y'",
               current_call.macro->symbol);
        current_call.parameter = NULL;
      } else {
        start_argument(
            &current_call.macro->parameters[current_call.parameter_idx]);
      }
      return true;
...
static void next_condition_token(void) {
  do {
    if (!expand_next()) next_input_token();
  } while (start_expanding());
}

void append_include_path(searchpath_t *const paths, char const *path,
                         bool const force_sysroot) {
  if (isysroot) {
    if (*path == '=') {
      ++path;
...
void print_include_paths(void) {
  ...
  fprintf(stderr, "#include <...> search starts here:\n");
  for (searchpath_entry_t *entry = bracket_searchpath.first; entry != NULL;
       entry = entry->next) {
    fprintf(stderr, " %s\n", entry->path);
  }
...
void print_defines(void) {
  ...
  for (symbol_t *symbol;
       (symbol = symbol_table_iterator_next(&iter)) != NULL;) {
    pp_definition_t *definition = symbol->pp_definition;
    if (definition == NULL) continue;
    if (definition->update != NULL) definition->update(definition);
...
static void input_error(unsigned const delta_lines, unsigned const delta_cols,
                        char const *const message) {
  position_t pos = input.pos;
  /* see hack at end for switch_input() */
  if (pos.lineno == 0) pos.lineno = 1;
  if (delta_lines > 0) {
...
void exit_preprocessor(void) {
  if (macro_call_stack == NULL) return;
