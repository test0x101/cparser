...
static void mangle_parameters(const function_type_t *type) {
  ...
  if (type->kr_style_parameters) panic("can't mangle kr_style_parameters type");
...
static void mangle_class_type(const compound_type_t *type) {
  ...
  if (sym == NULL) {
    if (type->compound->alias == NULL) panic("mangling anonymous type");
...
static void mangle_enum_type(const enum_type_t *type) {
  ...
  if (sym == NULL) {
    if (type->enume->alias == NULL) panic("mangling anonymous type");
...
static void mangle_qualifiers(type_qualifiers_t qualifiers) {
	...
	if (qualifiers & TYPE_QUALIFIER_RESTRICT)
		obstack_1grow(&obst, 'r');
#endif
  if (qualifiers & TYPE_QUALIFIER_VOLATILE) obstack_1grow(&obst, 'V');
  if (qualifiers & TYPE_QUALIFIER_CONST) obstack_1grow(&obst, 'K');
...
ident *create_ld_ident(entity_t const *const entity) {
  ...
  if (entity->kind == ENTITY_FUNCTION) {
    ...
    if (cc_prefix != 0) obstack_1grow(&obst, cc_prefix);

    mangle_cxx = type->function.linkage == LINKAGE_CXX;

    if (cc == CC_STDCALL || cc == CC_FASTCALL) {
      size_suffix = 0;
      for (function_parameter_t const *i = type->function.parameters; i != NULL;
           i = i->next) {
  ...
  if (mangle_cxx) {
    ...
    if (size_suffix != ~0u) obstack_printf(&obst, "@%u", size_suffix);
  }
...
void init_mangle(void) { obstack_init(&obst); }

void exit_mangle(void) { obstack_free(&obst, NULL); }
