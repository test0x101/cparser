...
static const struct params {
  ...
  const char *description; /**< description of this option */
    ...
    {X("no-const-fold"), &firm_opt.const_folding, 0,
     "disable constant folding"},
    {X("inline-max-size=<size>"), NULL, 0,
     "set maximum size for function inlining"},
    {X("inline-threshold=<size>"), NULL, 0,
    ...
    {X("unroll-max-size=<size>"), NULL, 0,
     "set maximum size of loops for loop unrolling"},
    {X("unroll-factor=<size>"), NULL, 0,
     "set unroll factor for loop unrolling"},
    ...
    {X("dump-all-phases"), &firm_dump.all_phases, 1,
     "dump graphs for all optimization phases"},
...
static void rts_map(ir_graph *irg) {
  if (rts_intrinsic_map == NULL) rts_intrinsic_map = make_rts_map();
...
static void do_stred(ir_graph *irg) {
  opt_osr(irg, osr_flag_default | osr_flag_keep_reg_pressure |
...
static void do_cloning(void) { proc_cloning((float)firm_opt.clone_threshold); }

static void do_lower_mux(ir_graph *irg) { lower_mux(irg, NULL); }
...
#define IRG(a, b, c, d) \
  { OPT_TARGET_IRG, a, .u.transform_irg = b, c, d }
#define IRP(a, b, c, d) \
  { OPT_TARGET_IRP, a, .u.transform_irp = b, c, d }
    ...
    IRG("confirm", construct_confirms, "confirm optimization",
        OPT_FLAG_HIDE_OPTIONS),
    IRG("control-flow", optimize_cf, "optimization of control-flow",
        OPT_FLAG_HIDE_OPTIONS),
    IRG("dead", dead_node_elimination, "dead node elimination",
        OPT_FLAG_HIDE_OPTIONS | OPT_FLAG_NO_DUMP | OPT_FLAG_NO_VERIFY),
    ...
    IRG("gvn-pre", do_gvn_pre,
        "global value numbering partial redundancy elimination", OPT_FLAG_NONE),
    IRG("if-conversion", opt_if_conv, "if-conversion", OPT_FLAG_NONE),
    IRG("invert-loops", do_loop_inversion, "loop inversion", OPT_FLAG_NONE),
    IRG("ivopts", do_stred, "induction variable strength reduction",
        OPT_FLAG_NONE),
    IRG("local", optimize_graph_df, "local graph optimizations",
        OPT_FLAG_HIDE_OPTIONS),
    IRG("lower", lower_highlevel_graph, "lowering",
        OPT_FLAG_HIDE_OPTIONS | OPT_FLAG_ESSENTIAL),
    IRG("lower-mux", do_lower_mux, "mux lowering", OPT_FLAG_NONE),
    IRG("opt-load-store", optimize_load_store, "load store optimization",
        OPT_FLAG_NONE),
    IRG("memcombine", combine_memops, "combine adjacent memory operations",
        OPT_FLAG_NONE),
    IRG("opt-tail-rec", opt_tail_rec_irg, "tail-recursion elimination",
        OPT_FLAG_NONE),
    IRG("parallelize-mem", opt_parallelize_mem, "parallelize memory",
        OPT_FLAG_NONE),
    IRG("gcse", do_gcse, "global common subexpression elimination",
        OPT_FLAG_NONE),
    IRG("place", place_code, "code placement", OPT_FLAG_NONE),
    IRG("reassociation", optimize_reassociation, "reassociation",
        OPT_FLAG_NONE),
    IRG("remove-confirms", remove_confirms, "confirm removal",
        OPT_FLAG_HIDE_OPTIONS | OPT_FLAG_NO_DUMP | OPT_FLAG_NO_VERIFY),
    IRG("remove-phi-cycles", remove_phi_cycles, "removal of phi cycles",
        OPT_FLAG_HIDE_OPTIONS),
    IRG("scalar-replace", scalar_replacement_opt, "scalar replacement",
        OPT_FLAG_NONE),
    IRG("shape-blocks", shape_blocks, "block shaping", OPT_FLAG_NONE),
    IRG("thread-jumps", opt_jumpthreading, "path-sensitive jumpthreading",
        OPT_FLAG_NONE),
    ...
    IRG("rts", rts_map, "optimization of known library functions",
        OPT_FLAG_NONE),
    IRP("inline", do_inline, "inlining", OPT_FLAG_NONE),
    IRP("lower-const", lower_const_code, "lowering of constant code",
        OPT_FLAG_HIDE_OPTIONS | OPT_FLAG_NO_DUMP | OPT_FLAG_NO_VERIFY |
            OPT_FLAG_ESSENTIAL),
    IRP("local-const", local_opts_const_code,
        "local optimisation of constant initializers",
        OPT_FLAG_HIDE_OPTIONS | OPT_FLAG_NO_DUMP | OPT_FLAG_NO_VERIFY |
            OPT_FLAG_ESSENTIAL),
    IRP("target-lowering", be_lower_for_target,
        "lowering necessary for target architecture",
        OPT_FLAG_HIDE_OPTIONS | OPT_FLAG_ESSENTIAL),
    IRP("opt-func-call", optimize_funccalls, "function call optimization",
        OPT_FLAG_NONE),
    IRP("opt-proc-clone", do_cloning, "procedure cloning", OPT_FLAG_NONE),
    IRP("remove-unused", garbage_collect_entities,
        "removal of unused functions/variables",
        OPT_FLAG_NO_DUMP | OPT_FLAG_NO_VERIFY),
    IRP("opt-cc", mark_private_methods, "calling conventions optimization",
        OPT_FLAG_NONE),
#undef IRP
#undef IRG
};
...
static opt_config_t *get_opt(const char *name) {
  FOR_EACH_OPT(config) {
    if (streq(config->name, name)) return config;
  }
...
static void set_opt_enabled(const char *name, bool enabled) {
  ...
  config->flags =
      (config->flags & ~OPT_FLAG_ENABLED) | (enabled ? OPT_FLAG_ENABLED : 0);
}
...
static void after_transform(ir_graph *irg, const char *name) {
  if (firm_dump.all_phases && firm_dump.ir_graph) dump_ir_graph(irg, name);
...
static bool do_irg_opt(ir_graph *irg, const char *name) {
  ...
  if (!(config->flags & OPT_FLAG_ENABLED)) return false;
...
static void do_irp_opt(const char *name) {
  ...
  if (!(config->flags & OPT_FLAG_ENABLED)) return;
...
static void do_firm_optimizations(void) {
  ...
  if (firm_opt.no_alias) aa_opt = aa_opt_no_alias;
  ...
  if (get_opt_enabled("ivopts")) set_opt_enabled("remove-phi-cycles", false);
...
static void dump_types(char const *const name) {
  if (firm_dump.all_types) dump_ir_prog_ext(dump_typegraph, name);
}
...
void optimize_lower_ir_prog(void) {
  ...
  ir_add_dump_flags(ir_dump_flag_keepalive_edges | ir_dump_flag_consts_local |
...
void generate_code(FILE *out, const char *input_filename) {
  ...
  if (experimental) warningf(WARN_EXPERIMENTAL, NULL, "%s", experimental);
...
void exit_firm_opt(void) { ir_finish(); }
...
static bool firm_opt_option(const char *opt) {
  ...
  if (config == NULL || (config->flags & OPT_FLAG_HIDE_OPTIONS)) return false;
...
void firm_option_help(print_option_help_func print_option_help) {
  FOR_EACH_OPT(config) {
    ...
    if (config->flags & OPT_FLAG_HIDE_OPTIONS) continue;
...
int firm_option(const char *const opt) {
  ...
  if (firm_opt_option(opt)) return 1;
