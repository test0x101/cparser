...
static ir_node *uninitialized_local_var(ir_graph *irg, ir_mode *mode, int pos) {
  ...
  if (entity)
    warningf(WARN_UNINITIALIZED, &entity->base.pos,
             "%N might be used uninitialized", entity);
...
static dbg_info *get_dbg_info(const position_t *pos) { return (dbg_info *)pos; }
...
static void set_unreachable_now(void) { set_cur_block(NULL); }
...
static ir_node *expression_to_control_flow(expression_t const *expr,
...
static ir_type *create_primitive_irtype(type_t const *const type,
                                        ir_mode *const mode) {
...
static ir_type *create_complex_type(type_t const *const type) {
  type_dbg_info *const dbgi = get_type_dbg_info_(type);
  type_t *const etype =
      make_atomic_type(type->atomic.akind, TYPE_QUALIFIER_NONE);
...
static mtp_additional_properties get_additional_type_properties(
  ...
  if (modifiers & DM_RETURNS_TWICE) result |= mtp_property_returns_twice;
  if (modifiers & DM_NORETURN) result |= mtp_property_noreturn;
  if (modifiers & DM_NOTHROW) result |= mtp_property_nothrow;
  if (modifiers & DM_MALLOC) result |= mtp_property_malloc;
  return result;
}
...
static mtp_additional_properties get_additional_entity_properties(
    const decl_modifiers_t modifiers) {
  mtp_additional_properties result = mtp_no_property;
  if (modifiers & DM_NOINLINE) result |= mtp_property_noinline;
  if (modifiers & DM_FORCEINLINE) result |= mtp_property_always_inline;
  if (modifiers & DM_NAKED) result |= mtp_property_naked;
  return result;
}

static unsigned determine_calling_convention(function_type_t const *const ft) {
  switch (ft->calling_convention) {
    case CC_DEFAULT: /* unspecified calling convention, equal to one of the
                        other, typically cdecl */
    case CC_CDECL:
    is_cdecl:
      return cc_cdecl_set;

    case CC_STDCALL:
      if (ft->variadic) goto is_cdecl;
...
static ir_type *create_method_type(const function_type_t *function_type) {
  ...
  bool const is_variadic =
      function_type->variadic || function_type->unspecified_parameters;
  unsigned const cc = determine_calling_convention(function_type);
  ir_type *const irtype =
      new_type_method(n_parameters, n_results, is_variadic, cc, props);
...
static ir_type *create_pointer_type(type_t const *const type,
  ...
  if (type->base.firm_type) return type->base.firm_type;
...
static ir_type *create_compound_type(type_t *const type) {
  ...
  ir_type *const irtype = is_union ? new_type_union(id) : new_type_struct(id);
  ...
  for (entity_t *entry = compound->members.first_entity; entry != NULL;
    ...
    if (entry->kind != ENTITY_COMPOUND_MEMBER) continue;
    ...
    if (symbol == NULL) {
      ...
      if (entry->compound_member.bitfield) continue;
    ...
    if (entry->compound_member.bitfield) {
      set_entity_bitfield_offset(entity, entry->compound_member.bit_offset);
...
static ir_type *create_ir_type(type_t *const type) {
  switch (type->kind) {
    case TYPE_ARRAY:
      return create_array_type(type);
    case TYPE_ATOMIC:
    case TYPE_ENUM:
    case TYPE_IMAGINARY:
      return create_atomic_type(type);
    case TYPE_COMPLEX:
      return create_complex_type(type);
    case TYPE_COMPOUND_STRUCT:
    case TYPE_COMPOUND_UNION:
      return create_compound_type(type);
    case TYPE_FUNCTION:
      return create_method_type(&type->function);
    case TYPE_POINTER:
      return create_pointer_type(type, type->pointer.points_to);
    case TYPE_REFERENCE:
      return create_pointer_type(type, type->reference.refers_to);
    case TYPE_VOID:
      return create_primitive_irtype(type, mode_ANY);
...
static ir_type *get_ir_type(type_t *type) {
  ...
  if (!*irtype) {
    ...
    if (!*irutype) *irutype = create_ir_type(utype);
    *irtype = *irutype;
  }
  return *irtype;
}
...
static bool is_main(entity_t const *const entity) {
  ...
  if (entity->base.symbol != sym_main) return false;
...
static ir_entity *get_function_entity(function_t *const function) {
  if (function->irentity != NULL) return function->irentity;

  if (function->btk != BUILTIN_NONE && !function->builtin_in_lib) return NULL;
  ...
  if (irentity != NULL) goto entity_created;
  ...
  if (function->alias.entity != NULL) {
    ...
    irentity = new_alias_entity(owner, ld_id, NULL, ir_type_method, visibility);
  } else {
 ...
 */
static ir_node *create_conv(dbg_info *dbgi, ir_node *value,
                            ir_mode *dest_mode) {
  ir_mode *value_mode = get_irn_mode(value);

  if (value_mode == dest_mode) return value;

  return new_d_Conv(dbgi, value, dest_mode);
}

static ir_node *conv_to_storage_type(dbg_info *const dbgi, ir_node *const val,
                                     type_t *const type) {
...
finish:;
  ...
  entity = new_global_entity(global_type, id, type, ir_visibility_private,
...
static ir_node *string_literal_to_firm(
    string_literal_expression_t const *const expr) {
...
static ir_node *deref_address(dbg_info *const dbgi, type_t *const type,
  ...
  if (is_type_compound(skipped) || is_type_function(skipped) ||
...
static ir_node *reference_expression_to_firm(
...
static ir_node *process_builtin_call(const call_expression_t *call) {
  ...
  switch ((builtin_kind_t)builtin->entity->function.btk) {
    ...
    case BUILTIN_OBJECT_SIZE: {
      ...
      ir_tarval *result =
          (type_val & 2) ? get_mode_null(mode) : get_mode_all_one(mode);
    ...
    case BUILTIN_ROTL: {
      ...
      ir_node *shlop =
          builtin->entity->function.btk == BUILTIN_ROTL ? val : sub;
      ir_node *shrop =
          builtin->entity->function.btk == BUILTIN_ROTR ? val : sub;
...
static ir_node *call_expression_to_firm(const call_expression_t *const call) {
  ...
  if (!firm_builtin) callee = expression_to_value(function);
  ...
  if (function_type->variadic || function_type->unspecified_parameters) {
    ...
    new_method_type =
        new_type_method(n_parameters, n_res, true, cc_mask, props);
  ...
  for (int n = 0; n < n_parameters; ++n) {
    ...
    if (is_type_complex(arg_type)) {
      ...
      if (is_type_compound(arg_type)) has_compound_parameter = true;
      in[n] =
          conv_to_storage_type(dbgi, expression_to_value(expression), arg_type);
    }
...
static ir_node *bitfield_store_to_firm(dbg_info *dbgi, ir_entity *entity,
                                       ir_node *addr, ir_node *value,
                                       bool set_volatile, bool need_return) {
...
static ir_node *set_value_for_expression_addr(const expression_t *expression,
  ...
  if (addr == NULL) addr = expression_to_addr(expression);
  assert(addr != NULL);
  ...
  if (expression->kind == EXPR_SELECT) {
    ...
    if (entity->compound_member.bitfield) {
      ...
      value = bitfield_store_to_firm(dbgi, irentity, addr, value, set_volatile,
                                     true);
      return value;
    }
  }
...
static ir_node *get_value_from_lvalue(const expression_t *expression,
  ...
  if (expression->kind == EXPR_REFERENCE) {
    ...
    entity_t *entity = ref->entity;
    assert(entity->kind == ENTITY_VARIABLE || entity->kind == ENTITY_PARAMETER);
    assert(entity->declaration.kind != DECLARATION_KIND_UNKNOWN);
    int value_number;
    if (entity->declaration.kind == DECLARATION_KIND_LOCAL_VARIABLE ||
        entity->declaration.kind == DECLARATION_KIND_PARAMETER) {
...
static ir_node *incdec_to_firm(unary_expression_t const *const expr,
                               bool const inc, bool const pre) {
  ...
  ir_node *const new_value = inc ? new_d_Add(dbgi, value_arith, offset)
                                 : new_d_Sub(dbgi, value_arith, offset);

  ir_node *const store_value =
...
static bool is_local_variable(expression_t *expression) {
  if (expression->kind != EXPR_REFERENCE) return false;
...
static ir_node *handle_assume_compare(dbg_info *dbi,
  ...
  if (is_local_variable(op1) &&
      is_constant_expression(op2) != EXPR_CLASS_VARIABLE) {
    var = op1->reference.entity;
    con = op2;
  } else if (is_constant_expression(op1) != EXPR_CLASS_VARIABLE &&
             is_local_variable(op2)) {
...
static ir_node *create_cast(expression_t const *const expr) {
  ...
  if (is_type_void(to_type)) return NULL;
...
static ir_node *complement_to_firm(unary_expression_t const *const expr) {
  ...
  ir_mode *const mode = get_ir_mode_arithmetic(type);
  ir_node *const value =
      create_conv(dbgi, expression_to_value(expr->value), mode);
...
static ir_node *create_op(binary_expression_t const *const expr, ir_node *left,
                          ir_node *right) {
  ir_mode *mode;
  dbg_info *const dbgi = get_dbg_info(&expr->base.pos);
...
static bool is_builtin_expect(const expression_t *expression) {
  if (expression->kind != EXPR_CALL) return false;

  expression_t *function = expression->call.function;
  if (function->kind != EXPR_REFERENCE) return false;
...
static void compare_to_control_flow(expression_t const *const expr,
  ...
  if (is_Const(cmp)) {
    ...
    if (is_builtin_expect(expr) && is_Cond(cond)) {
      ...
      if (is_constant_expression(argument->expression) != EXPR_CLASS_VARIABLE) {
        ...
        cond_jmp_predicate const pred =
            cnst ? COND_JMP_PRED_TRUE : COND_JMP_PRED_FALSE;
...
static ir_node *binop_assign_to_firm(binary_expression_t const *const expr) {
  ...
  if (is_type_atomic(type, ATOMIC_TYPE_BOOL)) {
    ...
    compare_to_control_flow((expression_t const *)expr, result, zero,
                            ir_relation_unordered_less_greater, &true_target,
                            &false_target);
    result = control_flow_to_1_0((expression_t const *)expr, &true_target,
                                 &false_target);
  }
...
static ir_node *assign_expression_to_firm(
    binary_expression_t const *const expr) {
...
static ir_node *comma_expression_to_firm(
    binary_expression_t const *const expr) {
...
static ir_node *array_access_to_firm(
  ...
  type_t *type =
      revert_automatic_type_conversion((const expression_t *)expression);
  type = skip_typeref(type);
...
static ir_node *compound_literal_addr(
    compound_literal_expression_t const *const expression) {
  dbg_info *dbgi = get_dbg_info(&expression->base.pos);
...
static ir_node *conditional_to_firm(
  ...
  jump_target true_target = init_jump_target(NULL);
  jump_target false_target = init_jump_target(NULL);
  ir_node *const cond_expr = expression_to_control_flow(
      expression->condition, &true_target, &false_target);

  ir_node *val = NULL;
  dbg_info *const dbgi = get_dbg_info(&expression->base.pos);
  type_t *const type = skip_typeref(expression->base.type);
  ir_mode *const mode =
      is_type_void(type) ? NULL : get_ir_mode_arithmetic(type);
  jump_target exit_target = init_jump_target(NULL);
  ...
  if (enter_jump_target(&true_target)) {
    ...
    if (val) val = create_conv(dbgi, val, mode);
    jump_to_target(&exit_target);
  }

  if (enter_jump_target(&false_target)) {
    ir_node *false_val = expression_to_value(expression->false_expression);
    if (false_val) false_val = create_conv(dbgi, false_val, mode);
    jump_to_target(&exit_target);
    if (val) {
      ir_node *const in[] = {val, false_val};
      val = new_rd_Phi(dbgi, exit_target.block, ARRAY_SIZE(in), in,
                       get_irn_mode(val));
    } else {
      val = false_val;
    }
  ...
  if (!enter_jump_target(&exit_target)) {
    set_unreachable_now();
    if (!is_type_void(type)) val = new_Bad(mode);
  }
  return val;
}
...
static ir_node *make_name(funcname_expression_t const *const expr,
...
static ir_node *statement_expression_to_firm(
    const statement_expression_t *expr) {
  ...
  if (!currently_reachable()) {
    set_soft_unreachable();
    type_t *const type = skip_typeref(expr->base.type);
    if (!is_type_void(type)) res = new_Bad(get_ir_mode_arithmetic(type));
  }
  return res;
}
...
static ir_node *va_start_expression_to_firm(
  ...
  if (is_Pointer_type(va_list_type)) {
    ...
    ir_type *const funtype =
        new_type_method(0, 1, false, cc_cdecl_set, mtp_no_property);
    set_method_res_type(funtype, 0, va_list_type);

    ir_node *const builtin =
        new_d_Builtin(dbgi, memory, 0, NULL, ir_bk_va_start, funtype);
    ...
    ir_type *const funtype =
        new_type_method(1, 0, false, cc_cdecl_set, mtp_no_property);
...
static ir_node *va_arg_expression_to_firm(
    const va_arg_expression_t *const expr) {
  dbg_info *const dbgi = get_dbg_info(&expr->base.pos);
  ...
  if (is_Pointer_type(va_list_type)) {
    ...
    ir_type *const funtype =
        new_type_method(1, 2, false, cc_cdecl_set, mtp_no_property);
    ...
    ir_node *const in[] = {ap};
    ir_node *const builtin =
        new_d_Builtin(dbgi, memory, ARRAY_SIZE(in), in, ir_bk_va_arg, funtype);
    ir_node *const projM = new_d_Proj(dbgi, builtin, mode_M, pn_Builtin_M);
 ...
 */
static ir_node *va_copy_expression_to_firm(
    const va_copy_expression_t *const expr) {
  ...
  if (is_Pointer_type(va_list_type)) {
    ...
    ir_node *const copyb =
        new_d_CopyB(dbgi, mem, dst, src, va_list_type, cons_none);
    set_store(copyb);
  } else {
    panic("unknown va_list_type");
  }
...
static ir_node *expression_to_value(expression_t const *const expr) {
  ...
  switch (expr->kind) {
    ...
    case EXPR_ARRAY_ACCESS:
      return array_access_to_firm(&expr->array_access);
    case EXPR_BINARY_ASSIGN:
      return assign_expression_to_firm(&expr->binary);
    case EXPR_UNARY_CAST:
      return create_cast(expr);
    case EXPR_BINARY_COMMA:
      return comma_expression_to_firm(&expr->binary);
    case EXPR_CALL:
      return call_expression_to_firm(&expr->call);
    case EXPR_COMPOUND_LITERAL:
      return compound_literal_to_firm(&expr->compound_literal);
    case EXPR_CONDITIONAL:
      return conditional_to_firm(&expr->conditional);
    case EXPR_FUNCNAME:
      return function_name_to_firm(&expr->funcname);
    case EXPR_LABEL_ADDRESS:
      return label_address_to_firm(&expr->label_address);
    case EXPR_REFERENCE:
      return reference_expression_to_firm(&expr->reference);
    case EXPR_SELECT:
      return select_to_firm(&expr->select);
    case EXPR_SIZEOF:
      return sizeof_to_firm(&expr->typeprop);
    case EXPR_STATEMENT:
      return statement_expression_to_firm(&expr->statement);
    case EXPR_STRING_LITERAL:
      return string_literal_to_firm(&expr->string_literal);
    case EXPR_UNARY_ASSUME:
      return handle_assume(expr->unary.value);
    case EXPR_UNARY_COMPLEMENT:
      return complement_to_firm(&expr->unary);
    case EXPR_UNARY_DEREFERENCE:
      return dereference_to_firm(&expr->unary);
    case EXPR_UNARY_NEGATE:
      return negate_to_firm(&expr->unary);
    case EXPR_UNARY_PLUS:
      return expression_to_value(expr->unary.value);
    case EXPR_UNARY_TAKE_ADDRESS:
      return expression_to_addr(expr->unary.value);
    case EXPR_VA_ARG:
      return va_arg_expression_to_firm(&expr->va_arge);
    case EXPR_VA_COPY:
      return va_copy_expression_to_firm(&expr->va_copye);
    case EXPR_VA_START:
      return va_start_expression_to_firm(&expr->va_starte);
...
static ir_node *short_circuit_expression_to_control_flow(
    bool const is_and, expression_t const *const expr,
    jump_target *const true_target, jump_target *const false_target) {
  jump_target extra_target = init_jump_target(NULL);
  expression_to_control_flow(expr->binary.left,
                             is_and ? &extra_target : true_target,
                             is_and ? false_target : &extra_target);
 ...
 */
static ir_node *expression_to_control_flow(expression_t const *const expr,
                                           jump_target *const true_target,
                                           jump_target *const false_target) {
  switch (expr->kind) {
    case EXPR_UNARY_NOT:
    ...
    case EXPR_BINARY_LOGICAL_AND:
      return short_circuit_expression_to_control_flow(true, expr, true_target,
                                                      false_target);

    case EXPR_BINARY_LOGICAL_OR:
      return short_circuit_expression_to_control_flow(false, expr, true_target,
                                                      false_target);

    case EXPR_BINARY_COMMA:
      evaluate_expression_discard_result(expr->binary.left);
      return expression_to_control_flow(expr->binary.right, true_target,
                                        false_target);
    ...
    case EXPR_BINARY_NOTEQUAL: {
      ...
      if (is_type_complex(type)) {
        complex_equality_evaluation(&expr->binary, true_target, false_target,
                                    relation);
        return NULL;
      }
      ...
      ir_mode *const mode = get_ir_mode_arithmetic(type);
      ir_node *const left =
          create_conv(dbgi, expression_to_value(expr->binary.left), mode);
      ir_node *const right =
          create_conv(dbgi, expression_to_value(expr->binary.right), mode);
      compare_to_control_flow(expr, left, right, relation, true_target,
                              false_target);
      return NULL;
    }
    ...
    case EXPR_UNARY_CAST:
      if (is_type_atomic(skip_typeref(expr->base.type), ATOMIC_TYPE_BOOL)) {
        expression_to_control_flow(expr->unary.value, true_target,
                                   false_target);
        return NULL;
      }
      /* FALLTHROUGH */
...
static void store_complex(dbg_info *dbgi, ir_node *addr, type_t *type,
  ...
  ir_node *const storer =
      new_d_Store(dbgi, mem, addr, real, irtype, cons_floats);
  ...
  ir_node *const storei =
      new_d_Store(dbgi, memr, addri, imag, irtype, cons_floats);
...
static complex_value read_localvar_complex(dbg_info *dbgi,
  ...
  return (complex_value){create_conv(dbgi, real, mode_arithmetic),
                         create_conv(dbgi, imag, mode_arithmetic)};
}

static complex_value complex_deref_address(dbg_info *const dbgi, type_t *type,
                                           ir_node *const addr,
                                           ir_cons_flags flags) {
  type = skip_typeref(type);
  assert(is_type_complex(type));

  if (type->base.qualifiers & TYPE_QUALIFIER_VOLATILE) flags |= cons_volatile;
  ir_type *const irtype = get_ir_type(type);
  ir_mode *const mode = get_complex_mode_storage(type);
  ir_node *const memory = get_store();
  ir_node *const load = new_d_Load(dbgi, memory, addr, mode, irtype, flags);
...
static complex_value complex_reference_to_firm(
    const reference_expression_t *ref) {
  dbg_info *const dbgi = get_dbg_info(&ref->base.pos);
  entity_t *const entity = ref->entity;
  assert(is_declaration(entity));
  ...
  switch ((declaration_kind_t)entity->declaration.kind) {
    ...
    default: {
      ir_node *const addr = reference_addr(ref);
      return complex_deref_address(dbgi, entity->declaration.type, addr,
                                   cons_none);
    }
  }
}
...
static complex_value complex_cast_to_firm(
...
static complex_value new_complex_add(dbg_info *dbgi, complex_value left,
                                     complex_value right) {
  return (complex_value){new_d_Add(dbgi, left.real, right.real),
                         new_d_Add(dbgi, left.imag, right.imag)};
}

static complex_value new_complex_sub(dbg_info *dbgi, complex_value left,
                                     complex_value right) {
  return (complex_value){new_d_Sub(dbgi, left.real, right.real),
...
static complex_value new_complex_mul(dbg_info *dbgi, complex_value left,
  ...
  return (complex_value){new_d_Sub(dbgi, op1, op2), new_d_Add(dbgi, op3, op4)};
}
...
static complex_value new_complex_div(dbg_info *dbgi, complex_value left,
  ...
  return (complex_value){create_div(dbgi, real_dividend, real_divisor),
...
static complex_value new_complex_increment(dbg_info *dbgi, complex_value value,
  ...
  return (complex_value){new_d_Add(dbgi, value.real, one), value.imag};
}

static complex_value new_complex_decrement(dbg_info *dbgi, complex_value value,
                                           ir_mode *mode) {
  ir_node *one = new_Const(get_mode_one(mode));
  return (complex_value){new_d_Sub(dbgi, value.real, one), value.imag};
}
...
static complex_value complex_negate_to_firm(const unary_expression_t *expr) {
  ...
  return (complex_value){new_d_Minus(dbgi, cvalue.real),
                         new_d_Minus(dbgi, cvalue.imag)};
}

static complex_value complex_complement_to_firm(
    const unary_expression_t *expr) {
  complex_value cvalue = expression_to_complex(expr->value);
  dbg_info *dbgi = get_dbg_info(&expr->base.pos);
  ir_mode *mode = get_complex_mode_arithmetic(expr->base.type);
  cvalue = complex_conv(dbgi, cvalue, mode);
  return (complex_value){cvalue.real, new_d_Minus(dbgi, cvalue.imag)};
}
...
static complex_value create_complex_assign_binop(
    const binary_expression_t *binexpr, new_complex_binop constructor) {
  dbg_info *dbgi = get_dbg_info(&binexpr->base.pos);
...
static void complex_equality_evaluation(const binary_expression_t *binexpr,
  ...
  add_pred_to_jump_target(false_target, false_proj);
  if (!enter_jump_target(&extra_target)) return;
...
static complex_value complex_to_control_flow(
  ...
  if (!enter_jump_target(&extra_target)) return value;
...
static complex_value complex_conditional_to_firm(
  ...
  if (expression->true_expression == NULL) {
    ...
    cond_val = complex_to_control_flow(expression->condition, &true_target,
                                       &false_target);
  } else {
    expression_to_control_flow(expression->condition, &true_target,
                               &false_target);
  }
  ...
  for (; statement != NULL; statement = next) {
    ...
    if (next == NULL) {
      ...
      if (statement->kind != STATEMENT_EXPRESSION)
        panic(
            "last member of complex statement expression not an expression "
            "statement");
...
static complex_value expression_to_complex(const expression_t *expression) {
  switch (expression->kind) {
    ...
    case EXPR_LITERAL_FLOATINGPOINT: {
      ...
      dbg_info *dbgi = get_dbg_info(&expression->base.pos);
      return (complex_value){new_d_Const(dbgi, cnst.real),
...
static void create_variable_entity(entity_t *variable,
  ...
  unsigned const alignment = get_declaration_alignment(&variable->declaration);
  ...
  if (variable->variable.alias.entity != NULL) {
    ...
    irentity = new_alias_entity(parent_type, ld_id, NULL, irtype, visibility);
  } else if (is_segment_type(parent_type)) {
    irentity =
        new_global_entity(parent_type, ld_id, irtype, visibility, linkage);
  } else {
...
static size_t get_compound_member_count(const compound_type_t *type) {
  ...
  size_t n_members = 0;
  for (entity_t const *member = compound->members.first_entity; member != NULL;
...
static void descend_into_subtype(type_path_t *path,
  ...
  if (initializer == NULL ||
    ...
    if (path_len > 1) {
      ...
      set_initializer_compound_value(tinitializer, entry->index, initializer);
    }
  }
...
static void walk_designator(type_path_t *path, const designator_t *designator) {
  ...
  for (; designator != NULL; designator = designator->next) {
    ...
    if (designator->symbol != NULL) {
        ...
        if (type->kind == TYPE_COMPOUND_UNION) {
          ...
          if (initializer != NULL &&
            ...
            if (get_initializer_kind(oldi) == IR_INITIALIZER_NULL) {
              /* clear initializer */
              size_t len = get_initializer_compound_n_entries(initializer);
              ir_initializer_t *nulli = get_initializer_null();
              for (size_t i = 0; i < len; ++i) {
                set_initializer_compound_value(initializer, i, nulli);
              }
            }
          }
        }
      ...
      assert(0 <= index_long && (!type->array.size_constant ||
                                 (size_t)index_long < type->array.size));
    ...
    if (designator->next != NULL) descend_into_subtype(path, &designator->pos);
  }
...
static ir_initializer_t *make_complex_init(ir_node *const real,
                                           ir_node *const imag) {
...
static bool is_string_type(type_t *type) {
  if (!is_type_array(type)) return false;
  ...
  for (size_t i = 0; i < initializer->len; ++i) {
    ...
    if (sub_initializer->kind == INITIALIZER_VALUE) {
      ...
      while (true) {
        ...
        if (types_compatible(top_type, expr_type)) break;
      ...
      while (true) {
        ...
        if (is_string_type(top_type)) break;
        descend_into_subtype(&path, &sub_initializer->base.pos);
      }
    }
...
static ir_initializer_t *create_ir_initializer_string(
    initializer_t const *const init, type_t *type) {
  type = skip_typeref(type);
...
static void create_dynamic_initializer_sub(dbg_info *dbgi, ir_node *addr,
                                           ir_type *type,
                                           ir_initializer_t *initializer) {
  switch (get_initializer_kind(initializer)) {
    case IR_INITIALIZER_NULL:
      if (is_compound_type(type)) goto compound_init;
      if (is_Array_type(type)) {
        /* Ignore flexible array members for initialization. */
        if (get_array_size(type) == 0) return;
        goto array_init;
      }
      ir_mode *mode = get_type_mode(type);
    ...
    case IR_INITIALIZER_CONST:
      ...
      if (is_compound_type(type)) {
        ir_node *mem = get_store();
        ir_node *copyb = new_d_CopyB(dbgi, mem, addr, node, type, cons_none);
        set_store(copyb);
        return;
      } else {
        goto store;
      ...
      compound_init:;
        ...
        for (size_t i = 0; i < n; ++i) {
          ...
          if (get_entity_bitfield_size(member) > 0) {
            create_dynamic_bitfield_init(dbgi, member_addr, member, subinit);
          } else {
            ir_type *entity_type = get_entity_type(member);
            create_dynamic_initializer_sub(dbgi, member_addr, entity_type,
                                           subinit);
          }
        }
      } else {
...
static void create_local_initializer(initializer_t *initializer, dbg_info *dbgi,
  ...
  ir_cons_flags flags = get_entity_volatility(entity) == volatility_is_volatile
                            ...
                            : cons_none;
  ir_node *const copyb =
      new_d_CopyB(dbgi, memory, addr, src_addr, irtype, flags);
  set_store(copyb);
}
...
static bool var_needs_entity(variable_t const *const var) {
  if (var->address_taken) return true;
  type_t *const type = skip_typeref(var->base.type);
  return !is_type_scalar(type) ||
...
static void create_local_variable(entity_t *entity) {
  ...
  create_variable_entity(entity, DECLARATION_KIND_LOCAL_VARIABLE_ENTITY,
                         frame_type, ir_visibility_private, IR_LINKAGE_DEFAULT);
}
...
static void create_local_static_variable(entity_t *entity) {
  ...
  ir_entity *const irentity = new_global_entity(
      var_type, id, irtype, ir_visibility_local, IR_LINKAGE_DEFAULT);
  set_entity_dbg_info(irentity, dbgi);
...
static ir_node *return_statement_to_firm(return_statement_t *statement) {
  if (!currently_reachable()) return NULL;
  ...
  if (is_type_void(type)) {
    ...
    if (statement->value) evaluate_expression_discard_result(statement->value);
    in[0] = NULL;
    in_len = 0;
  } else if (is_type_complex(type)) {
    if (statement->value) {
...
static ir_node *expression_statement_to_firm(
...
static void create_global_variable(entity_t *entity) {
  ...
  /* "common" symbols */
  if (storage == STORAGE_CLASS_NONE && entity->variable.initializer == NULL &&
      !entity->variable.thread_local && (modifiers & DM_WEAK) == 0) {
  ...
  create_variable_entity(entity, DECLARATION_KIND_GLOBAL_VARIABLE, var_type,
                         visibility, linkage);
  if (entity->variable.initializer == NULL && storage != STORAGE_CLASS_EXTERN) {
    ...
    case STORAGE_CLASS_REGISTER:
      if (entity->kind == ENTITY_FUNCTION) {
        if (entity->function.body != NULL) {
          ...
          errorf(&entity->base.pos,
                 "code generation does not support nested functions (at %N)",
                 entity);
          return;
        }
        (void)get_function_entity(&entity->function);
...
static void create_local_declarations(entity_t *first) {
  for (entity_t *e = first; e != NULL; e = e->base.next) {
    if (is_declaration(e)) create_local_declaration(e);
  }
}

static void initialize_local_declaration(entity_t *entity) {
  if (entity->base.symbol == NULL) return;
...
static ir_node *declaration_statement_to_firm(
  ...
  for (;; entity = entity->base.next) {
    if (entity->kind == ENTITY_TYPEDEF) {
      ...
      type_t *const type = skip_typeref(entity->declaration.type);
      if (is_type_array(type) && type->array.is_vla) get_vla_size(&type->array);
    } else if (is_declaration(entity)) {
      initialize_local_declaration(entity);
    }
    if (entity == last) break;
  }
...
static ir_node *if_statement_to_firm(if_statement_t *statement) {
  ...
  jump_target false_target = init_jump_target(NULL);
  if (currently_reachable())
    expression_to_control_flow(statement->condition, &true_target,
                               &false_target);
  ...
  if (statement->false_statement) statement_to_firm(statement->false_statement);
  jump_to_target(&exit_target);
...
static ir_node *do_while_statement_to_firm(do_while_statement_t *statement) {
  ...
  if (is_constant_expression(cond) != EXPR_CLASS_VARIABLE &&
      !fold_expression_to_bool(cond)) {
    /* do ... while (0);. */
    ...
    if (enter_jump_target(&continue_target))
      expression_to_control_flow(statement->condition, &body_target,
                                 &break_target);
...
static ir_node *for_statement_to_firm(for_statement_t *statement) {
  ...
  /* Create the condition. */
  expression_t *const cond = statement->condition;
  if (cond && (is_constant_expression(cond) == EXPR_CLASS_VARIABLE ||
               !fold_expression_to_bool(cond))) {
    jump_target body_target = init_jump_target(NULL);
...
static ir_switch_table *create_switch_table(
    const switch_statement_t *statement) {
...
static ir_node *case_label_to_firm(const case_label_statement_t *statement) {
  if (current_switch != NULL && !statement->is_empty_range) {
    ...
    if (statement->expression == NULL) saw_default_label = true;
...
static ir_node *computed_goto_to_firm(
    computed_goto_statement_t const *const statement) {
  if (currently_reachable()) {
...
static ir_node *asm_statement_to_firm(const asm_statement_t *statement) {
  ...
  if (!statement->is_volatile) flags |= cons_floats;
  if (statement->labels) flags |= cons_throws_exception;

  /* create asm node */
  dbg_info *dbgi = get_dbg_info(&statement->base.pos);
  ident *asm_text = new_id_from_str(statement->normalized_text->begin);
  ir_node *node = new_d_ASM(dbgi, mem, n_ins, ins, asm_text, n_constraints,
                            constraints, n_clobbers, clobbers, flags);
...
static ir_node *statement_to_firm(statement_t *const stmt) {
  ...
  switch (stmt->kind) {
    case STATEMENT_ASM:
      return asm_statement_to_firm(&stmt->asms);
    case STATEMENT_CASE_LABEL:
      return case_label_to_firm(&stmt->case_label);
    case STATEMENT_COMPOUND:
      return compound_statement_to_firm(&stmt->compound);
    case STATEMENT_COMPUTED_GOTO:
      return computed_goto_to_firm(&stmt->computed_goto);
    case STATEMENT_DECLARATION:
      return declaration_statement_to_firm(&stmt->declaration);
    case STATEMENT_DO_WHILE:
      return do_while_statement_to_firm(&stmt->do_while);
    case STATEMENT_EMPTY:
      return NULL; /* nothing */
    case STATEMENT_EXPRESSION:
      return expression_statement_to_firm(&stmt->expression);
    case STATEMENT_FOR:
      return for_statement_to_firm(&stmt->fors);
    case STATEMENT_GOTO:
      return goto_statement_to_firm(&stmt->gotos);
    case STATEMENT_IF:
      return if_statement_to_firm(&stmt->ifs);
    case STATEMENT_LABEL:
      return label_to_firm(&stmt->label);
    case STATEMENT_RETURN:
      return return_statement_to_firm(&stmt->returns);
    case STATEMENT_SWITCH:
      return switch_statement_to_firm(&stmt->switchs);
...
static void initialize_function_parameters(function_t *const function) {
  ...
  for (entity_t *parameter = function->parameters.first_entity;
    ...
    if (var_needs_entity(&parameter->variable)) {
      ...
      ir_entity *param = new_parameter_entity(frame_type, n, param_irtype);
      ...
      complex_value value =
          complex_deref_address(NULL, type, addr, cons_floats);

      parameter->declaration.kind = DECLARATION_KIND_PARAMETER;
      parameter->variable.v.value_number = next_value_number_function;
      set_irg_loc_description(irg, next_value_number_function, parameter);
      set_irg_loc_description(irg, next_value_number_function + 1, parameter);
...
static void create_function(function_t *const function) {
  ...
  if (enter_jump_target(&ijmp_target)) {
    ...
    ir_node *const op =
        n == 1 ? ijmp_ops[0] : new_Phi(n, ijmp_ops, get_irn_mode(ijmp_ops[0]));
...
static ir_entity *get_irentity(entity_t *entity) {
  switch (entity->kind) {
    case ENTITY_PARAMETER:
    case ENTITY_VARIABLE:
      return entity->variable.v.entity;
    case ENTITY_FUNCTION:
      return entity->function.irentity;
    case ENTITY_COMPOUND_MEMBER:
      return entity->compound_member.entity;
    case ENTITY_ASM_ARGUMENT:
    case ENTITY_ASM_LABEL:
    case ENTITY_CLASS:
    case ENTITY_ENUM:
...
static bool is_decl_necessary(entity_t const *const entity) {
  ...
  return decl->used || (declaration_is_definition(entity) &&
...
static void scope_to_firm(scope_t *scope) {
  ...
  for (entity_t *entity = scope->first_entity; entity != NULL;
       entity = entity->base.next) {
    if (entity->base.symbol == NULL) continue;
...
static void init_ast2firm(void) {
  if (ast2firm_initialized) return;
