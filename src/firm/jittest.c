...
void jit_compile_execute_main(void) {
  ...
  for (size_t i = 0, n = get_compound_n_members(global_type); i < n; ++i) {
    ...
    if (is_method_entity(entity) &&
      ...
      if (irg != NULL) {
        ...
        if (func == NULL) panic("Could not jit compile '%s'", ld_name);
  ...
  if (main_entity == NULL) panic("Could not find main() function");

  /* Allocate executable memory */
  char *const memory =
      mmap(0, code_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0);
  if (memory == NULL) panic("Could not mmap memory");
