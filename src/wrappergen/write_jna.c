...
static entity_t *find_typedef(const type_t *type) {
  ...
  for (; entity != NULL; entity = entity->base.next) {
    if (entity->kind != ENTITY_TYPEDEF) continue;
    if (entity->declaration.type == type) break;
  }
...
void jna_set_libname(const char *new_libname) { libname = new_libname; }
...
void write_jna_decls(FILE *output, const translation_unit_t *unit) {
  ...
  if (register_libname == NULL) register_libname = "library";
  ...
  for (; entity != NULL; entity = entity->base.next) {
    if (entity->kind == ENTITY_ENUM) {
      if (find_enum_typedef(&entity->enume) != NULL) continue;
  ...
  for (; entity != NULL; entity = entity->base.next) {
    if (entity->kind != ENTITY_FUNCTION) continue;
    if (entity->base.pos.is_system_header) continue;
    if (entity->function.elf_visibility != ELF_VISIBILITY_DEFAULT) continue;
    ...
    if (pset_new_contains(&avoid_symbols, entity->base.symbol)) continue;
