 ...
 */
static inline __attribute__((const)) int add_saturated(int x, int y) {
  int sum = x + y;
  /*
 ...
 */
static inline __attribute__((const)) unsigned popcnt(unsigned x) {
  x -= ((x >> 1) & 0x55555555);
 ...
 */
static inline __attribute__((const)) unsigned nlz(unsigned x) {
 ...
 */
static inline __attribute__((const)) unsigned ntz(unsigned x) {
#ifdef USE_X86_ASSEMBLY
  unsigned res;
  if (x == 0) return 32;

  __asm__("bsfl %1,%0" : "=r"(res) : "r"(x));
  return res;
#else
 ...
 */
static inline __attribute__((const)) unsigned floor_po2(unsigned x) {
 ...
 */
static inline __attribute__((const)) int is_po2(unsigned x) {
  return (x & (x - 1)) == 0;
}
